...
* When you run an example, you may get a low [f1 score](../glossary.html#f1) (the probability that the net's classification is accurate). This doesn't indicate poor performance in this case, because the examples train on small data sets. We gave them small data sets so they would run quickly. Because small data are less representative than large ones, the results they produce will vary a great deal. On the minuscule example data, our deep-belief net example's f1 score is currently varying between 0.32 and 1.0.
