...
class TestClassPath(unittest.TestCase):
    def test_simple_classpath(self):
        os.listdir = MagicMock(
            return_value=['nd4j-buffer-0.8.1-20170627.172301-977.jar', 'leptonica-1.73-1.3-linux-x86_64.jar'])
    ...
    def test_mix_jar_wildcard(self):
        ...
        if (os.path.sep == '/'):
            # the directory argument is moot
            self.assertEqual(get_classpath('/opt/skil/lib/nd4j-buffer-0.8.1-20170627.172301-977.jar:/opt/skil/lib/*'),
                             res)
