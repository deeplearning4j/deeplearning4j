...
/**
 * Implementation of ONNX ReduceLogSumExp operation.
 *
 * ONNX ReduceLogSumExp spec: https://github.com/onnx/onnx/blob/main/docs/Operators.md#ReduceLogSumExp
 *
 * Computes log(sum(exp(input))) along the specified axes.
 *
 ...
 * @author Adam Gibson
 */
@PreHookRule(nodeNames = [], opNames = ["ReduceLogSumExp"], frameworkName = "onnx")
class ReduceLogSumExp : PreImportHook {
    ...
    override fun doImport(
        ...
        // Handle reduction: log(sum(exp(x)))
        // For numerical stability, compute: max + log(sum(exp(x - max)))
        val output = when {
            axes == null || axes.isEmpty() -> {
                if (noopWithEmptyAxes) {
                    ...
                    // Reduce all axes - use the numerically stable formula
                    val maxVal = sd.max("${opName}_max", data, keepDims)
                    val maxBroadcast = sd.max("${opName}_max_bc", data, true)
                    val shifted = sd.math.sub("${opName}_shifted", data, maxBroadcast)
                    val expShifted = sd.math.exp("${opName}_exp", shifted)
                    val sumExp = sd.sum("${opName}_sumexp", expShifted, keepDims)
                    val logSum = sd.math.log("${opName}_logsum", sumExp)
                    sd.math.add(outputNames[0], maxVal, logSum)
                }
            }
            else -> {
                // Reduce along specific axes - use the numerically stable formula
                val maxVal = sd.max("${opName}_max", data, keepDims, *axes)
                val maxBroadcast = sd.max("${opName}_max_bc", data, true, *axes)
                val shifted = sd.math.sub("${opName}_shifted", data, maxBroadcast)
                val expShifted = sd.math.exp("${opName}_exp", shifted)
                val sumExp = sd.sum("${opName}_sumexp", expShifted, keepDims, *axes)
                val logSum = sd.math.log("${opName}_logsum", sumExp)
                sd.math.add(outputNames[0], maxVal, logSum)
            }
        }
