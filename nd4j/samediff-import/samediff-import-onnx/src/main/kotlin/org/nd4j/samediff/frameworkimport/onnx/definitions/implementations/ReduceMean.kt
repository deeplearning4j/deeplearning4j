...
/**
 * Implementation of ONNX ReduceMean operation.
 *
 * ONNX ReduceMean spec: https://github.com/onnx/onnx/blob/main/docs/Operators.md#ReduceMean
 *
 * Computes the mean of the input tensor's elements along the specified axes.
 *
 ...
 * @author Adam Gibson
 */
@PreHookRule(nodeNames = [], opNames = ["ReduceMean"], frameworkName = "onnx")
class ReduceMean : PreImportHook {
    ...
    override fun doImport(
        ...
        if (axes == null) {
            ...
            axes = when (axesAttr) {
                ...
                is Number -> longArrayOf(axesAttr.toLong())  // Handle single axis value (e.g., -1)
                else -> null
            }
        }
        ...
        val output = when {
            axes == null || axes.isEmpty() -> {
                if (noopWithEmptyAxes) {
                    ...
                    // No axes specified and noop_with_empty_axes=false
                    // Reduce over all axes (ONNX default behavior)
                    sd.mean(outputNames[0], data, keepDims)
                }
            }
            else -> {
                sd.mean(outputNames[0], data, keepDims, *axes)
            }
        }
