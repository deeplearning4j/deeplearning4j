...
/**
 * Implementation of ONNX ReduceProd operation.
 *
 * ONNX ReduceProd spec: https://github.com/onnx/onnx/blob/main/docs/Operators.md#ReduceProd
 *
 * Computes the product of the input tensor's elements along the specified axes.
 *
 ...
 */
@PreHookRule(nodeNames = [], opNames = ["ReduceProd"], frameworkName = "onnx")
class ReduceProd : PreImportHook {
        ...
        val output = when {
            axes == null || axes.isEmpty() -> {
                if (noopWithEmptyAxes) {
                    ...
                    // Reduce all axes
                    sd.prod(outputNames[0], data, keepDims)
                }
            }
            else -> {
                sd.prod(outputNames[0], data, keepDims, *axes)
            }
        }
