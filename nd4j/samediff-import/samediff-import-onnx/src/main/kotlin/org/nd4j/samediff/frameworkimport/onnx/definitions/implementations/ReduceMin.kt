...
/**
 * Implementation of ONNX ReduceMin operation.
 *
 * ONNX ReduceMin spec: https://github.com/onnx/onnx/blob/main/docs/Operators.md#ReduceMin
 *
 * Computes the min of the input tensor's elements along the specified axes.
 *
 ...
 * @author Adam Gibson
 */
@PreHookRule(nodeNames = [], opNames = ["ReduceMin"], frameworkName = "onnx")
class ReduceMin : PreImportHook {
        ...
        val output = when {
            axes == null || axes.isEmpty() -> {
                if (noopWithEmptyAxes) {
                    ...
                    // Reduce all axes
                    sd.min(outputNames[0], data, keepDims)
                }
            }
            else -> {
                sd.min(outputNames[0], data, keepDims, *axes)
            }
        }
