...
/**
 * ONNX Sum op: element-wise sum of variadic inputs.
 *
 * @author Adam Gibson
 */
@PreHookRule(nodeNames = [], opNames = ["Sum"], frameworkName = "onnx")
class Sum : PreImportHook {
        ...
        op.inputsToOp.forEachIndexed { index, input ->
            ...
            if (onGoingOutput == null) {
                ...
                if (index < op.inputsToOp.size - 1)
                    onGoingOutput = sd.math.add(onGoingOutput, currVariable)
                else {
                    onGoingOutput = sd.math.add(outputNames[0], onGoingOutput, currVariable)
                }
            }
        }
