...
/**
 * PreImportHook for ONNX Softmax operation.
 *
 * ONNX Softmax (opset 13+) defaults to axis=-1 (last dimension).
 * However, ND4J SoftMax defaults to dimension=1, which causes incorrect
 * results when the ONNX model doesn't explicitly set the axis attribute.
 *
 * This is critical for attention mechanisms where softmax should be applied
...
class Softmax : PreImportHook {
    ...
    override fun doImport(
        ...
        // Create softmax with the correct axis
        val result = sd.nn().softmax(input, axis)
