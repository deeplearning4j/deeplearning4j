...
/**
 * Implementation of ONNX ReduceMax operation.
 *
 * ONNX ReduceMax spec: https://github.com/onnx/onnx/blob/main/docs/Operators.md#ReduceMax
 *
 * Computes the max of the input tensor's elements along the specified axes.
 *
 ...
 */
@PreHookRule(nodeNames = [], opNames = ["ReduceMax"], frameworkName = "onnx")
class ReduceMax : PreImportHook {
        ...
        val output = when {
            axes == null || axes.isEmpty() -> {
                if (noopWithEmptyAxes) {
                    ...
                    // Reduce all axes
                    sd.max(outputNames[0], data, keepDims)
                }
            }
            else -> {
                sd.max(outputNames[0], data, keepDims, *axes)
            }
        }
