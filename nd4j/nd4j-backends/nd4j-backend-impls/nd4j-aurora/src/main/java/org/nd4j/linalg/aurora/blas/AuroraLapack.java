...
public class AuroraLapack extends BaseLapack {
    ...
    static final int LAPACK_COL_MAJOR = -1, LAPACK_ROW_MAJOR = -1;
...
// L U DECOMP
    ...
    public void sgetrf(int M, int N, INDArray A, INDArray IPIV, INDArray INFO) {
        int status = nativeOps.callInt("LAPACKE_sgetrf", getColumnOrder(A), M, N,
    ...
    public void dgetrf(int M, int N, INDArray A, INDArray IPIV, INDArray INFO) {
        int status = nativeOps.callInt("LAPACKE_dgetrf", getColumnOrder(A), M, N, (DoublePointer)A.data().addressPointer(),
            getLda(A), (IntPointer)IPIV.data().addressPointer()
            );
        if( status < 0 ) {
...
// Q R DECOMP
    ...
    public void sgeqrf(int M, int N, INDArray A, INDArray R, INDArray INFO) {
        ...
        int status = nativeOps.callInt("LAPACKE_sgeqrf", getColumnOrder(A), M, N,
        ...
        status = nativeOps.callInt("LAPACKE_sorgqr", getColumnOrder(A), M, N, N,
             (FloatPointer)A.data().addressPointer(), getLda(A),
             (FloatPointer)tau.data().addressPointer()
             );
        if( status != 0 ) {
    ...
    public void dgeqrf(int M, int N, INDArray A, INDArray R, INDArray INFO)  {
        ...
        int status = nativeOps.callInt("LAPACKE_dgeqrf", getColumnOrder(A), M, N,
        ...
        status = nativeOps.callInt("LAPACKE_dorgqr", getColumnOrder(A), M, N, N,
             (DoublePointer)A.data().addressPointer(), getLda(A),
             (DoublePointer)tau.data().addressPointer()
             );
        if( status != 0 ) {
...
// CHOLESKY DECOMP
    ...
    public void spotrf(byte uplo, int N, INDArray A, INDArray INFO) {
        int status = nativeOps.callInt("LAPACKE_spotrf", getColumnOrder(A), uplo, N,
                        (FloatPointer)A.data().addressPointer(), getLda(A) );
        if( status != 0 ) {
    ...
    public void dpotrf(byte uplo, int N, INDArray A, INDArray INFO) {
        int status = nativeOps.callInt("LAPACKE_dpotrf", getColumnOrder(A), uplo, N,
                    (DoublePointer)A.data().addressPointer(), getLda(A) );
        if( status != 0 ) {
...
// U S V' DECOMP  (aka SVD)
    ...
    public void sgesvd(byte jobu, byte jobvt, int M, int N, INDArray A, INDArray S, INDArray U, INDArray VT,
        ...
        int status = nativeOps.callInt("LAPACKE_sgesvd", getColumnOrder(A), jobu, jobvt, M, N,
                        (FloatPointer)A.data().addressPointer(), getLda(A),
    ...
    public void dgesvd(byte jobu, byte jobvt, int M, int N, INDArray A, INDArray S, INDArray U, INDArray VT,
        ...
        int status = nativeOps.callInt("LAPACKE_dgesvd", getColumnOrder(A), jobu, jobvt, M, N,
                        (DoublePointer)A.data().addressPointer(), getLda(A),
...
//
	...
	int status = nativeOps.callInt("LAPACKE_ssyev_work", getColumnOrder(A), (byte)jobz, (byte)uplo,
	...
	if( status == 0 ) {
		...
		status = nativeOps.callInt("LAPACKE_ssyev", getColumnOrder(A), (byte)jobz, (byte)uplo, N,
		            (FloatPointer)A.data().addressPointer(), getLda(A),
	...
	int status = nativeOps.callInt("LAPACKE_dsyev_work", getColumnOrder(A), (byte)jobz, (byte)uplo,
	...
	if( status == 0 ) {
		...
		status = nativeOps.callInt("LAPACKE_dsyev", getColumnOrder(A), (byte)jobz, (byte)uplo, N,
		            (DoublePointer)A.data().addressPointer(), getLda(A),
