...
//        System.out.println(Arrays.toString(dLdInExp.data().asFloat()));
    ...
    public void testSquaredNormBP() {
        ...
        for (boolean keepDims : new boolean[]{false, true}) {
            ...
            INDArray dLdOut_0 = Nd4j.create(new double[]{1, 2, 3, 4}, reducedShape_0);
            //INDArray dLdInExpected_0 = sgn.mul(max_0).mulRowVector(dLdOut_0);
            INDArray dLdInExpected_0 = Nd4j.createFromArray(new double[]{
                    2.0000,    8.0000,   18.0000,   32.0000,
                   10.0000,   24.0000,   42.0000,   64.0000,
                   18.0000,   40.0000,   66.0000,   96.0000
            }).reshape(3,4);

            INDArray dLdIn = Nd4j.createUninitialized(3, 4);

            String err = OpValidation.validate(new OpTestCase(new SquaredNormBp(preReduceInput, dLdOut_0, dLdIn, keepDims, 0))
                    .expectedOutput(0, dLdInExpected_0));
            assertNull(err);
        }
    }
    ...
    @Test
    public void testStandardDeviationBP() {
        ...
        for (boolean keepDims : new boolean[]{false, true}) {
            ...
            //INDArray dLdInExpected_0 = sgn.mul(max_0).mulRowVector(dLdOut_0);
            INDArray dLdInExpected_0 = Nd4j.createFromArray(new double[]{
                    -0.4082,   -0.8165,   -1.2247,   -1.6330,
                    0,         0,         0,         0,
                    0.4082,    0.8165,    1.2247,    1.6330
            }).reshape(3,4);

            INDArray dLdIn = Nd4j.createUninitialized(3, 4);
            String err = OpValidation.validate(new OpTestCase(new StandardDeviationBp(preReduceInput, dLdOut_0, dLdIn, false,keepDims, 0))
                    .expectedOutput(0, dLdInExpected_0));
            assertNull(err);
        }
    }
