...
public class PreProcessor3D4DTest extends BaseNd4jTest {
    ...
    public class Construct3dDataSet {
        ...
        public Construct3dDataSet(INDArray featureScale, int timeSteps, int samples, int origin) {
            ...
            expectedStd = Nd4j.create(new double[] {stdNaturalNums, stdNaturalNums, stdNaturalNums}).reshape(3, 1);
            ...
            expectedMin = Nd4j.ones(DataType.DOUBLE, 3, 1).muliColumnVector(featureScale);
            expectedMax = Nd4j.ones(DataType.DOUBLE,3, 1).muli(samples * timeSteps).muliColumnVector(featureScale);
        }
