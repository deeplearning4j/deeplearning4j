...
public class NDBase {
  ...
  public INDArray mean(INDArray x, boolean keepDims, int... dimensions) {
    ...
    Preconditions.checkArgument(dimensions.length >= 0, "dimensions has incorrect size/length. Expected: dimensions.length >= 0, got %s", dimensions.length);
    return Nd4j.exec(new org.nd4j.linalg.api.ops.impl.reduce.floating.custom.Mean(x, keepDims, dimensions))[0];
  }
  ...
  public INDArray mean(INDArray x, int... dimensions) {
    NDValidation.validateNumerical("mean", "x", x);
    Preconditions.checkArgument(dimensions.length >= 0, "dimensions has incorrect size/length. Expected: dimensions.length >= 0, got %s", dimensions.length);
    return Nd4j.exec(new org.nd4j.linalg.api.ops.impl.reduce.floating.custom.Mean(x, false, dimensions))[0];
  }
  ...
  public INDArray mean(INDArray x, INDArray dimensions, boolean keepDims) {
    ...
    return Nd4j.exec(new org.nd4j.linalg.api.ops.impl.reduce.floating.custom.Mean(x, dimensions, keepDims))[0];
  }
  ...
  public INDArray mean(INDArray x, INDArray dimensions) {
    NDValidation.validateNumerical("mean", "x", x);
    NDValidation.validateInteger("mean", "dimensions", dimensions);
    return Nd4j.exec(new org.nd4j.linalg.api.ops.impl.reduce.floating.custom.Mean(x, dimensions, false))[0];
  }
