/*
 *  ******************************************************************************
 *  *
 *  *
 *  * This program and the accompanying materials are made available under the
 *  * terms of the Apache License, Version 2.0 which is available at
 *  * https://www.apache.org/licenses/LICENSE-2.0.
 *  *
 *  *  See the NOTICE file distributed with this work for additional
 *  *  information regarding copyright ownership.
 *  * Unless required by applicable law or agreed to in writing, software
 *  * distributed under the License is distributed on an "AS IS" BASIS, WITHOUT
 *  * WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the
 *  * License for the specific language governing permissions and limitations
 *  * under the License.
 *  *
 *  * SPDX-License-Identifier: Apache-2.0
 *  *****************************************************************************
 */

//================== GENERATED CODE - DO NOT MODIFY THIS FILE ==================

package org.nd4j.autodiff.samediff.ops;

import static org.nd4j.autodiff.samediff.ops.SDValidation.isSameType;

import java.lang.String;
import org.nd4j.autodiff.samediff.SDVariable;
import org.nd4j.autodiff.samediff.SameDiff;
import org.nd4j.common.base.Preconditions;

public class SDGNN extends SDOps {
  public SDGNN(SameDiff sameDiff) {
    super(sameDiff);
  }

  /**
   * Approximate Personalized Propagation of Neural Predictions (Klicpera et al. 2019).<br>
   * Decouples prediction from propagation: k steps of personalized PageRank with teleport alpha.<br>
   *
   * @param X Node features [rows, F] (FLOATING_POINT type)
   * @param W Prediction weight [F, H] (FLOATING_POINT type)
   * @param bias Optional bias [H]; pass null to omit (FLOATING_POINT type)
   * @param aNormVals CSR values of the normalised adjacency [nnz] (FLOATING_POINT type)
   * @param aNormColIdx CSR column indices [nnz] INT32 (INT type)
   * @param aNormRowPtr CSR row pointers [rows+1] INT32 (INT type)
   * @param rows Number of nodes
   * @param cols Number of columns of A_norm
   * @param k Number of propagation steps
   * @param alpha Teleport (restart) probability in [0,1)
   * @return out Propagated node predictions [rows, H] (FLOATING_POINT type)
   */
  public SDVariable appnp(SDVariable X, SDVariable W, SDVariable bias, SDVariable aNormVals,
      SDVariable aNormColIdx, SDVariable aNormRowPtr, long rows, long cols, int k, double alpha) {
    SDValidation.validateFloatingPoint("appnp", "X", X);
    SDValidation.validateFloatingPoint("appnp", "W", W);
    SDValidation.validateFloatingPoint("appnp", "bias", bias);
    SDValidation.validateFloatingPoint("appnp", "aNormVals", aNormVals);
    SDValidation.validateInteger("appnp", "aNormColIdx", aNormColIdx);
    SDValidation.validateInteger("appnp", "aNormRowPtr", aNormRowPtr);
    SDVariable H0 = sd.mmul(X, W);
    if (bias != null) {
        H0 = H0.add(bias);
    }
    SDVariable H = H0;
    for (int i = 0; i < k; i++) {
        SDVariable AH = sd.sparse().csrSpmm(aNormVals, aNormColIdx, aNormRowPtr, H, (int) rows, (int) cols, false);
        H = AH.mul(1.0 - alpha).add(H0.mul(alpha));
    }
    SDVariable out = H;
    return out;
  }

  /**
   * Approximate Personalized Propagation of Neural Predictions (Klicpera et al. 2019).<br>
   * Decouples prediction from propagation: k steps of personalized PageRank with teleport alpha.<br>
   *
   * @param name name May be null. Name for the output variable
   * @param X Node features [rows, F] (FLOATING_POINT type)
   * @param W Prediction weight [F, H] (FLOATING_POINT type)
   * @param bias Optional bias [H]; pass null to omit (FLOATING_POINT type)
   * @param aNormVals CSR values of the normalised adjacency [nnz] (FLOATING_POINT type)
   * @param aNormColIdx CSR column indices [nnz] INT32 (INT type)
   * @param aNormRowPtr CSR row pointers [rows+1] INT32 (INT type)
   * @param rows Number of nodes
   * @param cols Number of columns of A_norm
   * @param k Number of propagation steps
   * @param alpha Teleport (restart) probability in [0,1)
   * @return out Propagated node predictions [rows, H] (FLOATING_POINT type)
   */
  public SDVariable appnp(String name, SDVariable X, SDVariable W, SDVariable bias,
      SDVariable aNormVals, SDVariable aNormColIdx, SDVariable aNormRowPtr, long rows, long cols,
      int k, double alpha) {
    SDValidation.validateFloatingPoint("appnp", "X", X);
    SDValidation.validateFloatingPoint("appnp", "W", W);
    SDValidation.validateFloatingPoint("appnp", "bias", bias);
    SDValidation.validateFloatingPoint("appnp", "aNormVals", aNormVals);
    SDValidation.validateInteger("appnp", "aNormColIdx", aNormColIdx);
    SDValidation.validateInteger("appnp", "aNormRowPtr", aNormRowPtr);
    SDVariable H0 = sd.mmul(X, W);
    if (bias != null) {
        H0 = H0.add(bias);
    }
    SDVariable H = H0;
    for (int i = 0; i < k; i++) {
        SDVariable AH = sd.sparse().csrSpmm(aNormVals, aNormColIdx, aNormRowPtr, H, (int) rows, (int) cols, false);
        H = AH.mul(1.0 - alpha).add(H0.mul(alpha));
    }
    SDVariable out = H;
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * Chebyshev spectral graph convolution (Defferrard et al. 2016).<br>
   * T_0=X, T_1=L_hat*X, T_k=2*L_hat*T_{k-1}-T_{k-2}; out=sum_k T_k*W_k<br>
   *
   * @param X Node features [rows, F] (FLOATING_POINT type)
   * @param weights K Chebyshev-coefficient matrices, each [F, H] (FLOATING_POINT type)
   * @param lapVals CSR values of the scaled Laplacian L_hat [nnz] (FLOATING_POINT type)
   * @param colIdx CSR column indices [nnz] INT32 (INT type)
   * @param rowPtr CSR row pointers [rows+1] INT32 (INT type)
   * @param rows Number of nodes
   * @param cols Number of columns of L_hat
   * @return out Filtered node embeddings [rows, H] (FLOATING_POINT type)
   */
  public SDVariable chebConv(SDVariable X, SDVariable[] weights, SDVariable lapVals,
      SDVariable colIdx, SDVariable rowPtr, long rows, long cols) {
    SDValidation.validateFloatingPoint("chebConv", "X", X);
    SDValidation.validateFloatingPoint("chebConv", "weights", weights);
    Preconditions.checkArgument(weights.length >= 1, "weights has incorrect size/length. Expected: weights.length >= 1, got %s", weights.length);
    SDValidation.validateFloatingPoint("chebConv", "lapVals", lapVals);
    SDValidation.validateInteger("chebConv", "colIdx", colIdx);
    SDValidation.validateInteger("chebConv", "rowPtr", rowPtr);
    SDVariable tPrev2 = X;
    SDVariable out = sd.mmul(X, weights[0]);
    if (weights.length >= 2) {
        SDVariable tPrev1 = sd.sparse().csrSpmm(lapVals, colIdx, rowPtr, X, (int) rows, (int) cols, false);
        out = out.add(sd.mmul(tPrev1, weights[1]));
        for (int k = 2; k < weights.length; k++) {
            SDVariable lTk = sd.sparse().csrSpmm(lapVals, colIdx, rowPtr, tPrev1, (int) rows, (int) cols, false);
            SDVariable tk = lTk.mul(2.0).sub(tPrev2);
            out = out.add(sd.mmul(tk, weights[k]));
            tPrev2 = tPrev1;
            tPrev1 = tk;
        }
    }
    return out;
  }

  /**
   * Chebyshev spectral graph convolution (Defferrard et al. 2016).<br>
   * T_0=X, T_1=L_hat*X, T_k=2*L_hat*T_{k-1}-T_{k-2}; out=sum_k T_k*W_k<br>
   *
   * @param name name May be null. Name for the output variable
   * @param X Node features [rows, F] (FLOATING_POINT type)
   * @param weights K Chebyshev-coefficient matrices, each [F, H] (FLOATING_POINT type)
   * @param lapVals CSR values of the scaled Laplacian L_hat [nnz] (FLOATING_POINT type)
   * @param colIdx CSR column indices [nnz] INT32 (INT type)
   * @param rowPtr CSR row pointers [rows+1] INT32 (INT type)
   * @param rows Number of nodes
   * @param cols Number of columns of L_hat
   * @return out Filtered node embeddings [rows, H] (FLOATING_POINT type)
   */
  public SDVariable chebConv(String name, SDVariable X, SDVariable[] weights, SDVariable lapVals,
      SDVariable colIdx, SDVariable rowPtr, long rows, long cols) {
    SDValidation.validateFloatingPoint("chebConv", "X", X);
    SDValidation.validateFloatingPoint("chebConv", "weights", weights);
    Preconditions.checkArgument(weights.length >= 1, "weights has incorrect size/length. Expected: weights.length >= 1, got %s", weights.length);
    SDValidation.validateFloatingPoint("chebConv", "lapVals", lapVals);
    SDValidation.validateInteger("chebConv", "colIdx", colIdx);
    SDValidation.validateInteger("chebConv", "rowPtr", rowPtr);
    SDVariable tPrev2 = X;
    SDVariable out = sd.mmul(X, weights[0]);
    if (weights.length >= 2) {
        SDVariable tPrev1 = sd.sparse().csrSpmm(lapVals, colIdx, rowPtr, X, (int) rows, (int) cols, false);
        out = out.add(sd.mmul(tPrev1, weights[1]));
        for (int k = 2; k < weights.length; k++) {
            SDVariable lTk = sd.sparse().csrSpmm(lapVals, colIdx, rowPtr, tPrev1, (int) rows, (int) cols, false);
            SDVariable tk = lTk.mul(2.0).sub(tPrev2);
            out = out.add(sd.mmul(tk, weights[k]));
            tPrev2 = tPrev1;
            tPrev1 = tk;
        }
    }
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * CompGCN convolution (Vashishth et al. 2020): multi-relational GNN composing entity+relation embeddings.<br>
   * compOp=0: sub (TransE-style); else: elementwise mult (DistMult-style).<br>
   *
   * @param X Entity embeddings [n, dim] (FLOATING_POINT type)
   * @param relEmb Relation embeddings [numRelations, dim] (FLOATING_POINT type)
   * @param edgeRelIdx Per-edge relation index [nnz] INT32 (INT type)
   * @param W Output weight [dim, dim_out] (FLOATING_POINT type)
   * @param colIdx CSR column indices [nnz] INT32 (INT type)
   * @param rowPtr CSR row pointers [rows+1] INT32 (INT type)
   * @param rows Number of entities (nodes)
   * @param n Number of entities (for CsrEdgeGather doDiff)
   * @param compOp Composition: 0=subtraction (TransE-style), else element-wise mult (DistMult-style)
   * @param applyRelu Whether to apply ReLU to the output
   * @return out Updated entity embeddings [rows, dim_out] (FLOATING_POINT type)
   */
  public SDVariable compGcnConv(SDVariable X, SDVariable relEmb, SDVariable edgeRelIdx,
      SDVariable W, SDVariable colIdx, SDVariable rowPtr, long rows, long n, int compOp,
      boolean applyRelu) {
    SDValidation.validateFloatingPoint("compGcnConv", "X", X);
    SDValidation.validateFloatingPoint("compGcnConv", "relEmb", relEmb);
    SDValidation.validateInteger("compGcnConv", "edgeRelIdx", edgeRelIdx);
    SDValidation.validateFloatingPoint("compGcnConv", "W", W);
    SDValidation.validateInteger("compGcnConv", "colIdx", colIdx);
    SDValidation.validateInteger("compGcnConv", "rowPtr", rowPtr);
    SDVariable Xj = sd.sparse().csrEdgeGather(colIdx, X, (int) n);
    SDVariable Rj = sd.gather(relEmb, edgeRelIdx, 0);
    SDVariable phi = (compOp == 0) ? Xj.sub(Rj) : Xj.mul(Rj);
    SDVariable agg = sd.sparse().csrEdgeAggregate(rowPtr, phi, (int) rows, 1);
    SDVariable out = sd.mmul(agg, W);
    if (applyRelu) {
        out = sd.nn().relu(out, 0.0);
    }
    return out;
  }

  /**
   * CompGCN convolution (Vashishth et al. 2020): multi-relational GNN composing entity+relation embeddings.<br>
   * compOp=0: sub (TransE-style); else: elementwise mult (DistMult-style).<br>
   *
   * @param name name May be null. Name for the output variable
   * @param X Entity embeddings [n, dim] (FLOATING_POINT type)
   * @param relEmb Relation embeddings [numRelations, dim] (FLOATING_POINT type)
   * @param edgeRelIdx Per-edge relation index [nnz] INT32 (INT type)
   * @param W Output weight [dim, dim_out] (FLOATING_POINT type)
   * @param colIdx CSR column indices [nnz] INT32 (INT type)
   * @param rowPtr CSR row pointers [rows+1] INT32 (INT type)
   * @param rows Number of entities (nodes)
   * @param n Number of entities (for CsrEdgeGather doDiff)
   * @param compOp Composition: 0=subtraction (TransE-style), else element-wise mult (DistMult-style)
   * @param applyRelu Whether to apply ReLU to the output
   * @return out Updated entity embeddings [rows, dim_out] (FLOATING_POINT type)
   */
  public SDVariable compGcnConv(String name, SDVariable X, SDVariable relEmb, SDVariable edgeRelIdx,
      SDVariable W, SDVariable colIdx, SDVariable rowPtr, long rows, long n, int compOp,
      boolean applyRelu) {
    SDValidation.validateFloatingPoint("compGcnConv", "X", X);
    SDValidation.validateFloatingPoint("compGcnConv", "relEmb", relEmb);
    SDValidation.validateInteger("compGcnConv", "edgeRelIdx", edgeRelIdx);
    SDValidation.validateFloatingPoint("compGcnConv", "W", W);
    SDValidation.validateInteger("compGcnConv", "colIdx", colIdx);
    SDValidation.validateInteger("compGcnConv", "rowPtr", rowPtr);
    SDVariable Xj = sd.sparse().csrEdgeGather(colIdx, X, (int) n);
    SDVariable Rj = sd.gather(relEmb, edgeRelIdx, 0);
    SDVariable phi = (compOp == 0) ? Xj.sub(Rj) : Xj.mul(Rj);
    SDVariable agg = sd.sparse().csrEdgeAggregate(rowPtr, phi, (int) rows, 1);
    SDVariable out = sd.mmul(agg, W);
    if (applyRelu) {
        out = sd.nn().relu(out, 0.0);
    }
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * Single-head Graph Attention Network convolution (Veličković et al. 2018).<br>
   *
   * @param X Node features [N, F] (FLOATING_POINT type)
   * @param W Linear weight [F, H] (FLOATING_POINT type)
   * @param attSrc Attention source vector [H, 1] (FLOATING_POINT type)
   * @param attDst Attention destination vector [H, 1] (FLOATING_POINT type)
   * @param colIdx CSR column indices (source nodes per edge) [nnz] INT32 (INT type)
   * @param rowPtr CSR row pointers [N+1] INT32 (INT type)
   * @param rowIdx Per-edge destination-node index [nnz] INT32 (INT type)
   * @param nnz Number of edges
   * @param N Number of nodes
   * @param leakyAlpha Negative slope for LeakyReLU
   * @return out Aggregated node embeddings [N, H] (FLOATING_POINT type)
   */
  public SDVariable gatConvHead(SDVariable X, SDVariable W, SDVariable attSrc, SDVariable attDst,
      SDVariable colIdx, SDVariable rowPtr, SDVariable rowIdx, long nnz, long N,
      double leakyAlpha) {
    SDValidation.validateFloatingPoint("gatConvHead", "X", X);
    SDValidation.validateFloatingPoint("gatConvHead", "W", W);
    SDValidation.validateFloatingPoint("gatConvHead", "attSrc", attSrc);
    SDValidation.validateFloatingPoint("gatConvHead", "attDst", attDst);
    SDValidation.validateInteger("gatConvHead", "colIdx", colIdx);
    SDValidation.validateInteger("gatConvHead", "rowPtr", rowPtr);
    SDValidation.validateInteger("gatConvHead", "rowIdx", rowIdx);
    SDVariable Wh = sd.mmul(X, W);
    SDVariable srcFeat = sd.sparse().csrEdgeGather(colIdx, Wh, (int) N);
    SDVariable dstFeat = sd.gather(Wh, rowIdx, 0);
    SDVariable eSrc = sd.reshape(sd.mmul(srcFeat, attSrc), nnz);
    SDVariable eDst = sd.reshape(sd.mmul(dstFeat, attDst), nnz);
    SDVariable eLogit = sd.nn().leakyRelu(eSrc.add(eDst), leakyAlpha);
    SDVariable alpha = sd.sparse().csrRowSoftmax(eLogit, rowPtr, (int) N);
    SDVariable weighted = srcFeat.mul(sd.reshape(alpha, nnz, 1));
    SDVariable out = sd.sparse().csrEdgeAggregate(rowPtr, weighted, (int) N, 0);
    return out;
  }

  /**
   * Single-head Graph Attention Network convolution (Veličković et al. 2018).<br>
   *
   * @param name name May be null. Name for the output variable
   * @param X Node features [N, F] (FLOATING_POINT type)
   * @param W Linear weight [F, H] (FLOATING_POINT type)
   * @param attSrc Attention source vector [H, 1] (FLOATING_POINT type)
   * @param attDst Attention destination vector [H, 1] (FLOATING_POINT type)
   * @param colIdx CSR column indices (source nodes per edge) [nnz] INT32 (INT type)
   * @param rowPtr CSR row pointers [N+1] INT32 (INT type)
   * @param rowIdx Per-edge destination-node index [nnz] INT32 (INT type)
   * @param nnz Number of edges
   * @param N Number of nodes
   * @param leakyAlpha Negative slope for LeakyReLU
   * @return out Aggregated node embeddings [N, H] (FLOATING_POINT type)
   */
  public SDVariable gatConvHead(String name, SDVariable X, SDVariable W, SDVariable attSrc,
      SDVariable attDst, SDVariable colIdx, SDVariable rowPtr, SDVariable rowIdx, long nnz, long N,
      double leakyAlpha) {
    SDValidation.validateFloatingPoint("gatConvHead", "X", X);
    SDValidation.validateFloatingPoint("gatConvHead", "W", W);
    SDValidation.validateFloatingPoint("gatConvHead", "attSrc", attSrc);
    SDValidation.validateFloatingPoint("gatConvHead", "attDst", attDst);
    SDValidation.validateInteger("gatConvHead", "colIdx", colIdx);
    SDValidation.validateInteger("gatConvHead", "rowPtr", rowPtr);
    SDValidation.validateInteger("gatConvHead", "rowIdx", rowIdx);
    SDVariable Wh = sd.mmul(X, W);
    SDVariable srcFeat = sd.sparse().csrEdgeGather(colIdx, Wh, (int) N);
    SDVariable dstFeat = sd.gather(Wh, rowIdx, 0);
    SDVariable eSrc = sd.reshape(sd.mmul(srcFeat, attSrc), nnz);
    SDVariable eDst = sd.reshape(sd.mmul(dstFeat, attDst), nnz);
    SDVariable eLogit = sd.nn().leakyRelu(eSrc.add(eDst), leakyAlpha);
    SDVariable alpha = sd.sparse().csrRowSoftmax(eLogit, rowPtr, (int) N);
    SDVariable weighted = srcFeat.mul(sd.reshape(alpha, nnz, 1));
    SDVariable out = sd.sparse().csrEdgeAggregate(rowPtr, weighted, (int) N, 0);
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * Single-head GATv2 convolution (Brody et al. 2021). GATv2 applies the nonlinearity before<br>
   * the attention projection (dynamic attention), fixing the static attention limitation of GAT v1.<br>
   *
   * @param X Node features [N, F] (FLOATING_POINT type)
   * @param W Linear weight [F, H] (FLOATING_POINT type)
   * @param att Shared attention vector [H, 1] (FLOATING_POINT type)
   * @param colIdx CSR column indices (source nodes per edge) [nnz] INT32 (INT type)
   * @param rowPtr CSR row pointers [N+1] INT32 (INT type)
   * @param rowIdx Per-edge destination-node index [nnz] INT32 (INT type)
   * @param nnz Number of edges
   * @param N Number of nodes
   * @param leakyAlpha Negative slope for LeakyReLU
   * @return out Aggregated node embeddings [N, H] (FLOATING_POINT type)
   */
  public SDVariable gatV2ConvHead(SDVariable X, SDVariable W, SDVariable att, SDVariable colIdx,
      SDVariable rowPtr, SDVariable rowIdx, long nnz, long N, double leakyAlpha) {
    SDValidation.validateFloatingPoint("gatV2ConvHead", "X", X);
    SDValidation.validateFloatingPoint("gatV2ConvHead", "W", W);
    SDValidation.validateFloatingPoint("gatV2ConvHead", "att", att);
    SDValidation.validateInteger("gatV2ConvHead", "colIdx", colIdx);
    SDValidation.validateInteger("gatV2ConvHead", "rowPtr", rowPtr);
    SDValidation.validateInteger("gatV2ConvHead", "rowIdx", rowIdx);
    SDVariable Wh = sd.mmul(X, W);
    SDVariable srcFeat = sd.sparse().csrEdgeGather(colIdx, Wh, (int) N);
    SDVariable dstFeat = sd.gather(Wh, rowIdx, 0);
    SDVariable g = sd.nn().leakyRelu(srcFeat.add(dstFeat), leakyAlpha);
    SDVariable eLogit = sd.reshape(sd.mmul(g, att), nnz);
    SDVariable alpha = sd.sparse().csrRowSoftmax(eLogit, rowPtr, (int) N);
    SDVariable weighted = srcFeat.mul(sd.reshape(alpha, nnz, 1));
    SDVariable out = sd.sparse().csrEdgeAggregate(rowPtr, weighted, (int) N, 0);
    return out;
  }

  /**
   * Single-head GATv2 convolution (Brody et al. 2021). GATv2 applies the nonlinearity before<br>
   * the attention projection (dynamic attention), fixing the static attention limitation of GAT v1.<br>
   *
   * @param name name May be null. Name for the output variable
   * @param X Node features [N, F] (FLOATING_POINT type)
   * @param W Linear weight [F, H] (FLOATING_POINT type)
   * @param att Shared attention vector [H, 1] (FLOATING_POINT type)
   * @param colIdx CSR column indices (source nodes per edge) [nnz] INT32 (INT type)
   * @param rowPtr CSR row pointers [N+1] INT32 (INT type)
   * @param rowIdx Per-edge destination-node index [nnz] INT32 (INT type)
   * @param nnz Number of edges
   * @param N Number of nodes
   * @param leakyAlpha Negative slope for LeakyReLU
   * @return out Aggregated node embeddings [N, H] (FLOATING_POINT type)
   */
  public SDVariable gatV2ConvHead(String name, SDVariable X, SDVariable W, SDVariable att,
      SDVariable colIdx, SDVariable rowPtr, SDVariable rowIdx, long nnz, long N,
      double leakyAlpha) {
    SDValidation.validateFloatingPoint("gatV2ConvHead", "X", X);
    SDValidation.validateFloatingPoint("gatV2ConvHead", "W", W);
    SDValidation.validateFloatingPoint("gatV2ConvHead", "att", att);
    SDValidation.validateInteger("gatV2ConvHead", "colIdx", colIdx);
    SDValidation.validateInteger("gatV2ConvHead", "rowPtr", rowPtr);
    SDValidation.validateInteger("gatV2ConvHead", "rowIdx", rowIdx);
    SDVariable Wh = sd.mmul(X, W);
    SDVariable srcFeat = sd.sparse().csrEdgeGather(colIdx, Wh, (int) N);
    SDVariable dstFeat = sd.gather(Wh, rowIdx, 0);
    SDVariable g = sd.nn().leakyRelu(srcFeat.add(dstFeat), leakyAlpha);
    SDVariable eLogit = sd.reshape(sd.mmul(g, att), nnz);
    SDVariable alpha = sd.sparse().csrRowSoftmax(eLogit, rowPtr, (int) N);
    SDVariable weighted = srcFeat.mul(sd.reshape(alpha, nnz, 1));
    SDVariable out = sd.sparse().csrEdgeAggregate(rowPtr, weighted, (int) N, 0);
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * Graph Convolutional Network layer (Kipf & Welling 2017).<br>
   * out = relu?( A_norm · X · W + bias )<br>
   *
   * @param X Node features [rows, F] (FLOATING_POINT type)
   * @param W Weight matrix [F, H] (FLOATING_POINT type)
   * @param bias Optional bias [H]; pass null to omit (FLOATING_POINT type)
   * @param aNormVals CSR non-zero values of the normalised adjacency [nnz] (FLOATING_POINT type)
   * @param aNormColIdx CSR column indices [nnz] (INT type)
   * @param aNormRowPtr CSR row pointers [rows+1] (INT type)
   * @param rows Number of nodes / rows in A_norm
   * @param cols Number of columns in A_norm
   * @param applyRelu Whether to apply ReLU to the output
   * @return out Output node embeddings [rows, H] (FLOATING_POINT type)
   */
  public SDVariable gcnConv(SDVariable X, SDVariable W, SDVariable bias, SDVariable aNormVals,
      SDVariable aNormColIdx, SDVariable aNormRowPtr, long rows, long cols, boolean applyRelu) {
    SDValidation.validateFloatingPoint("gcnConv", "X", X);
    SDValidation.validateFloatingPoint("gcnConv", "W", W);
    SDValidation.validateFloatingPoint("gcnConv", "bias", bias);
    SDValidation.validateFloatingPoint("gcnConv", "aNormVals", aNormVals);
    SDValidation.validateInteger("gcnConv", "aNormColIdx", aNormColIdx);
    SDValidation.validateInteger("gcnConv", "aNormRowPtr", aNormRowPtr);
    SDVariable AX = sd.sparse().csrSpmm(aNormVals, aNormColIdx, aNormRowPtr, X, (int) rows, (int) cols, false);
    SDVariable AXW = sd.mmul(AX, W);
    if (bias != null) {
        AXW = AXW.add(bias);
    }
    SDVariable out = applyRelu ? sd.nn().relu(AXW, 0.0) : AXW;
    return out;
  }

  /**
   * Graph Convolutional Network layer (Kipf & Welling 2017).<br>
   * out = relu?( A_norm · X · W + bias )<br>
   *
   * @param name name May be null. Name for the output variable
   * @param X Node features [rows, F] (FLOATING_POINT type)
   * @param W Weight matrix [F, H] (FLOATING_POINT type)
   * @param bias Optional bias [H]; pass null to omit (FLOATING_POINT type)
   * @param aNormVals CSR non-zero values of the normalised adjacency [nnz] (FLOATING_POINT type)
   * @param aNormColIdx CSR column indices [nnz] (INT type)
   * @param aNormRowPtr CSR row pointers [rows+1] (INT type)
   * @param rows Number of nodes / rows in A_norm
   * @param cols Number of columns in A_norm
   * @param applyRelu Whether to apply ReLU to the output
   * @return out Output node embeddings [rows, H] (FLOATING_POINT type)
   */
  public SDVariable gcnConv(String name, SDVariable X, SDVariable W, SDVariable bias,
      SDVariable aNormVals, SDVariable aNormColIdx, SDVariable aNormRowPtr, long rows, long cols,
      boolean applyRelu) {
    SDValidation.validateFloatingPoint("gcnConv", "X", X);
    SDValidation.validateFloatingPoint("gcnConv", "W", W);
    SDValidation.validateFloatingPoint("gcnConv", "bias", bias);
    SDValidation.validateFloatingPoint("gcnConv", "aNormVals", aNormVals);
    SDValidation.validateInteger("gcnConv", "aNormColIdx", aNormColIdx);
    SDValidation.validateInteger("gcnConv", "aNormRowPtr", aNormRowPtr);
    SDVariable AX = sd.sparse().csrSpmm(aNormVals, aNormColIdx, aNormRowPtr, X, (int) rows, (int) cols, false);
    SDVariable AXW = sd.mmul(AX, W);
    if (bias != null) {
        AXW = AXW.add(bias);
    }
    SDVariable out = applyRelu ? sd.nn().relu(AXW, 0.0) : AXW;
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * GCNII convolution (Chen et al. 2020): deep GCN layer combining initial residual connection<br>
   * with identity mapping. M = (1-alpha)*(A_norm*H) + alpha*H0; out = sigma((1-beta)*M + beta*(M*W))<br>
   *
   * @param H Current layer representation [rows, F] (FLOATING_POINT type)
   * @param H0 Initial (input-projected) representation [rows, F] (FLOATING_POINT type)
   * @param W Weight matrix [F, F] (FLOATING_POINT type)
   * @param aNormVals CSR values of the normalised adjacency [nnz] (FLOATING_POINT type)
   * @param aNormColIdx CSR column indices [nnz] INT32 (INT type)
   * @param aNormRowPtr CSR row pointers [rows+1] INT32 (INT type)
   * @param rows Number of nodes
   * @param cols Number of columns of A_norm
   * @param alpha Initial-residual strength in [0,1]
   * @param beta Identity-mapping strength in [0,1]
   * @param applyRelu Whether to apply ReLU to the output
   * @return out Updated node representation [rows, F] (FLOATING_POINT type)
   */
  public SDVariable gcniiConv(SDVariable H, SDVariable H0, SDVariable W, SDVariable aNormVals,
      SDVariable aNormColIdx, SDVariable aNormRowPtr, long rows, long cols, double alpha,
      double beta, boolean applyRelu) {
    SDValidation.validateFloatingPoint("gcniiConv", "H", H);
    SDValidation.validateFloatingPoint("gcniiConv", "H0", H0);
    SDValidation.validateFloatingPoint("gcniiConv", "W", W);
    SDValidation.validateFloatingPoint("gcniiConv", "aNormVals", aNormVals);
    SDValidation.validateInteger("gcniiConv", "aNormColIdx", aNormColIdx);
    SDValidation.validateInteger("gcniiConv", "aNormRowPtr", aNormRowPtr);
    SDVariable AH = sd.sparse().csrSpmm(aNormVals, aNormColIdx, aNormRowPtr, H, (int) rows, (int) cols, false);
    SDVariable M = AH.mul(1.0 - alpha).add(H0.mul(alpha));
    SDVariable MW = sd.mmul(M, W);
    SDVariable out = M.mul(1.0 - beta).add(MW.mul(beta));
    if (applyRelu) {
        out = sd.nn().relu(out, 0.0);
    }
    return out;
  }

  /**
   * GCNII convolution (Chen et al. 2020): deep GCN layer combining initial residual connection<br>
   * with identity mapping. M = (1-alpha)*(A_norm*H) + alpha*H0; out = sigma((1-beta)*M + beta*(M*W))<br>
   *
   * @param name name May be null. Name for the output variable
   * @param H Current layer representation [rows, F] (FLOATING_POINT type)
   * @param H0 Initial (input-projected) representation [rows, F] (FLOATING_POINT type)
   * @param W Weight matrix [F, F] (FLOATING_POINT type)
   * @param aNormVals CSR values of the normalised adjacency [nnz] (FLOATING_POINT type)
   * @param aNormColIdx CSR column indices [nnz] INT32 (INT type)
   * @param aNormRowPtr CSR row pointers [rows+1] INT32 (INT type)
   * @param rows Number of nodes
   * @param cols Number of columns of A_norm
   * @param alpha Initial-residual strength in [0,1]
   * @param beta Identity-mapping strength in [0,1]
   * @param applyRelu Whether to apply ReLU to the output
   * @return out Updated node representation [rows, F] (FLOATING_POINT type)
   */
  public SDVariable gcniiConv(String name, SDVariable H, SDVariable H0, SDVariable W,
      SDVariable aNormVals, SDVariable aNormColIdx, SDVariable aNormRowPtr, long rows, long cols,
      double alpha, double beta, boolean applyRelu) {
    SDValidation.validateFloatingPoint("gcniiConv", "H", H);
    SDValidation.validateFloatingPoint("gcniiConv", "H0", H0);
    SDValidation.validateFloatingPoint("gcniiConv", "W", W);
    SDValidation.validateFloatingPoint("gcniiConv", "aNormVals", aNormVals);
    SDValidation.validateInteger("gcniiConv", "aNormColIdx", aNormColIdx);
    SDValidation.validateInteger("gcniiConv", "aNormRowPtr", aNormRowPtr);
    SDVariable AH = sd.sparse().csrSpmm(aNormVals, aNormColIdx, aNormRowPtr, H, (int) rows, (int) cols, false);
    SDVariable M = AH.mul(1.0 - alpha).add(H0.mul(alpha));
    SDVariable MW = sd.mmul(M, W);
    SDVariable out = M.mul(1.0 - beta).add(MW.mul(beta));
    if (applyRelu) {
        out = sd.nn().relu(out, 0.0);
    }
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * Gated Graph Neural Network (Li et al. 2016): steps rounds of neighbour aggregation + GRU update.<br>
   * Uses concat([in1,in2])*concat([W1,W2]) gate form for correct CUDA backward.<br>
   *
   * @param X Initial node states [rows, H] (FLOATING_POINT type)
   * @param aggW Message transform [H, H] (FLOATING_POINT type)
   * @param wz Update-gate weight [H, H] (FLOATING_POINT type)
   * @param uz Update-gate recurrent weight [H, H] (FLOATING_POINT type)
   * @param wr Reset-gate weight [H, H] (FLOATING_POINT type)
   * @param ur Reset-gate recurrent weight [H, H] (FLOATING_POINT type)
   * @param wh Candidate weight [H, H] (FLOATING_POINT type)
   * @param uh Candidate recurrent weight [H, H] (FLOATING_POINT type)
   * @param aNormVals CSR values of the normalised adjacency [nnz] (FLOATING_POINT type)
   * @param aNormColIdx CSR column indices [nnz] INT32 (INT type)
   * @param aNormRowPtr CSR row pointers [rows+1] INT32 (INT type)
   * @param rows Number of nodes
   * @param cols Columns of A_norm
   * @param steps Number of propagation / GRU steps
   * @return out Final node states [rows, H] (FLOATING_POINT type)
   */
  public SDVariable ggnn(SDVariable X, SDVariable aggW, SDVariable wz, SDVariable uz, SDVariable wr,
      SDVariable ur, SDVariable wh, SDVariable uh, SDVariable aNormVals, SDVariable aNormColIdx,
      SDVariable aNormRowPtr, long rows, long cols, int steps) {
    SDValidation.validateFloatingPoint("ggnn", "X", X);
    SDValidation.validateFloatingPoint("ggnn", "aggW", aggW);
    SDValidation.validateFloatingPoint("ggnn", "wz", wz);
    SDValidation.validateFloatingPoint("ggnn", "uz", uz);
    SDValidation.validateFloatingPoint("ggnn", "wr", wr);
    SDValidation.validateFloatingPoint("ggnn", "ur", ur);
    SDValidation.validateFloatingPoint("ggnn", "wh", wh);
    SDValidation.validateFloatingPoint("ggnn", "uh", uh);
    SDValidation.validateFloatingPoint("ggnn", "aNormVals", aNormVals);
    SDValidation.validateInteger("ggnn", "aNormColIdx", aNormColIdx);
    SDValidation.validateInteger("ggnn", "aNormRowPtr", aNormRowPtr);
    SDVariable h = X;
    for (int t = 0; t < steps; t++) {
        SDVariable agg = sd.sparse().csrSpmm(aNormVals, aNormColIdx, aNormRowPtr, h, (int) rows, (int) cols, false);
        SDVariable a = sd.mmul(agg, aggW);
        SDVariable ah = sd.concat(1, a, h);
        SDVariable z = sd.nn().sigmoid(sd.mmul(ah, sd.concat(0, wz, uz)));
        SDVariable r = sd.nn().sigmoid(sd.mmul(ah, sd.concat(0, wr, ur)));
        SDVariable arh = sd.concat(1, a, r.mul(h));
        SDVariable hh = sd.math().tanh(sd.mmul(arh, sd.concat(0, wh, uh)));
        h = h.mul(z.mul(-1.0).add(1.0)).add(z.mul(hh));
    }
    SDVariable out = h;
    return out;
  }

  /**
   * Gated Graph Neural Network (Li et al. 2016): steps rounds of neighbour aggregation + GRU update.<br>
   * Uses concat([in1,in2])*concat([W1,W2]) gate form for correct CUDA backward.<br>
   *
   * @param name name May be null. Name for the output variable
   * @param X Initial node states [rows, H] (FLOATING_POINT type)
   * @param aggW Message transform [H, H] (FLOATING_POINT type)
   * @param wz Update-gate weight [H, H] (FLOATING_POINT type)
   * @param uz Update-gate recurrent weight [H, H] (FLOATING_POINT type)
   * @param wr Reset-gate weight [H, H] (FLOATING_POINT type)
   * @param ur Reset-gate recurrent weight [H, H] (FLOATING_POINT type)
   * @param wh Candidate weight [H, H] (FLOATING_POINT type)
   * @param uh Candidate recurrent weight [H, H] (FLOATING_POINT type)
   * @param aNormVals CSR values of the normalised adjacency [nnz] (FLOATING_POINT type)
   * @param aNormColIdx CSR column indices [nnz] INT32 (INT type)
   * @param aNormRowPtr CSR row pointers [rows+1] INT32 (INT type)
   * @param rows Number of nodes
   * @param cols Columns of A_norm
   * @param steps Number of propagation / GRU steps
   * @return out Final node states [rows, H] (FLOATING_POINT type)
   */
  public SDVariable ggnn(String name, SDVariable X, SDVariable aggW, SDVariable wz, SDVariable uz,
      SDVariable wr, SDVariable ur, SDVariable wh, SDVariable uh, SDVariable aNormVals,
      SDVariable aNormColIdx, SDVariable aNormRowPtr, long rows, long cols, int steps) {
    SDValidation.validateFloatingPoint("ggnn", "X", X);
    SDValidation.validateFloatingPoint("ggnn", "aggW", aggW);
    SDValidation.validateFloatingPoint("ggnn", "wz", wz);
    SDValidation.validateFloatingPoint("ggnn", "uz", uz);
    SDValidation.validateFloatingPoint("ggnn", "wr", wr);
    SDValidation.validateFloatingPoint("ggnn", "ur", ur);
    SDValidation.validateFloatingPoint("ggnn", "wh", wh);
    SDValidation.validateFloatingPoint("ggnn", "uh", uh);
    SDValidation.validateFloatingPoint("ggnn", "aNormVals", aNormVals);
    SDValidation.validateInteger("ggnn", "aNormColIdx", aNormColIdx);
    SDValidation.validateInteger("ggnn", "aNormRowPtr", aNormRowPtr);
    SDVariable h = X;
    for (int t = 0; t < steps; t++) {
        SDVariable agg = sd.sparse().csrSpmm(aNormVals, aNormColIdx, aNormRowPtr, h, (int) rows, (int) cols, false);
        SDVariable a = sd.mmul(agg, aggW);
        SDVariable ah = sd.concat(1, a, h);
        SDVariable z = sd.nn().sigmoid(sd.mmul(ah, sd.concat(0, wz, uz)));
        SDVariable r = sd.nn().sigmoid(sd.mmul(ah, sd.concat(0, wr, ur)));
        SDVariable arh = sd.concat(1, a, r.mul(h));
        SDVariable hh = sd.math().tanh(sd.mmul(arh, sd.concat(0, wh, uh)));
        h = h.mul(z.mul(-1.0).add(1.0)).add(z.mul(hh));
    }
    SDVariable out = h;
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * Graph Isomorphism Network convolution (Xu et al. 2019) with optional Layer Normalisation.<br>
   *
   * @param X Node features [rows, F] (FLOATING_POINT type)
   * @param w1 First MLP weight [F, H] (FLOATING_POINT type)
   * @param b1 First MLP bias [H] (FLOATING_POINT type)
   * @param w2 Second MLP weight [H, out] (FLOATING_POINT type)
   * @param b2 Second MLP bias [out] (FLOATING_POINT type)
   * @param eps Learnable epsilon scalar SDVariable (shape []) (FLOATING_POINT type)
   * @param colIdx CSR column indices [nnz] INT32 (INT type)
   * @param rowPtr CSR row pointers [rows+1] INT32 (INT type)
   * @param rows Number of nodes
   * @param n Number of nodes (for CsrEdgeGather doDiff)
   * @param layerNorm Whether to apply parameter-free layer normalisation
   * @return out Node embeddings [rows, out], optionally layer-normalised (FLOATING_POINT type)
   */
  public SDVariable ginConv(SDVariable X, SDVariable w1, SDVariable b1, SDVariable w2,
      SDVariable b2, SDVariable eps, SDVariable colIdx, SDVariable rowPtr, long rows, long n,
      boolean layerNorm) {
    SDValidation.validateFloatingPoint("ginConv", "X", X);
    SDValidation.validateFloatingPoint("ginConv", "w1", w1);
    SDValidation.validateFloatingPoint("ginConv", "b1", b1);
    SDValidation.validateFloatingPoint("ginConv", "w2", w2);
    SDValidation.validateFloatingPoint("ginConv", "b2", b2);
    SDValidation.validateFloatingPoint("ginConv", "eps", eps);
    SDValidation.validateInteger("ginConv", "colIdx", colIdx);
    SDValidation.validateInteger("ginConv", "rowPtr", rowPtr);
    SDVariable Xj = sd.sparse().csrEdgeGather(colIdx, X, (int) n);
    SDVariable agg = sd.sparse().csrEdgeAggregate(rowPtr, Xj, (int) rows, 0);
    SDVariable hi = X.mul(eps.add(1.0)).add(agg);
    SDVariable h1 = sd.nn().relu(sd.mmul(hi, w1).add(b1), 0.0);
    SDVariable out = sd.mmul(h1, w2).add(b2);
    if (layerNorm) {
        SDVariable lnMean = sd.mean(out, true, 1);
        SDVariable d = out.sub(lnMean);
        SDVariable var = sd.mean(d.mul(d), true, 1);
        out = d.div(sd.math().sqrt(var.add(1e-5)));
    }
    return out;
  }

  /**
   * Graph Isomorphism Network convolution (Xu et al. 2019) with optional Layer Normalisation.<br>
   *
   * @param name name May be null. Name for the output variable
   * @param X Node features [rows, F] (FLOATING_POINT type)
   * @param w1 First MLP weight [F, H] (FLOATING_POINT type)
   * @param b1 First MLP bias [H] (FLOATING_POINT type)
   * @param w2 Second MLP weight [H, out] (FLOATING_POINT type)
   * @param b2 Second MLP bias [out] (FLOATING_POINT type)
   * @param eps Learnable epsilon scalar SDVariable (shape []) (FLOATING_POINT type)
   * @param colIdx CSR column indices [nnz] INT32 (INT type)
   * @param rowPtr CSR row pointers [rows+1] INT32 (INT type)
   * @param rows Number of nodes
   * @param n Number of nodes (for CsrEdgeGather doDiff)
   * @param layerNorm Whether to apply parameter-free layer normalisation
   * @return out Node embeddings [rows, out], optionally layer-normalised (FLOATING_POINT type)
   */
  public SDVariable ginConv(String name, SDVariable X, SDVariable w1, SDVariable b1, SDVariable w2,
      SDVariable b2, SDVariable eps, SDVariable colIdx, SDVariable rowPtr, long rows, long n,
      boolean layerNorm) {
    SDValidation.validateFloatingPoint("ginConv", "X", X);
    SDValidation.validateFloatingPoint("ginConv", "w1", w1);
    SDValidation.validateFloatingPoint("ginConv", "b1", b1);
    SDValidation.validateFloatingPoint("ginConv", "w2", w2);
    SDValidation.validateFloatingPoint("ginConv", "b2", b2);
    SDValidation.validateFloatingPoint("ginConv", "eps", eps);
    SDValidation.validateInteger("ginConv", "colIdx", colIdx);
    SDValidation.validateInteger("ginConv", "rowPtr", rowPtr);
    SDVariable Xj = sd.sparse().csrEdgeGather(colIdx, X, (int) n);
    SDVariable agg = sd.sparse().csrEdgeAggregate(rowPtr, Xj, (int) rows, 0);
    SDVariable hi = X.mul(eps.add(1.0)).add(agg);
    SDVariable h1 = sd.nn().relu(sd.mmul(hi, w1).add(b1), 0.0);
    SDVariable out = sd.mmul(h1, w2).add(b2);
    if (layerNorm) {
        SDVariable lnMean = sd.mean(out, true, 1);
        SDVariable d = out.sub(lnMean);
        SDVariable var = sd.mean(d.mul(d), true, 1);
        out = d.div(sd.math().sqrt(var.add(1e-5)));
    }
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * Graph Isomorphism Network convolution (Xu et al. 2019) with optional Layer Normalisation.<br>
   *
   * @param X Node features [rows, F] (FLOATING_POINT type)
   * @param w1 First MLP weight [F, H] (FLOATING_POINT type)
   * @param b1 First MLP bias [H] (FLOATING_POINT type)
   * @param w2 Second MLP weight [H, out] (FLOATING_POINT type)
   * @param b2 Second MLP bias [out] (FLOATING_POINT type)
   * @param eps Learnable epsilon scalar SDVariable (shape []) (FLOATING_POINT type)
   * @param colIdx CSR column indices [nnz] INT32 (INT type)
   * @param rowPtr CSR row pointers [rows+1] INT32 (INT type)
   * @param rows Number of nodes
   * @param n Number of nodes (for CsrEdgeGather doDiff)
   * @return out Node embeddings [rows, out], optionally layer-normalised (FLOATING_POINT type)
   */
  public SDVariable ginConv(SDVariable X, SDVariable w1, SDVariable b1, SDVariable w2,
      SDVariable b2, SDVariable eps, SDVariable colIdx, SDVariable rowPtr, long rows, long n) {
    SDValidation.validateFloatingPoint("ginConv", "X", X);
    SDValidation.validateFloatingPoint("ginConv", "w1", w1);
    SDValidation.validateFloatingPoint("ginConv", "b1", b1);
    SDValidation.validateFloatingPoint("ginConv", "w2", w2);
    SDValidation.validateFloatingPoint("ginConv", "b2", b2);
    SDValidation.validateFloatingPoint("ginConv", "eps", eps);
    SDValidation.validateInteger("ginConv", "colIdx", colIdx);
    SDValidation.validateInteger("ginConv", "rowPtr", rowPtr);
    boolean layerNorm = false;
    SDVariable Xj = sd.sparse().csrEdgeGather(colIdx, X, (int) n);
    SDVariable agg = sd.sparse().csrEdgeAggregate(rowPtr, Xj, (int) rows, 0);
    SDVariable hi = X.mul(eps.add(1.0)).add(agg);
    SDVariable h1 = sd.nn().relu(sd.mmul(hi, w1).add(b1), 0.0);
    SDVariable out = sd.mmul(h1, w2).add(b2);
    if (layerNorm) {
        SDVariable lnMean = sd.mean(out, true, 1);
        SDVariable d = out.sub(lnMean);
        SDVariable var = sd.mean(d.mul(d), true, 1);
        out = d.div(sd.math().sqrt(var.add(1e-5)));
    }
    return out;
  }

  /**
   * Graph Isomorphism Network convolution (Xu et al. 2019) with optional Layer Normalisation.<br>
   *
   * @param name name May be null. Name for the output variable
   * @param X Node features [rows, F] (FLOATING_POINT type)
   * @param w1 First MLP weight [F, H] (FLOATING_POINT type)
   * @param b1 First MLP bias [H] (FLOATING_POINT type)
   * @param w2 Second MLP weight [H, out] (FLOATING_POINT type)
   * @param b2 Second MLP bias [out] (FLOATING_POINT type)
   * @param eps Learnable epsilon scalar SDVariable (shape []) (FLOATING_POINT type)
   * @param colIdx CSR column indices [nnz] INT32 (INT type)
   * @param rowPtr CSR row pointers [rows+1] INT32 (INT type)
   * @param rows Number of nodes
   * @param n Number of nodes (for CsrEdgeGather doDiff)
   * @return out Node embeddings [rows, out], optionally layer-normalised (FLOATING_POINT type)
   */
  public SDVariable ginConv(String name, SDVariable X, SDVariable w1, SDVariable b1, SDVariable w2,
      SDVariable b2, SDVariable eps, SDVariable colIdx, SDVariable rowPtr, long rows, long n) {
    SDValidation.validateFloatingPoint("ginConv", "X", X);
    SDValidation.validateFloatingPoint("ginConv", "w1", w1);
    SDValidation.validateFloatingPoint("ginConv", "b1", b1);
    SDValidation.validateFloatingPoint("ginConv", "w2", w2);
    SDValidation.validateFloatingPoint("ginConv", "b2", b2);
    SDValidation.validateFloatingPoint("ginConv", "eps", eps);
    SDValidation.validateInteger("ginConv", "colIdx", colIdx);
    SDValidation.validateInteger("ginConv", "rowPtr", rowPtr);
    boolean layerNorm = false;
    SDVariable Xj = sd.sparse().csrEdgeGather(colIdx, X, (int) n);
    SDVariable agg = sd.sparse().csrEdgeAggregate(rowPtr, Xj, (int) rows, 0);
    SDVariable hi = X.mul(eps.add(1.0)).add(agg);
    SDVariable h1 = sd.nn().relu(sd.mmul(hi, w1).add(b1), 0.0);
    SDVariable out = sd.mmul(h1, w2).add(b2);
    if (layerNorm) {
        SDVariable lnMean = sd.mean(out, true, 1);
        SDVariable d = out.sub(lnMean);
        SDVariable var = sd.mean(d.mul(d), true, 1);
        out = d.div(sd.math().sqrt(var.add(1e-5)));
    }
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * GraphNorm (Cai et al. 2021): learnable graph-level normalisation. Implemented in transposed<br>
   * [F, rows] layout for correct CUDA gradient flow on per-feature alpha scaling.<br>
   *
   * @param X Node features [rows, F] (FLOATING_POINT type)
   * @param gamma Learnable scale [F] (FLOATING_POINT type)
   * @param beta Learnable shift [F] (FLOATING_POINT type)
   * @param alpha Learnable mean-retention coefficient [F] (FLOATING_POINT type)
   * @return out Normalised node features [rows, F] (FLOATING_POINT type)
   */
  public SDVariable graphNorm(SDVariable X, SDVariable gamma, SDVariable beta, SDVariable alpha) {
    SDValidation.validateFloatingPoint("graphNorm", "X", X);
    SDValidation.validateFloatingPoint("graphNorm", "gamma", gamma);
    SDValidation.validateFloatingPoint("graphNorm", "beta", beta);
    SDValidation.validateFloatingPoint("graphNorm", "alpha", alpha);
    SDVariable Xt = sd.transpose(X);
    SDVariable meanT = sd.mean(Xt, true, 1);
    SDVariable alphaCol = sd.reshape(alpha, -1, 1);
    SDVariable shiftedT = Xt.sub(meanT.mul(alphaCol));
    SDVariable varT = sd.mean(shiftedT.mul(shiftedT), true, 1);
    SDVariable normT = shiftedT.div(sd.math().sqrt(varT.add(1e-5)));
    SDVariable gammaCol = sd.reshape(gamma, -1, 1);
    SDVariable betaCol = sd.reshape(beta, -1, 1);
    SDVariable outT = normT.mul(gammaCol).add(betaCol);
    SDVariable out = sd.transpose(outT);
    return out;
  }

  /**
   * GraphNorm (Cai et al. 2021): learnable graph-level normalisation. Implemented in transposed<br>
   * [F, rows] layout for correct CUDA gradient flow on per-feature alpha scaling.<br>
   *
   * @param name name May be null. Name for the output variable
   * @param X Node features [rows, F] (FLOATING_POINT type)
   * @param gamma Learnable scale [F] (FLOATING_POINT type)
   * @param beta Learnable shift [F] (FLOATING_POINT type)
   * @param alpha Learnable mean-retention coefficient [F] (FLOATING_POINT type)
   * @return out Normalised node features [rows, F] (FLOATING_POINT type)
   */
  public SDVariable graphNorm(String name, SDVariable X, SDVariable gamma, SDVariable beta,
      SDVariable alpha) {
    SDValidation.validateFloatingPoint("graphNorm", "X", X);
    SDValidation.validateFloatingPoint("graphNorm", "gamma", gamma);
    SDValidation.validateFloatingPoint("graphNorm", "beta", beta);
    SDValidation.validateFloatingPoint("graphNorm", "alpha", alpha);
    SDVariable Xt = sd.transpose(X);
    SDVariable meanT = sd.mean(Xt, true, 1);
    SDVariable alphaCol = sd.reshape(alpha, -1, 1);
    SDVariable shiftedT = Xt.sub(meanT.mul(alphaCol));
    SDVariable varT = sd.mean(shiftedT.mul(shiftedT), true, 1);
    SDVariable normT = shiftedT.div(sd.math().sqrt(varT.add(1e-5)));
    SDVariable gammaCol = sd.reshape(gamma, -1, 1);
    SDVariable betaCol = sd.reshape(beta, -1, 1);
    SDVariable outT = normT.mul(gammaCol).add(betaCol);
    SDVariable out = sd.transpose(outT);
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * Single-head Graph Transformer layer (Dwivedi & Bresson 2021, simplified):<br>
   * scaled dot-product self-attention, optionally restricted via additive mask.<br>
   *
   * @param X Node features [N, F] (FLOATING_POINT type)
   * @param wq Query weight [F, d] (FLOATING_POINT type)
   * @param wk Key weight [F, d] (FLOATING_POINT type)
   * @param wv Value weight [F, d] (FLOATING_POINT type)
   * @param wo Output weight [d, d_out] (FLOATING_POINT type)
   * @param adjMask Optional additive attention mask [N, N]; pass null for full attention (FLOATING_POINT type)
   * @param scale Score scaling (typically 1/sqrt(d))
   * @return out Node embeddings [N, d_out] (FLOATING_POINT type)
   */
  public SDVariable graphTransformer(SDVariable X, SDVariable wq, SDVariable wk, SDVariable wv,
      SDVariable wo, SDVariable adjMask, double scale) {
    SDValidation.validateFloatingPoint("graphTransformer", "X", X);
    SDValidation.validateFloatingPoint("graphTransformer", "wq", wq);
    SDValidation.validateFloatingPoint("graphTransformer", "wk", wk);
    SDValidation.validateFloatingPoint("graphTransformer", "wv", wv);
    SDValidation.validateFloatingPoint("graphTransformer", "wo", wo);
    SDValidation.validateFloatingPoint("graphTransformer", "adjMask", adjMask);
    SDVariable q = sd.mmul(X, wq);
    SDVariable k = sd.mmul(X, wk);
    SDVariable v = sd.mmul(X, wv);
    SDVariable scores = sd.mmul(q, k, false, true, false).mul(scale);
    if (adjMask != null) {
        scores = scores.add(adjMask);
    }
    SDVariable attn = sd.nn().softmax(scores, 1);
    SDVariable ctx = sd.mmul(attn, v);
    SDVariable out = sd.mmul(ctx, wo);
    return out;
  }

  /**
   * Single-head Graph Transformer layer (Dwivedi & Bresson 2021, simplified):<br>
   * scaled dot-product self-attention, optionally restricted via additive mask.<br>
   *
   * @param name name May be null. Name for the output variable
   * @param X Node features [N, F] (FLOATING_POINT type)
   * @param wq Query weight [F, d] (FLOATING_POINT type)
   * @param wk Key weight [F, d] (FLOATING_POINT type)
   * @param wv Value weight [F, d] (FLOATING_POINT type)
   * @param wo Output weight [d, d_out] (FLOATING_POINT type)
   * @param adjMask Optional additive attention mask [N, N]; pass null for full attention (FLOATING_POINT type)
   * @param scale Score scaling (typically 1/sqrt(d))
   * @return out Node embeddings [N, d_out] (FLOATING_POINT type)
   */
  public SDVariable graphTransformer(String name, SDVariable X, SDVariable wq, SDVariable wk,
      SDVariable wv, SDVariable wo, SDVariable adjMask, double scale) {
    SDValidation.validateFloatingPoint("graphTransformer", "X", X);
    SDValidation.validateFloatingPoint("graphTransformer", "wq", wq);
    SDValidation.validateFloatingPoint("graphTransformer", "wk", wk);
    SDValidation.validateFloatingPoint("graphTransformer", "wv", wv);
    SDValidation.validateFloatingPoint("graphTransformer", "wo", wo);
    SDValidation.validateFloatingPoint("graphTransformer", "adjMask", adjMask);
    SDVariable q = sd.mmul(X, wq);
    SDVariable k = sd.mmul(X, wk);
    SDVariable v = sd.mmul(X, wv);
    SDVariable scores = sd.mmul(q, k, false, true, false).mul(scale);
    if (adjMask != null) {
        scores = scores.add(adjMask);
    }
    SDVariable attn = sd.nn().softmax(scores, 1);
    SDVariable ctx = sd.mmul(attn, v);
    SDVariable out = sd.mmul(ctx, wo);
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * Heterogeneous Attention Network (Wang et al. 2019).<br>
   * For each meta-path: run a single-head node-level GAT (gatConvHead), then compute a semantic<br>
   * attention score via tanh(Z*semW + semB)*semQ. Softmax over meta-path scores gives mixing<br>
   * weights beta_p; final output = sum_p(beta_p * Z_p).<br>
   *
   * @param X Node features [N, F] (FLOATING_POINT type)
   * @param metaW Per-meta-path linear weight [F, H], one per meta-path (FLOATING_POINT type)
   * @param attSrc Per-meta-path attention source vector [H, 1] (FLOATING_POINT type)
   * @param attDst Per-meta-path attention destination vector [H, 1] (FLOATING_POINT type)
   * @param colIdx Per-meta-path CSR column indices [nnz_p] INT32 (INT type)
   * @param rowPtr Per-meta-path CSR row pointers [N+1] INT32 (INT type)
   * @param rowIdx Per-meta-path per-edge destination indices [nnz_p] INT32 (INT type)
   * @param semW Semantic-attention weight [H, A] (FLOATING_POINT type)
   * @param semB Semantic-attention bias [A] (FLOATING_POINT type)
   * @param semQ Semantic-attention query [A, 1] (FLOATING_POINT type)
   * @param nnz Per-meta-path edge counts (Size: AtLeast(min=1))
   * @param N Number of nodes
   * @param leakyAlpha LeakyReLU negative slope for node-level GAT
   * @return out Fused node embeddings [N, H] (FLOATING_POINT type)
   */
  public SDVariable han(SDVariable X, SDVariable[] metaW, SDVariable[] attSrc, SDVariable[] attDst,
      SDVariable[] colIdx, SDVariable[] rowPtr, SDVariable[] rowIdx, SDVariable semW,
      SDVariable semB, SDVariable semQ, long[] nnz, long N, double leakyAlpha) {
    SDValidation.validateFloatingPoint("han", "X", X);
    SDValidation.validateFloatingPoint("han", "metaW", metaW);
    Preconditions.checkArgument(metaW.length >= 1, "metaW has incorrect size/length. Expected: metaW.length >= 1, got %s", metaW.length);
    SDValidation.validateFloatingPoint("han", "attSrc", attSrc);
    Preconditions.checkArgument(attSrc.length >= 1, "attSrc has incorrect size/length. Expected: attSrc.length >= 1, got %s", attSrc.length);
    SDValidation.validateFloatingPoint("han", "attDst", attDst);
    Preconditions.checkArgument(attDst.length >= 1, "attDst has incorrect size/length. Expected: attDst.length >= 1, got %s", attDst.length);
    SDValidation.validateInteger("han", "colIdx", colIdx);
    Preconditions.checkArgument(colIdx.length >= 1, "colIdx has incorrect size/length. Expected: colIdx.length >= 1, got %s", colIdx.length);
    SDValidation.validateInteger("han", "rowPtr", rowPtr);
    Preconditions.checkArgument(rowPtr.length >= 1, "rowPtr has incorrect size/length. Expected: rowPtr.length >= 1, got %s", rowPtr.length);
    SDValidation.validateInteger("han", "rowIdx", rowIdx);
    Preconditions.checkArgument(rowIdx.length >= 1, "rowIdx has incorrect size/length. Expected: rowIdx.length >= 1, got %s", rowIdx.length);
    SDValidation.validateFloatingPoint("han", "semW", semW);
    SDValidation.validateFloatingPoint("han", "semB", semB);
    SDValidation.validateFloatingPoint("han", "semQ", semQ);
    Preconditions.checkArgument(nnz.length >= 1, "nnz has incorrect size/length. Expected: nnz.length >= 1, got %s", nnz.length);
    final int P = metaW.length;
    SDVariable[] z = new SDVariable[P];
    SDVariable[] expw = new SDVariable[P];
    SDVariable sumExp = null;
    for (int p = 0; p < P; p++) {
        z[p] = sd.gnn().gatConvHead(X, metaW[p], attSrc[p], attDst[p],
                colIdx[p], rowPtr[p], rowIdx[p], nnz[p], N, leakyAlpha);
        SDVariable proj  = sd.math().tanh(sd.mmul(z[p], semW).add(semB));
        SDVariable score = sd.mean(sd.mmul(proj, semQ), false);
        expw[p] = sd.math().exp(score);
        sumExp = (p == 0) ? expw[p] : sumExp.add(expw[p]);
    }
    SDVariable out = null;
    for (int p = 0; p < P; p++) {
        SDVariable betaP = expw[p].div(sumExp);
        SDVariable term  = z[p].mul(betaP);
        out = (p == 0) ? term : out.add(term);
    }
    return out;
  }

  /**
   * Heterogeneous Attention Network (Wang et al. 2019).<br>
   * For each meta-path: run a single-head node-level GAT (gatConvHead), then compute a semantic<br>
   * attention score via tanh(Z*semW + semB)*semQ. Softmax over meta-path scores gives mixing<br>
   * weights beta_p; final output = sum_p(beta_p * Z_p).<br>
   *
   * @param name name May be null. Name for the output variable
   * @param X Node features [N, F] (FLOATING_POINT type)
   * @param metaW Per-meta-path linear weight [F, H], one per meta-path (FLOATING_POINT type)
   * @param attSrc Per-meta-path attention source vector [H, 1] (FLOATING_POINT type)
   * @param attDst Per-meta-path attention destination vector [H, 1] (FLOATING_POINT type)
   * @param colIdx Per-meta-path CSR column indices [nnz_p] INT32 (INT type)
   * @param rowPtr Per-meta-path CSR row pointers [N+1] INT32 (INT type)
   * @param rowIdx Per-meta-path per-edge destination indices [nnz_p] INT32 (INT type)
   * @param semW Semantic-attention weight [H, A] (FLOATING_POINT type)
   * @param semB Semantic-attention bias [A] (FLOATING_POINT type)
   * @param semQ Semantic-attention query [A, 1] (FLOATING_POINT type)
   * @param nnz Per-meta-path edge counts (Size: AtLeast(min=1))
   * @param N Number of nodes
   * @param leakyAlpha LeakyReLU negative slope for node-level GAT
   * @return out Fused node embeddings [N, H] (FLOATING_POINT type)
   */
  public SDVariable han(String name, SDVariable X, SDVariable[] metaW, SDVariable[] attSrc,
      SDVariable[] attDst, SDVariable[] colIdx, SDVariable[] rowPtr, SDVariable[] rowIdx,
      SDVariable semW, SDVariable semB, SDVariable semQ, long[] nnz, long N, double leakyAlpha) {
    SDValidation.validateFloatingPoint("han", "X", X);
    SDValidation.validateFloatingPoint("han", "metaW", metaW);
    Preconditions.checkArgument(metaW.length >= 1, "metaW has incorrect size/length. Expected: metaW.length >= 1, got %s", metaW.length);
    SDValidation.validateFloatingPoint("han", "attSrc", attSrc);
    Preconditions.checkArgument(attSrc.length >= 1, "attSrc has incorrect size/length. Expected: attSrc.length >= 1, got %s", attSrc.length);
    SDValidation.validateFloatingPoint("han", "attDst", attDst);
    Preconditions.checkArgument(attDst.length >= 1, "attDst has incorrect size/length. Expected: attDst.length >= 1, got %s", attDst.length);
    SDValidation.validateInteger("han", "colIdx", colIdx);
    Preconditions.checkArgument(colIdx.length >= 1, "colIdx has incorrect size/length. Expected: colIdx.length >= 1, got %s", colIdx.length);
    SDValidation.validateInteger("han", "rowPtr", rowPtr);
    Preconditions.checkArgument(rowPtr.length >= 1, "rowPtr has incorrect size/length. Expected: rowPtr.length >= 1, got %s", rowPtr.length);
    SDValidation.validateInteger("han", "rowIdx", rowIdx);
    Preconditions.checkArgument(rowIdx.length >= 1, "rowIdx has incorrect size/length. Expected: rowIdx.length >= 1, got %s", rowIdx.length);
    SDValidation.validateFloatingPoint("han", "semW", semW);
    SDValidation.validateFloatingPoint("han", "semB", semB);
    SDValidation.validateFloatingPoint("han", "semQ", semQ);
    Preconditions.checkArgument(nnz.length >= 1, "nnz has incorrect size/length. Expected: nnz.length >= 1, got %s", nnz.length);
    final int P = metaW.length;
    SDVariable[] z = new SDVariable[P];
    SDVariable[] expw = new SDVariable[P];
    SDVariable sumExp = null;
    for (int p = 0; p < P; p++) {
        z[p] = sd.gnn().gatConvHead(X, metaW[p], attSrc[p], attDst[p],
                colIdx[p], rowPtr[p], rowIdx[p], nnz[p], N, leakyAlpha);
        SDVariable proj  = sd.math().tanh(sd.mmul(z[p], semW).add(semB));
        SDVariable score = sd.mean(sd.mmul(proj, semQ), false);
        expw[p] = sd.math().exp(score);
        sumExp = (p == 0) ? expw[p] : sumExp.add(expw[p]);
    }
    SDVariable out = null;
    for (int p = 0; p < P; p++) {
        SDVariable betaP = expw[p].div(sumExp);
        SDVariable term  = z[p].mul(betaP);
        out = (p == 0) ? term : out.add(term);
    }
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * Simplified single-head Heterogeneous Graph Transformer (Hu et al. 2020):<br>
   * scaled dot-product attention where relation embedding modulates the key.<br>
   *
   * @param X Node features [N, F] (FLOATING_POINT type)
   * @param wq Query weight [F, d] (FLOATING_POINT type)
   * @param wk Key weight [F, d] (FLOATING_POINT type)
   * @param wv Value weight [F, d] (FLOATING_POINT type)
   * @param relEmb Relation embeddings [numRelations, d] (FLOATING_POINT type)
   * @param edgeRelIdx Per-edge relation index [nnz] INT32 (INT type)
   * @param colIdx CSR column indices (source nodes) [nnz] INT32 (INT type)
   * @param rowPtr CSR row pointers [N+1] INT32 (INT type)
   * @param rowIdx Per-edge destination index [nnz] INT32 (INT type)
   * @param nnz Number of edges
   * @param N Number of nodes
   * @param scale Attention scaling (typically 1/sqrt(d))
   * @return out Aggregated node embeddings [N, d] (FLOATING_POINT type)
   */
  public SDVariable hgtConvHead(SDVariable X, SDVariable wq, SDVariable wk, SDVariable wv,
      SDVariable relEmb, SDVariable edgeRelIdx, SDVariable colIdx, SDVariable rowPtr,
      SDVariable rowIdx, long nnz, long N, double scale) {
    SDValidation.validateFloatingPoint("hgtConvHead", "X", X);
    SDValidation.validateFloatingPoint("hgtConvHead", "wq", wq);
    SDValidation.validateFloatingPoint("hgtConvHead", "wk", wk);
    SDValidation.validateFloatingPoint("hgtConvHead", "wv", wv);
    SDValidation.validateFloatingPoint("hgtConvHead", "relEmb", relEmb);
    SDValidation.validateInteger("hgtConvHead", "edgeRelIdx", edgeRelIdx);
    SDValidation.validateInteger("hgtConvHead", "colIdx", colIdx);
    SDValidation.validateInteger("hgtConvHead", "rowPtr", rowPtr);
    SDValidation.validateInteger("hgtConvHead", "rowIdx", rowIdx);
    SDVariable q = sd.mmul(X, wq);
    SDVariable k = sd.mmul(X, wk);
    SDVariable v = sd.mmul(X, wv);
    SDVariable qDst = sd.gather(q, rowIdx, 0);
    SDVariable kSrc = sd.sparse().csrEdgeGather(colIdx, k, (int) N);
    SDVariable vSrc = sd.sparse().csrEdgeGather(colIdx, v, (int) N);
    SDVariable rel = sd.gather(relEmb, edgeRelIdx, 0);
    SDVariable e = sd.sum(qDst.mul(kSrc.add(rel)), false, 1).mul(scale);
    SDVariable alpha = sd.sparse().csrRowSoftmax(e, rowPtr, (int) N);
    SDVariable weighted = vSrc.mul(sd.reshape(alpha, nnz, 1));
    SDVariable out = sd.sparse().csrEdgeAggregate(rowPtr, weighted, (int) N, 0);
    return out;
  }

  /**
   * Simplified single-head Heterogeneous Graph Transformer (Hu et al. 2020):<br>
   * scaled dot-product attention where relation embedding modulates the key.<br>
   *
   * @param name name May be null. Name for the output variable
   * @param X Node features [N, F] (FLOATING_POINT type)
   * @param wq Query weight [F, d] (FLOATING_POINT type)
   * @param wk Key weight [F, d] (FLOATING_POINT type)
   * @param wv Value weight [F, d] (FLOATING_POINT type)
   * @param relEmb Relation embeddings [numRelations, d] (FLOATING_POINT type)
   * @param edgeRelIdx Per-edge relation index [nnz] INT32 (INT type)
   * @param colIdx CSR column indices (source nodes) [nnz] INT32 (INT type)
   * @param rowPtr CSR row pointers [N+1] INT32 (INT type)
   * @param rowIdx Per-edge destination index [nnz] INT32 (INT type)
   * @param nnz Number of edges
   * @param N Number of nodes
   * @param scale Attention scaling (typically 1/sqrt(d))
   * @return out Aggregated node embeddings [N, d] (FLOATING_POINT type)
   */
  public SDVariable hgtConvHead(String name, SDVariable X, SDVariable wq, SDVariable wk,
      SDVariable wv, SDVariable relEmb, SDVariable edgeRelIdx, SDVariable colIdx, SDVariable rowPtr,
      SDVariable rowIdx, long nnz, long N, double scale) {
    SDValidation.validateFloatingPoint("hgtConvHead", "X", X);
    SDValidation.validateFloatingPoint("hgtConvHead", "wq", wq);
    SDValidation.validateFloatingPoint("hgtConvHead", "wk", wk);
    SDValidation.validateFloatingPoint("hgtConvHead", "wv", wv);
    SDValidation.validateFloatingPoint("hgtConvHead", "relEmb", relEmb);
    SDValidation.validateInteger("hgtConvHead", "edgeRelIdx", edgeRelIdx);
    SDValidation.validateInteger("hgtConvHead", "colIdx", colIdx);
    SDValidation.validateInteger("hgtConvHead", "rowPtr", rowPtr);
    SDValidation.validateInteger("hgtConvHead", "rowIdx", rowIdx);
    SDVariable q = sd.mmul(X, wq);
    SDVariable k = sd.mmul(X, wk);
    SDVariable v = sd.mmul(X, wv);
    SDVariable qDst = sd.gather(q, rowIdx, 0);
    SDVariable kSrc = sd.sparse().csrEdgeGather(colIdx, k, (int) N);
    SDVariable vSrc = sd.sparse().csrEdgeGather(colIdx, v, (int) N);
    SDVariable rel = sd.gather(relEmb, edgeRelIdx, 0);
    SDVariable e = sd.sum(qDst.mul(kSrc.add(rel)), false, 1).mul(scale);
    SDVariable alpha = sd.sparse().csrRowSoftmax(e, rowPtr, (int) N);
    SDVariable weighted = vSrc.mul(sd.reshape(alpha, nnz, 1));
    SDVariable out = sd.sparse().csrEdgeAggregate(rowPtr, weighted, (int) N, 0);
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * Inner-product link decoder (Kipf & Welling 2016): Z*Z^T. Apply sigmoid downstream for edge probabilities.<br>
   *
   * @param z Node latents [N, d] (FLOATING_POINT type)
   * @return out Edge logits [N, N] (FLOATING_POINT type)
   */
  public SDVariable innerProductDecoder(SDVariable z) {
    SDValidation.validateFloatingPoint("innerProductDecoder", "z", z);
    SDVariable out = sd.mmul(z, z, false, true, false);
    return out;
  }

  /**
   * Inner-product link decoder (Kipf & Welling 2016): Z*Z^T. Apply sigmoid downstream for edge probabilities.<br>
   *
   * @param name name May be null. Name for the output variable
   * @param z Node latents [N, d] (FLOATING_POINT type)
   * @return out Edge logits [N, N] (FLOATING_POINT type)
   */
  public SDVariable innerProductDecoder(String name, SDVariable z) {
    SDValidation.validateFloatingPoint("innerProductDecoder", "z", z);
    SDVariable out = sd.mmul(z, z, false, true, false);
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * Jumping-Knowledge concatenation aggregator (Xu et al. 2018).<br>
   * Concatenates per-layer representations along the feature dimension.<br>
   *
   * @param layerOutputs Per-layer node representations, each [rows, H_l] (FLOATING_POINT type)
   * @return out Concatenated representation [rows, sum(H_l)] (FLOATING_POINT type)
   */
  public SDVariable jkNetConcat(SDVariable... layerOutputs) {
    SDValidation.validateFloatingPoint("jkNetConcat", "layerOutputs", layerOutputs);
    Preconditions.checkArgument(layerOutputs.length >= 1, "layerOutputs has incorrect size/length. Expected: layerOutputs.length >= 1, got %s", layerOutputs.length);
    SDVariable out = sd.concat(1, layerOutputs);
    return out;
  }

  /**
   * Jumping-Knowledge concatenation aggregator (Xu et al. 2018).<br>
   * Concatenates per-layer representations along the feature dimension.<br>
   *
   * @param name name May be null. Name for the output variable
   * @param layerOutputs Per-layer node representations, each [rows, H_l] (FLOATING_POINT type)
   * @return out Concatenated representation [rows, sum(H_l)] (FLOATING_POINT type)
   */
  public SDVariable jkNetConcat(String name, SDVariable... layerOutputs) {
    SDValidation.validateFloatingPoint("jkNetConcat", "layerOutputs", layerOutputs);
    Preconditions.checkArgument(layerOutputs.length >= 1, "layerOutputs has incorrect size/length. Expected: layerOutputs.length >= 1, got %s", layerOutputs.length);
    SDVariable out = sd.concat(1, layerOutputs);
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * Jumping-Knowledge max-pooling aggregator (Xu et al. 2018).<br>
   * Element-wise maximum across per-layer node representations.<br>
   *
   * @param layerOutputs Per-layer node representations, each [rows, H] (FLOATING_POINT type)
   * @return out Element-wise max across layers [rows, H] (FLOATING_POINT type)
   */
  public SDVariable jkNetMax(SDVariable... layerOutputs) {
    SDValidation.validateFloatingPoint("jkNetMax", "layerOutputs", layerOutputs);
    Preconditions.checkArgument(layerOutputs.length >= 1, "layerOutputs has incorrect size/length. Expected: layerOutputs.length >= 1, got %s", layerOutputs.length);
    SDVariable out = layerOutputs[0];
    for (int i = 1; i < layerOutputs.length; i++) {
        out = sd.math().max(out, layerOutputs[i]);
    }
    return out;
  }

  /**
   * Jumping-Knowledge max-pooling aggregator (Xu et al. 2018).<br>
   * Element-wise maximum across per-layer node representations.<br>
   *
   * @param name name May be null. Name for the output variable
   * @param layerOutputs Per-layer node representations, each [rows, H] (FLOATING_POINT type)
   * @return out Element-wise max across layers [rows, H] (FLOATING_POINT type)
   */
  public SDVariable jkNetMax(String name, SDVariable... layerOutputs) {
    SDValidation.validateFloatingPoint("jkNetMax", "layerOutputs", layerOutputs);
    Preconditions.checkArgument(layerOutputs.length >= 1, "layerOutputs has incorrect size/length. Expected: layerOutputs.length >= 1, got %s", layerOutputs.length);
    SDVariable out = layerOutputs[0];
    for (int i = 1; i < layerOutputs.length; i++) {
        out = sd.math().max(out, layerOutputs[i]);
    }
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * Edge-conditioned convolution / NNConv (Simonovsky & Komodakis 2017; Gilmer et al. MPNN 2017).<br>
   * An edge network maps edge features to [Fin, Fout] weight matrices applied to neighbour features.<br>
   *
   * @param X Node features [rows, Fin] (FLOATING_POINT type)
   * @param edgeFeatures Per-edge features [nnz, edgeF] (FLOATING_POINT type)
   * @param edgeNetW Edge-network weight [edgeF, Fin*Fout] (FLOATING_POINT type)
   * @param edgeNetB Edge-network bias [Fin*Fout] (FLOATING_POINT type)
   * @param rootW Root/self transform [Fin, Fout] (FLOATING_POINT type)
   * @param bias Optional output bias [Fout]; pass null to omit (FLOATING_POINT type)
   * @param colIdx CSR column indices [nnz] INT32 (INT type)
   * @param rowPtr CSR row pointers [rows+1] INT32 (INT type)
   * @param rows Number of nodes
   * @param n Number of nodes (for CsrEdgeGather doDiff)
   * @param fin Input feature dimension
   * @param fout Output feature dimension
   * @return out Node embeddings [rows, Fout] (FLOATING_POINT type)
   */
  public SDVariable nnConv(SDVariable X, SDVariable edgeFeatures, SDVariable edgeNetW,
      SDVariable edgeNetB, SDVariable rootW, SDVariable bias, SDVariable colIdx, SDVariable rowPtr,
      long rows, long n, long fin, long fout) {
    SDValidation.validateFloatingPoint("nnConv", "X", X);
    SDValidation.validateFloatingPoint("nnConv", "edgeFeatures", edgeFeatures);
    SDValidation.validateFloatingPoint("nnConv", "edgeNetW", edgeNetW);
    SDValidation.validateFloatingPoint("nnConv", "edgeNetB", edgeNetB);
    SDValidation.validateFloatingPoint("nnConv", "rootW", rootW);
    SDValidation.validateFloatingPoint("nnConv", "bias", bias);
    SDValidation.validateInteger("nnConv", "colIdx", colIdx);
    SDValidation.validateInteger("nnConv", "rowPtr", rowPtr);
    SDVariable Xj = sd.sparse().csrEdgeGather(colIdx, X, (int) n);
    SDVariable eW = sd.mmul(edgeFeatures, edgeNetW).add(edgeNetB);
    SDVariable eWr = sd.reshape(eW, -1, fin, fout);
    SDVariable XjE = sd.reshape(Xj, -1, fin, 1);
    SDVariable prod = XjE.mul(eWr);
    SDVariable msg = sd.sum(prod, false, 1);
    SDVariable agg = sd.sparse().csrEdgeAggregate(rowPtr, msg, (int) rows, 0);
    SDVariable out = agg.add(sd.mmul(X, rootW));
    if (bias != null) {
        out = out.add(bias);
    }
    return out;
  }

  /**
   * Edge-conditioned convolution / NNConv (Simonovsky & Komodakis 2017; Gilmer et al. MPNN 2017).<br>
   * An edge network maps edge features to [Fin, Fout] weight matrices applied to neighbour features.<br>
   *
   * @param name name May be null. Name for the output variable
   * @param X Node features [rows, Fin] (FLOATING_POINT type)
   * @param edgeFeatures Per-edge features [nnz, edgeF] (FLOATING_POINT type)
   * @param edgeNetW Edge-network weight [edgeF, Fin*Fout] (FLOATING_POINT type)
   * @param edgeNetB Edge-network bias [Fin*Fout] (FLOATING_POINT type)
   * @param rootW Root/self transform [Fin, Fout] (FLOATING_POINT type)
   * @param bias Optional output bias [Fout]; pass null to omit (FLOATING_POINT type)
   * @param colIdx CSR column indices [nnz] INT32 (INT type)
   * @param rowPtr CSR row pointers [rows+1] INT32 (INT type)
   * @param rows Number of nodes
   * @param n Number of nodes (for CsrEdgeGather doDiff)
   * @param fin Input feature dimension
   * @param fout Output feature dimension
   * @return out Node embeddings [rows, Fout] (FLOATING_POINT type)
   */
  public SDVariable nnConv(String name, SDVariable X, SDVariable edgeFeatures, SDVariable edgeNetW,
      SDVariable edgeNetB, SDVariable rootW, SDVariable bias, SDVariable colIdx, SDVariable rowPtr,
      long rows, long n, long fin, long fout) {
    SDValidation.validateFloatingPoint("nnConv", "X", X);
    SDValidation.validateFloatingPoint("nnConv", "edgeFeatures", edgeFeatures);
    SDValidation.validateFloatingPoint("nnConv", "edgeNetW", edgeNetW);
    SDValidation.validateFloatingPoint("nnConv", "edgeNetB", edgeNetB);
    SDValidation.validateFloatingPoint("nnConv", "rootW", rootW);
    SDValidation.validateFloatingPoint("nnConv", "bias", bias);
    SDValidation.validateInteger("nnConv", "colIdx", colIdx);
    SDValidation.validateInteger("nnConv", "rowPtr", rowPtr);
    SDVariable Xj = sd.sparse().csrEdgeGather(colIdx, X, (int) n);
    SDVariable eW = sd.mmul(edgeFeatures, edgeNetW).add(edgeNetB);
    SDVariable eWr = sd.reshape(eW, -1, fin, fout);
    SDVariable XjE = sd.reshape(Xj, -1, fin, 1);
    SDVariable prod = XjE.mul(eWr);
    SDVariable msg = sd.sum(prod, false, 1);
    SDVariable agg = sd.sparse().csrEdgeAggregate(rowPtr, msg, (int) rows, 0);
    SDVariable out = agg.add(sd.mmul(X, rootW));
    if (bias != null) {
        out = out.add(bias);
    }
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * PairNorm (Zhao & Akoglu 2020): parameter-free normalisation that keeps total pairwise feature distance constant.<br>
   *
   * @param X Node features [rows, F] (FLOATING_POINT type)
   * @param scale Target row-norm scale s (typically ~1.0)
   * @return out Normalised node features [rows, F] (FLOATING_POINT type)
   */
  public SDVariable pairNorm(SDVariable X, double scale) {
    SDValidation.validateFloatingPoint("pairNorm", "X", X);
    SDVariable mean = sd.mean(X, true, 0);
    SDVariable Xc = X.sub(mean);
    SDVariable rowSq = sd.sum(Xc.mul(Xc), true, 1);
    SDVariable denom = sd.math().sqrt(sd.mean(rowSq, false).add(1e-6));
    SDVariable out = Xc.mul(scale).div(denom);
    return out;
  }

  /**
   * PairNorm (Zhao & Akoglu 2020): parameter-free normalisation that keeps total pairwise feature distance constant.<br>
   *
   * @param name name May be null. Name for the output variable
   * @param X Node features [rows, F] (FLOATING_POINT type)
   * @param scale Target row-norm scale s (typically ~1.0)
   * @return out Normalised node features [rows, F] (FLOATING_POINT type)
   */
  public SDVariable pairNorm(String name, SDVariable X, double scale) {
    SDValidation.validateFloatingPoint("pairNorm", "X", X);
    SDVariable mean = sd.mean(X, true, 0);
    SDVariable Xc = X.sub(mean);
    SDVariable rowSq = sd.sum(Xc.mul(Xc), true, 1);
    SDVariable denom = sd.math().sqrt(sd.mean(rowSq, false).add(1e-6));
    SDVariable out = Xc.mul(scale).div(denom);
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * Principal Neighbourhood Aggregation convolution (Corso et al. 2020).<br>
   * Combines mean, max, min and std aggregators, concatenated and linearly projected.<br>
   *
   * @param X Node features [rows, F] (FLOATING_POINT type)
   * @param W Weight matrix [4F, H] (FLOATING_POINT type)
   * @param bias Optional bias [H]; pass null to omit (FLOATING_POINT type)
   * @param colIdx CSR column indices [nnz] INT32 (INT type)
   * @param rowPtr CSR row pointers [rows+1] INT32 (INT type)
   * @param rows Number of nodes
   * @param n Number of nodes (for CsrEdgeGather doDiff)
   * @param applyRelu Whether to apply ReLU to the output
   * @return out Node embeddings [rows, H] (FLOATING_POINT type)
   */
  public SDVariable pnaConv(SDVariable X, SDVariable W, SDVariable bias, SDVariable colIdx,
      SDVariable rowPtr, long rows, long n, boolean applyRelu) {
    SDValidation.validateFloatingPoint("pnaConv", "X", X);
    SDValidation.validateFloatingPoint("pnaConv", "W", W);
    SDValidation.validateFloatingPoint("pnaConv", "bias", bias);
    SDValidation.validateInteger("pnaConv", "colIdx", colIdx);
    SDValidation.validateInteger("pnaConv", "rowPtr", rowPtr);
    SDVariable Xj = sd.sparse().csrEdgeGather(colIdx, X, (int) n);
    SDVariable mean = sd.sparse().csrEdgeAggregate(rowPtr, Xj, (int) rows, 1);
    SDVariable max = sd.sparse().csrEdgeAggregate(rowPtr, Xj, (int) rows, 2);
    SDVariable min = sd.sparse().csrEdgeAggregate(rowPtr, Xj.mul(-1.0), (int) rows, 2).mul(-1.0);
    SDVariable meanSq = sd.sparse().csrEdgeAggregate(rowPtr, Xj.mul(Xj), (int) rows, 1);
    SDVariable std = sd.math().sqrt(meanSq.sub(mean.mul(mean)).add(1e-6));
    SDVariable agg = sd.concat(1, mean, max, min, std);
    SDVariable out = sd.mmul(agg, W);
    if (bias != null) {
        out = out.add(bias);
    }
    if (applyRelu) {
        out = sd.nn().relu(out, 0.0);
    }
    return out;
  }

  /**
   * Principal Neighbourhood Aggregation convolution (Corso et al. 2020).<br>
   * Combines mean, max, min and std aggregators, concatenated and linearly projected.<br>
   *
   * @param name name May be null. Name for the output variable
   * @param X Node features [rows, F] (FLOATING_POINT type)
   * @param W Weight matrix [4F, H] (FLOATING_POINT type)
   * @param bias Optional bias [H]; pass null to omit (FLOATING_POINT type)
   * @param colIdx CSR column indices [nnz] INT32 (INT type)
   * @param rowPtr CSR row pointers [rows+1] INT32 (INT type)
   * @param rows Number of nodes
   * @param n Number of nodes (for CsrEdgeGather doDiff)
   * @param applyRelu Whether to apply ReLU to the output
   * @return out Node embeddings [rows, H] (FLOATING_POINT type)
   */
  public SDVariable pnaConv(String name, SDVariable X, SDVariable W, SDVariable bias,
      SDVariable colIdx, SDVariable rowPtr, long rows, long n, boolean applyRelu) {
    SDValidation.validateFloatingPoint("pnaConv", "X", X);
    SDValidation.validateFloatingPoint("pnaConv", "W", W);
    SDValidation.validateFloatingPoint("pnaConv", "bias", bias);
    SDValidation.validateInteger("pnaConv", "colIdx", colIdx);
    SDValidation.validateInteger("pnaConv", "rowPtr", rowPtr);
    SDVariable Xj = sd.sparse().csrEdgeGather(colIdx, X, (int) n);
    SDVariable mean = sd.sparse().csrEdgeAggregate(rowPtr, Xj, (int) rows, 1);
    SDVariable max = sd.sparse().csrEdgeAggregate(rowPtr, Xj, (int) rows, 2);
    SDVariable min = sd.sparse().csrEdgeAggregate(rowPtr, Xj.mul(-1.0), (int) rows, 2).mul(-1.0);
    SDVariable meanSq = sd.sparse().csrEdgeAggregate(rowPtr, Xj.mul(Xj), (int) rows, 1);
    SDVariable std = sd.math().sqrt(meanSq.sub(mean.mul(mean)).add(1e-6));
    SDVariable agg = sd.concat(1, mean, max, min, std);
    SDVariable out = sd.mmul(agg, W);
    if (bias != null) {
        out = out.add(bias);
    }
    if (applyRelu) {
        out = sd.nn().relu(out, 0.0);
    }
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * Single-head relational GAT: graph attention with relation embedding modulating each edge's message.<br>
   *
   * @param X Node features [N, F] (FLOATING_POINT type)
   * @param W Linear weight [F, H] (FLOATING_POINT type)
   * @param relEmb Relation embeddings [numRelations, H] (FLOATING_POINT type)
   * @param edgeRelIdx Per-edge relation index [nnz] INT32 (INT type)
   * @param att Shared attention vector [H, 1] (FLOATING_POINT type)
   * @param colIdx CSR column indices (source nodes) [nnz] INT32 (INT type)
   * @param rowPtr CSR row pointers [N+1] INT32 (INT type)
   * @param rowIdx Per-edge destination index [nnz] INT32 (INT type)
   * @param nnz Number of edges
   * @param N Number of nodes
   * @param leakyAlpha LeakyReLU negative slope
   * @return out Aggregated node embeddings [N, H] (FLOATING_POINT type)
   */
  public SDVariable rgatConvHead(SDVariable X, SDVariable W, SDVariable relEmb,
      SDVariable edgeRelIdx, SDVariable att, SDVariable colIdx, SDVariable rowPtr,
      SDVariable rowIdx, long nnz, long N, double leakyAlpha) {
    SDValidation.validateFloatingPoint("rgatConvHead", "X", X);
    SDValidation.validateFloatingPoint("rgatConvHead", "W", W);
    SDValidation.validateFloatingPoint("rgatConvHead", "relEmb", relEmb);
    SDValidation.validateInteger("rgatConvHead", "edgeRelIdx", edgeRelIdx);
    SDValidation.validateFloatingPoint("rgatConvHead", "att", att);
    SDValidation.validateInteger("rgatConvHead", "colIdx", colIdx);
    SDValidation.validateInteger("rgatConvHead", "rowPtr", rowPtr);
    SDValidation.validateInteger("rgatConvHead", "rowIdx", rowIdx);
    SDVariable Wh = sd.mmul(X, W);
    SDVariable src = sd.sparse().csrEdgeGather(colIdx, Wh, (int) N);
    SDVariable dst = sd.gather(Wh, rowIdx, 0);
    SDVariable rel = sd.gather(relEmb, edgeRelIdx, 0);
    SDVariable msg = src.add(rel);
    SDVariable g = sd.nn().leakyRelu(msg.add(dst), leakyAlpha);
    SDVariable e = sd.reshape(sd.mmul(g, att), nnz);
    SDVariable alpha = sd.sparse().csrRowSoftmax(e, rowPtr, (int) N);
    SDVariable weighted = msg.mul(sd.reshape(alpha, nnz, 1));
    SDVariable out = sd.sparse().csrEdgeAggregate(rowPtr, weighted, (int) N, 0);
    return out;
  }

  /**
   * Single-head relational GAT: graph attention with relation embedding modulating each edge's message.<br>
   *
   * @param name name May be null. Name for the output variable
   * @param X Node features [N, F] (FLOATING_POINT type)
   * @param W Linear weight [F, H] (FLOATING_POINT type)
   * @param relEmb Relation embeddings [numRelations, H] (FLOATING_POINT type)
   * @param edgeRelIdx Per-edge relation index [nnz] INT32 (INT type)
   * @param att Shared attention vector [H, 1] (FLOATING_POINT type)
   * @param colIdx CSR column indices (source nodes) [nnz] INT32 (INT type)
   * @param rowPtr CSR row pointers [N+1] INT32 (INT type)
   * @param rowIdx Per-edge destination index [nnz] INT32 (INT type)
   * @param nnz Number of edges
   * @param N Number of nodes
   * @param leakyAlpha LeakyReLU negative slope
   * @return out Aggregated node embeddings [N, H] (FLOATING_POINT type)
   */
  public SDVariable rgatConvHead(String name, SDVariable X, SDVariable W, SDVariable relEmb,
      SDVariable edgeRelIdx, SDVariable att, SDVariable colIdx, SDVariable rowPtr,
      SDVariable rowIdx, long nnz, long N, double leakyAlpha) {
    SDValidation.validateFloatingPoint("rgatConvHead", "X", X);
    SDValidation.validateFloatingPoint("rgatConvHead", "W", W);
    SDValidation.validateFloatingPoint("rgatConvHead", "relEmb", relEmb);
    SDValidation.validateInteger("rgatConvHead", "edgeRelIdx", edgeRelIdx);
    SDValidation.validateFloatingPoint("rgatConvHead", "att", att);
    SDValidation.validateInteger("rgatConvHead", "colIdx", colIdx);
    SDValidation.validateInteger("rgatConvHead", "rowPtr", rowPtr);
    SDValidation.validateInteger("rgatConvHead", "rowIdx", rowIdx);
    SDVariable Wh = sd.mmul(X, W);
    SDVariable src = sd.sparse().csrEdgeGather(colIdx, Wh, (int) N);
    SDVariable dst = sd.gather(Wh, rowIdx, 0);
    SDVariable rel = sd.gather(relEmb, edgeRelIdx, 0);
    SDVariable msg = src.add(rel);
    SDVariable g = sd.nn().leakyRelu(msg.add(dst), leakyAlpha);
    SDVariable e = sd.reshape(sd.mmul(g, att), nnz);
    SDVariable alpha = sd.sparse().csrRowSoftmax(e, rowPtr, (int) N);
    SDVariable weighted = msg.mul(sd.reshape(alpha, nnz, 1));
    SDVariable out = sd.sparse().csrEdgeAggregate(rowPtr, weighted, (int) N, 0);
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * Relational Graph Convolutional Network layer (Schlichtkrull et al. 2018).<br>
   * out = X*W_self + sum_r(A_r*X*W_r) + bias<br>
   *
   * @param X Node features [rows, F] (FLOATING_POINT type)
   * @param relVals Per-relation CSR values, relVals[r]=[nnz_r] (FLOATING_POINT type)
   * @param relColIdx Per-relation CSR column indices [nnz_r] INT32 (INT type)
   * @param relRowPtr Per-relation CSR row pointers [rows+1] INT32 (INT type)
   * @param relW Per-relation weights relW[r]=[F,H] (FLOATING_POINT type)
   * @param selfW Self-loop weight [F, H] (FLOATING_POINT type)
   * @param bias Optional bias [H]; pass null to omit (FLOATING_POINT type)
   * @param rows Number of nodes
   * @param cols Number of columns of each A_r
   * @param applyRelu Whether to apply ReLU to the output
   * @return out Node embeddings [rows, H] (FLOATING_POINT type)
   */
  public SDVariable rgcnConv(SDVariable X, SDVariable[] relVals, SDVariable[] relColIdx,
      SDVariable[] relRowPtr, SDVariable[] relW, SDVariable selfW, SDVariable bias, long rows,
      long cols, boolean applyRelu) {
    SDValidation.validateFloatingPoint("rgcnConv", "X", X);
    SDValidation.validateFloatingPoint("rgcnConv", "relVals", relVals);
    Preconditions.checkArgument(relVals.length >= 1, "relVals has incorrect size/length. Expected: relVals.length >= 1, got %s", relVals.length);
    SDValidation.validateInteger("rgcnConv", "relColIdx", relColIdx);
    Preconditions.checkArgument(relColIdx.length >= 1, "relColIdx has incorrect size/length. Expected: relColIdx.length >= 1, got %s", relColIdx.length);
    SDValidation.validateInteger("rgcnConv", "relRowPtr", relRowPtr);
    Preconditions.checkArgument(relRowPtr.length >= 1, "relRowPtr has incorrect size/length. Expected: relRowPtr.length >= 1, got %s", relRowPtr.length);
    SDValidation.validateFloatingPoint("rgcnConv", "relW", relW);
    Preconditions.checkArgument(relW.length >= 1, "relW has incorrect size/length. Expected: relW.length >= 1, got %s", relW.length);
    SDValidation.validateFloatingPoint("rgcnConv", "selfW", selfW);
    SDValidation.validateFloatingPoint("rgcnConv", "bias", bias);
    SDVariable out = sd.mmul(X, selfW);
    for (int r = 0; r < relW.length; r++) {
        SDVariable aX = sd.sparse().csrSpmm(relVals[r], relColIdx[r], relRowPtr[r], X, (int) rows, (int) cols, false);
        out = out.add(sd.mmul(aX, relW[r]));
    }
    if (bias != null) {
        out = out.add(bias);
    }
    if (applyRelu) {
        out = sd.nn().relu(out, 0.0);
    }
    return out;
  }

  /**
   * Relational Graph Convolutional Network layer (Schlichtkrull et al. 2018).<br>
   * out = X*W_self + sum_r(A_r*X*W_r) + bias<br>
   *
   * @param name name May be null. Name for the output variable
   * @param X Node features [rows, F] (FLOATING_POINT type)
   * @param relVals Per-relation CSR values, relVals[r]=[nnz_r] (FLOATING_POINT type)
   * @param relColIdx Per-relation CSR column indices [nnz_r] INT32 (INT type)
   * @param relRowPtr Per-relation CSR row pointers [rows+1] INT32 (INT type)
   * @param relW Per-relation weights relW[r]=[F,H] (FLOATING_POINT type)
   * @param selfW Self-loop weight [F, H] (FLOATING_POINT type)
   * @param bias Optional bias [H]; pass null to omit (FLOATING_POINT type)
   * @param rows Number of nodes
   * @param cols Number of columns of each A_r
   * @param applyRelu Whether to apply ReLU to the output
   * @return out Node embeddings [rows, H] (FLOATING_POINT type)
   */
  public SDVariable rgcnConv(String name, SDVariable X, SDVariable[] relVals,
      SDVariable[] relColIdx, SDVariable[] relRowPtr, SDVariable[] relW, SDVariable selfW,
      SDVariable bias, long rows, long cols, boolean applyRelu) {
    SDValidation.validateFloatingPoint("rgcnConv", "X", X);
    SDValidation.validateFloatingPoint("rgcnConv", "relVals", relVals);
    Preconditions.checkArgument(relVals.length >= 1, "relVals has incorrect size/length. Expected: relVals.length >= 1, got %s", relVals.length);
    SDValidation.validateInteger("rgcnConv", "relColIdx", relColIdx);
    Preconditions.checkArgument(relColIdx.length >= 1, "relColIdx has incorrect size/length. Expected: relColIdx.length >= 1, got %s", relColIdx.length);
    SDValidation.validateInteger("rgcnConv", "relRowPtr", relRowPtr);
    Preconditions.checkArgument(relRowPtr.length >= 1, "relRowPtr has incorrect size/length. Expected: relRowPtr.length >= 1, got %s", relRowPtr.length);
    SDValidation.validateFloatingPoint("rgcnConv", "relW", relW);
    Preconditions.checkArgument(relW.length >= 1, "relW has incorrect size/length. Expected: relW.length >= 1, got %s", relW.length);
    SDValidation.validateFloatingPoint("rgcnConv", "selfW", selfW);
    SDValidation.validateFloatingPoint("rgcnConv", "bias", bias);
    SDVariable out = sd.mmul(X, selfW);
    for (int r = 0; r < relW.length; r++) {
        SDVariable aX = sd.sparse().csrSpmm(relVals[r], relColIdx[r], relRowPtr[r], X, (int) rows, (int) cols, false);
        out = out.add(sd.mmul(aX, relW[r]));
    }
    if (bias != null) {
        out = out.add(bias);
    }
    if (applyRelu) {
        out = sd.nn().relu(out, 0.0);
    }
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * GraphSAGE max aggregation.<br>
   *
   * @param X Node features [rows, F] (FLOATING_POINT type)
   * @param W Weight matrix [2F, H] (FLOATING_POINT type)
   * @param bias Optional bias [H]; pass null to omit (FLOATING_POINT type)
   * @param colIdx CSR column indices [nnz] INT32 (INT type)
   * @param rowPtr CSR row pointers [rows+1] INT32 (INT type)
   * @param rows Number of nodes
   * @return out Node embeddings [rows, H] (FLOATING_POINT type)
   */
  public SDVariable sageMax(SDVariable X, SDVariable W, SDVariable bias, SDVariable colIdx,
      SDVariable rowPtr, long rows) {
    SDValidation.validateFloatingPoint("sageMax", "X", X);
    SDValidation.validateFloatingPoint("sageMax", "W", W);
    SDValidation.validateFloatingPoint("sageMax", "bias", bias);
    SDValidation.validateInteger("sageMax", "colIdx", colIdx);
    SDValidation.validateInteger("sageMax", "rowPtr", rowPtr);
    SDVariable agg = sd.sparse().csrSegmentMax(colIdx, rowPtr, X, (int) rows);
    SDVariable cat = sd.concat(1, X, agg);
    SDVariable h = sd.mmul(cat, W);
    if (bias != null) {
        h = h.add(bias);
    }
    SDVariable out = sd.nn().relu(h, 0.0);
    return out;
  }

  /**
   * GraphSAGE max aggregation.<br>
   *
   * @param name name May be null. Name for the output variable
   * @param X Node features [rows, F] (FLOATING_POINT type)
   * @param W Weight matrix [2F, H] (FLOATING_POINT type)
   * @param bias Optional bias [H]; pass null to omit (FLOATING_POINT type)
   * @param colIdx CSR column indices [nnz] INT32 (INT type)
   * @param rowPtr CSR row pointers [rows+1] INT32 (INT type)
   * @param rows Number of nodes
   * @return out Node embeddings [rows, H] (FLOATING_POINT type)
   */
  public SDVariable sageMax(String name, SDVariable X, SDVariable W, SDVariable bias,
      SDVariable colIdx, SDVariable rowPtr, long rows) {
    SDValidation.validateFloatingPoint("sageMax", "X", X);
    SDValidation.validateFloatingPoint("sageMax", "W", W);
    SDValidation.validateFloatingPoint("sageMax", "bias", bias);
    SDValidation.validateInteger("sageMax", "colIdx", colIdx);
    SDValidation.validateInteger("sageMax", "rowPtr", rowPtr);
    SDVariable agg = sd.sparse().csrSegmentMax(colIdx, rowPtr, X, (int) rows);
    SDVariable cat = sd.concat(1, X, agg);
    SDVariable h = sd.mmul(cat, W);
    if (bias != null) {
        h = h.add(bias);
    }
    SDVariable out = sd.nn().relu(h, 0.0);
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * GraphSAGE mean aggregation (Hamilton et al. 2017).<br>
   *
   * @param X Node features [rows, F] (FLOATING_POINT type)
   * @param W Weight matrix [2F, H] (FLOATING_POINT type)
   * @param bias Optional bias [H]; pass null to omit (FLOATING_POINT type)
   * @param colIdx CSR column indices [nnz] INT32 (INT type)
   * @param rowPtr CSR row pointers [rows+1] INT32 (INT type)
   * @param rows Number of nodes
   * @param n Number of nodes (= X.shape[0], for CsrEdgeGather doDiff)
   * @return out Node embeddings [rows, H] (FLOATING_POINT type)
   */
  public SDVariable sageMean(SDVariable X, SDVariable W, SDVariable bias, SDVariable colIdx,
      SDVariable rowPtr, long rows, long n) {
    SDValidation.validateFloatingPoint("sageMean", "X", X);
    SDValidation.validateFloatingPoint("sageMean", "W", W);
    SDValidation.validateFloatingPoint("sageMean", "bias", bias);
    SDValidation.validateInteger("sageMean", "colIdx", colIdx);
    SDValidation.validateInteger("sageMean", "rowPtr", rowPtr);
    SDVariable Xj = sd.sparse().csrEdgeGather(colIdx, X, (int) n);
    SDVariable agg = sd.sparse().csrEdgeAggregate(rowPtr, Xj, (int) rows, 1);
    SDVariable cat = sd.concat(1, X, agg);
    SDVariable h = sd.mmul(cat, W);
    if (bias != null) {
        h = h.add(bias);
    }
    SDVariable out = sd.nn().relu(h, 0.0);
    return out;
  }

  /**
   * GraphSAGE mean aggregation (Hamilton et al. 2017).<br>
   *
   * @param name name May be null. Name for the output variable
   * @param X Node features [rows, F] (FLOATING_POINT type)
   * @param W Weight matrix [2F, H] (FLOATING_POINT type)
   * @param bias Optional bias [H]; pass null to omit (FLOATING_POINT type)
   * @param colIdx CSR column indices [nnz] INT32 (INT type)
   * @param rowPtr CSR row pointers [rows+1] INT32 (INT type)
   * @param rows Number of nodes
   * @param n Number of nodes (= X.shape[0], for CsrEdgeGather doDiff)
   * @return out Node embeddings [rows, H] (FLOATING_POINT type)
   */
  public SDVariable sageMean(String name, SDVariable X, SDVariable W, SDVariable bias,
      SDVariable colIdx, SDVariable rowPtr, long rows, long n) {
    SDValidation.validateFloatingPoint("sageMean", "X", X);
    SDValidation.validateFloatingPoint("sageMean", "W", W);
    SDValidation.validateFloatingPoint("sageMean", "bias", bias);
    SDValidation.validateInteger("sageMean", "colIdx", colIdx);
    SDValidation.validateInteger("sageMean", "rowPtr", rowPtr);
    SDVariable Xj = sd.sparse().csrEdgeGather(colIdx, X, (int) n);
    SDVariable agg = sd.sparse().csrEdgeAggregate(rowPtr, Xj, (int) rows, 1);
    SDVariable cat = sd.concat(1, X, agg);
    SDVariable h = sd.mmul(cat, W);
    if (bias != null) {
        h = h.add(bias);
    }
    SDVariable out = sd.nn().relu(h, 0.0);
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * GraphSAGE pool aggregation: apply an MLP to each neighbour, max-aggregate, then predict.<br>
   *
   * @param X Node features [rows, F] (FLOATING_POINT type)
   * @param wPool MLP weight for neighbours [F, H_pool] (FLOATING_POINT type)
   * @param bPool MLP bias [H_pool] (FLOATING_POINT type)
   * @param wOut Output weight [F + H_pool, H_out] (FLOATING_POINT type)
   * @param bOut Output bias [H_out]; pass null to omit (FLOATING_POINT type)
   * @param colIdx CSR column indices [nnz] INT32 (INT type)
   * @param rowPtr CSR row pointers [rows+1] INT32 (INT type)
   * @param rows Number of nodes
   * @param n Number of nodes (for CsrEdgeGather doDiff)
   * @return out Node embeddings [rows, H_out] (FLOATING_POINT type)
   */
  public SDVariable sagePool(SDVariable X, SDVariable wPool, SDVariable bPool, SDVariable wOut,
      SDVariable bOut, SDVariable colIdx, SDVariable rowPtr, long rows, long n) {
    SDValidation.validateFloatingPoint("sagePool", "X", X);
    SDValidation.validateFloatingPoint("sagePool", "wPool", wPool);
    SDValidation.validateFloatingPoint("sagePool", "bPool", bPool);
    SDValidation.validateFloatingPoint("sagePool", "wOut", wOut);
    SDValidation.validateFloatingPoint("sagePool", "bOut", bOut);
    SDValidation.validateInteger("sagePool", "colIdx", colIdx);
    SDValidation.validateInteger("sagePool", "rowPtr", rowPtr);
    SDVariable Xj = sd.sparse().csrEdgeGather(colIdx, X, (int) n);
    SDVariable hj = sd.nn().relu(sd.mmul(Xj, wPool).add(bPool), 0.0);
    SDVariable agg = sd.sparse().csrEdgeAggregate(rowPtr, hj, (int) rows, 2);
    SDVariable cat = sd.concat(1, X, agg);
    SDVariable out = sd.mmul(cat, wOut);
    if (bOut != null) {
        out = out.add(bOut);
    }
    out = sd.nn().relu(out, 0.0);
    return out;
  }

  /**
   * GraphSAGE pool aggregation: apply an MLP to each neighbour, max-aggregate, then predict.<br>
   *
   * @param name name May be null. Name for the output variable
   * @param X Node features [rows, F] (FLOATING_POINT type)
   * @param wPool MLP weight for neighbours [F, H_pool] (FLOATING_POINT type)
   * @param bPool MLP bias [H_pool] (FLOATING_POINT type)
   * @param wOut Output weight [F + H_pool, H_out] (FLOATING_POINT type)
   * @param bOut Output bias [H_out]; pass null to omit (FLOATING_POINT type)
   * @param colIdx CSR column indices [nnz] INT32 (INT type)
   * @param rowPtr CSR row pointers [rows+1] INT32 (INT type)
   * @param rows Number of nodes
   * @param n Number of nodes (for CsrEdgeGather doDiff)
   * @return out Node embeddings [rows, H_out] (FLOATING_POINT type)
   */
  public SDVariable sagePool(String name, SDVariable X, SDVariable wPool, SDVariable bPool,
      SDVariable wOut, SDVariable bOut, SDVariable colIdx, SDVariable rowPtr, long rows, long n) {
    SDValidation.validateFloatingPoint("sagePool", "X", X);
    SDValidation.validateFloatingPoint("sagePool", "wPool", wPool);
    SDValidation.validateFloatingPoint("sagePool", "bPool", bPool);
    SDValidation.validateFloatingPoint("sagePool", "wOut", wOut);
    SDValidation.validateFloatingPoint("sagePool", "bOut", bOut);
    SDValidation.validateInteger("sagePool", "colIdx", colIdx);
    SDValidation.validateInteger("sagePool", "rowPtr", rowPtr);
    SDVariable Xj = sd.sparse().csrEdgeGather(colIdx, X, (int) n);
    SDVariable hj = sd.nn().relu(sd.mmul(Xj, wPool).add(bPool), 0.0);
    SDVariable agg = sd.sparse().csrEdgeAggregate(rowPtr, hj, (int) rows, 2);
    SDVariable cat = sd.concat(1, X, agg);
    SDVariable out = sd.mmul(cat, wOut);
    if (bOut != null) {
        out = out.add(bOut);
    }
    out = sd.nn().relu(out, 0.0);
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * Temporal Graph Convolutional Network: weight-shared spatial GCN applied at each timestep,<br>
   * fused by temporal attention. For each timestep t: h[t] = gcnConv(X_t, W, bias, A_norm).<br>
   * Temporal attention score = mean(tanh(h[t]*tempW)*tempQ). Softmax gives beta_t;<br>
   * out = sum_t(beta_t * h[t]).<br>
   *
   * @param Xt Per-timestep node features, each [N, F] (FLOATING_POINT type)
   * @param W Shared spatial GCN weight [F, H] (FLOATING_POINT type)
   * @param bias Shared spatial GCN bias [H]; pass null to omit (FLOATING_POINT type)
   * @param aNormVals CSR values of the normalised adjacency [nnz] (FLOATING_POINT type)
   * @param aNormColIdx CSR column indices [nnz] INT32 (INT type)
   * @param aNormRowPtr CSR row pointers [rows+1] INT32 (INT type)
   * @param tempW Temporal-attention weight [H, A] (FLOATING_POINT type)
   * @param tempQ Temporal-attention query [A, 1] (FLOATING_POINT type)
   * @param rows Number of nodes
   * @param cols Columns of A_norm (= rows for square graphs)
   * @param applyRelu Whether the spatial GCN applies ReLU
   * @return out Temporally-fused node embeddings [N, H] (FLOATING_POINT type)
   */
  public SDVariable temporalGcn(SDVariable[] Xt, SDVariable W, SDVariable bias,
      SDVariable aNormVals, SDVariable aNormColIdx, SDVariable aNormRowPtr, SDVariable tempW,
      SDVariable tempQ, long rows, long cols, boolean applyRelu) {
    SDValidation.validateFloatingPoint("temporalGcn", "Xt", Xt);
    Preconditions.checkArgument(Xt.length >= 1, "Xt has incorrect size/length. Expected: Xt.length >= 1, got %s", Xt.length);
    SDValidation.validateFloatingPoint("temporalGcn", "W", W);
    SDValidation.validateFloatingPoint("temporalGcn", "bias", bias);
    SDValidation.validateFloatingPoint("temporalGcn", "aNormVals", aNormVals);
    SDValidation.validateInteger("temporalGcn", "aNormColIdx", aNormColIdx);
    SDValidation.validateInteger("temporalGcn", "aNormRowPtr", aNormRowPtr);
    SDValidation.validateFloatingPoint("temporalGcn", "tempW", tempW);
    SDValidation.validateFloatingPoint("temporalGcn", "tempQ", tempQ);
    final int T = Xt.length;
    SDVariable[] h = new SDVariable[T];
    SDVariable[] expw = new SDVariable[T];
    SDVariable sumExp = null;
    for (int t = 0; t < T; t++) {
        h[t] = sd.gnn().gcnConv(Xt[t], W, bias, aNormVals, aNormColIdx, aNormRowPtr, rows, cols, applyRelu);
        SDVariable score = sd.mean(sd.mmul(sd.math().tanh(sd.mmul(h[t], tempW)), tempQ), false);
        expw[t] = sd.math().exp(score);
        sumExp = (t == 0) ? expw[t] : sumExp.add(expw[t]);
    }
    SDVariable out = null;
    for (int t = 0; t < T; t++) {
        SDVariable betaT = expw[t].div(sumExp);
        SDVariable term  = h[t].mul(betaT);
        out = (t == 0) ? term : out.add(term);
    }
    return out;
  }

  /**
   * Temporal Graph Convolutional Network: weight-shared spatial GCN applied at each timestep,<br>
   * fused by temporal attention. For each timestep t: h[t] = gcnConv(X_t, W, bias, A_norm).<br>
   * Temporal attention score = mean(tanh(h[t]*tempW)*tempQ). Softmax gives beta_t;<br>
   * out = sum_t(beta_t * h[t]).<br>
   *
   * @param name name May be null. Name for the output variable
   * @param Xt Per-timestep node features, each [N, F] (FLOATING_POINT type)
   * @param W Shared spatial GCN weight [F, H] (FLOATING_POINT type)
   * @param bias Shared spatial GCN bias [H]; pass null to omit (FLOATING_POINT type)
   * @param aNormVals CSR values of the normalised adjacency [nnz] (FLOATING_POINT type)
   * @param aNormColIdx CSR column indices [nnz] INT32 (INT type)
   * @param aNormRowPtr CSR row pointers [rows+1] INT32 (INT type)
   * @param tempW Temporal-attention weight [H, A] (FLOATING_POINT type)
   * @param tempQ Temporal-attention query [A, 1] (FLOATING_POINT type)
   * @param rows Number of nodes
   * @param cols Columns of A_norm (= rows for square graphs)
   * @param applyRelu Whether the spatial GCN applies ReLU
   * @return out Temporally-fused node embeddings [N, H] (FLOATING_POINT type)
   */
  public SDVariable temporalGcn(String name, SDVariable[] Xt, SDVariable W, SDVariable bias,
      SDVariable aNormVals, SDVariable aNormColIdx, SDVariable aNormRowPtr, SDVariable tempW,
      SDVariable tempQ, long rows, long cols, boolean applyRelu) {
    SDValidation.validateFloatingPoint("temporalGcn", "Xt", Xt);
    Preconditions.checkArgument(Xt.length >= 1, "Xt has incorrect size/length. Expected: Xt.length >= 1, got %s", Xt.length);
    SDValidation.validateFloatingPoint("temporalGcn", "W", W);
    SDValidation.validateFloatingPoint("temporalGcn", "bias", bias);
    SDValidation.validateFloatingPoint("temporalGcn", "aNormVals", aNormVals);
    SDValidation.validateInteger("temporalGcn", "aNormColIdx", aNormColIdx);
    SDValidation.validateInteger("temporalGcn", "aNormRowPtr", aNormRowPtr);
    SDValidation.validateFloatingPoint("temporalGcn", "tempW", tempW);
    SDValidation.validateFloatingPoint("temporalGcn", "tempQ", tempQ);
    final int T = Xt.length;
    SDVariable[] h = new SDVariable[T];
    SDVariable[] expw = new SDVariable[T];
    SDVariable sumExp = null;
    for (int t = 0; t < T; t++) {
        h[t] = sd.gnn().gcnConv(Xt[t], W, bias, aNormVals, aNormColIdx, aNormRowPtr, rows, cols, applyRelu);
        SDVariable score = sd.mean(sd.mmul(sd.math().tanh(sd.mmul(h[t], tempW)), tempQ), false);
        expw[t] = sd.math().exp(score);
        sumExp = (t == 0) ? expw[t] : sumExp.add(expw[t]);
    }
    SDVariable out = null;
    for (int t = 0; t < T; t++) {
        SDVariable betaT = expw[t].div(sumExp);
        SDVariable term  = h[t].mul(betaT);
        out = (t == 0) ? term : out.add(term);
    }
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * VGAE KL-divergence regulariser (Kipf & Welling 2016): 0.5 * mean(exp(logvar) + mu^2 - logvar - 1)<br>
   *
   * @param mu Latent mean [N, d] (FLOATING_POINT type)
   * @param logvar Latent log-variance [N, d] (FLOATING_POINT type)
   * @return out Scalar KL divergence (FLOATING_POINT type)
   */
  public SDVariable vgaeKlLoss(SDVariable mu, SDVariable logvar) {
    SDValidation.validateFloatingPoint("vgaeKlLoss", "mu", mu);
    SDValidation.validateFloatingPoint("vgaeKlLoss", "logvar", logvar);
    SDVariable kl = sd.math().exp(logvar).add(mu.mul(mu)).sub(logvar).sub(1.0);
    SDVariable out = sd.mean(kl, false).mul(0.5);
    return out;
  }

  /**
   * VGAE KL-divergence regulariser (Kipf & Welling 2016): 0.5 * mean(exp(logvar) + mu^2 - logvar - 1)<br>
   *
   * @param name name May be null. Name for the output variable
   * @param mu Latent mean [N, d] (FLOATING_POINT type)
   * @param logvar Latent log-variance [N, d] (FLOATING_POINT type)
   * @return out Scalar KL divergence (FLOATING_POINT type)
   */
  public SDVariable vgaeKlLoss(String name, SDVariable mu, SDVariable logvar) {
    SDValidation.validateFloatingPoint("vgaeKlLoss", "mu", mu);
    SDValidation.validateFloatingPoint("vgaeKlLoss", "logvar", logvar);
    SDVariable kl = sd.math().exp(logvar).add(mu.mul(mu)).sub(logvar).sub(1.0);
    SDVariable out = sd.mean(kl, false).mul(0.5);
    return sd.updateVariableNameAndReference(out, name);
  }

  /**
   * VGAE reparameterisation trick (Kipf & Welling 2016): z = mu + exp(0.5*logvar) * noise<br>
   *
   * @param mu Latent mean [N, d] (FLOATING_POINT type)
   * @param logvar Latent log-variance [N, d] (FLOATING_POINT type)
   * @param noise Standard-normal sample [N, d] (FLOATING_POINT type)
   * @return out Latent sample z [N, d] (FLOATING_POINT type)
   */
  public SDVariable vgaeReparam(SDVariable mu, SDVariable logvar, SDVariable noise) {
    SDValidation.validateFloatingPoint("vgaeReparam", "mu", mu);
    SDValidation.validateFloatingPoint("vgaeReparam", "logvar", logvar);
    SDValidation.validateFloatingPoint("vgaeReparam", "noise", noise);
    SDVariable out = mu.add(sd.math().exp(logvar.mul(0.5)).mul(noise));
    return out;
  }

  /**
   * VGAE reparameterisation trick (Kipf & Welling 2016): z = mu + exp(0.5*logvar) * noise<br>
   *
   * @param name name May be null. Name for the output variable
   * @param mu Latent mean [N, d] (FLOATING_POINT type)
   * @param logvar Latent log-variance [N, d] (FLOATING_POINT type)
   * @param noise Standard-normal sample [N, d] (FLOATING_POINT type)
   * @return out Latent sample z [N, d] (FLOATING_POINT type)
   */
  public SDVariable vgaeReparam(String name, SDVariable mu, SDVariable logvar, SDVariable noise) {
    SDValidation.validateFloatingPoint("vgaeReparam", "mu", mu);
    SDValidation.validateFloatingPoint("vgaeReparam", "logvar", logvar);
    SDValidation.validateFloatingPoint("vgaeReparam", "noise", noise);
    SDVariable out = mu.add(sd.math().exp(logvar.mul(0.5)).mul(noise));
    return sd.updateVariableNameAndReference(out, name);
  }
}
