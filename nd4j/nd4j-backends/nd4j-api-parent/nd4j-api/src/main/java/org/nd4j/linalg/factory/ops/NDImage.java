...
public class NDImage {
  ...
  public INDArray imageResize(INDArray input, INDArray size, boolean preserveAspectRatio,
    ...
    NDValidation.validateNumerical("imageResize", "size", size);
    return Nd4j.exec(new org.nd4j.linalg.api.ops.impl.image.ImageResize(input, size, preserveAspectRatio, antialis, ImageResizeMethods))[0];
  }
   ...
   * @return output Output image (NUMERIC type)
   */
  public INDArray imageResize(INDArray input, INDArray size,
      ImageResizeMethods ImageResizeMethods) {
    NDValidation.validateNumerical("imageResize", "input", input);
    NDValidation.validateNumerical("imageResize", "size", size);
    return Nd4j.exec(new org.nd4j.linalg.api.ops.impl.image.ImageResize(input, size, false, false, ImageResizeMethods))[0];
  }
