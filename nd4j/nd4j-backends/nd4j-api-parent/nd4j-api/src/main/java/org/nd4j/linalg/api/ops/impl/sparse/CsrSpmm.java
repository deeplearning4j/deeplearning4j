...
/**
 * Sparse matrix-matrix product in CSR format: C = A * B  (or Aᵀ * B).
 *
 * <p>C++ op name: {@code csr_spmm}
 * <ul>
 *   <li>Inputs:  values[nnz], colIdx[nnz INT32], rowPtr[rows+1 INT32],
 *                B[(cols,n)] (or B[(rows,n)] when transposeA=true)</li>
 *   <li>IArgs:   rows, cols, transposeA (0 or 1)</li>
 *   <li>Output:  C[(rows,n)] (or C[(cols,n)] when transposeA=true)</li>
 * </ul>
 *
 * <p>Only {@code values} and {@code B} are differentiable; {@code colIdx} and {@code rowPtr}
 * are integer/structural and receive zero gradients.
 */
public class CsrSpmm extends DynamicCustomOp {
     ...
     * @param B          2D dense matrix [cols, n] (or [rows, n] if transposeA=true)
     ...
     */
    public CsrSpmm(INDArray values, INDArray colIdx, INDArray rowPtr, INDArray B,
     ...
     * @param B          SDVariable 2D dense matrix
     * @param rows       number of rows
     * @param cols       number of columns
     * @param transposeA if true compute Aᵀ · B
     */
    public CsrSpmm(SameDiff sd, SDVariable values, SDVariable colIdx, SDVariable rowPtr,
                   SDVariable B, long rows, long cols, boolean transposeA) {
    ...
    @Override
    public String opName() { return "csr_spmm"; }
    ...
    /**
     * Backward pass for csr_spmm — delegates to the dedicated {@code csr_spmm_bp} op.
     *
     * <p>Outputs of the backward op:
     * <ul>
     *   <li>[0] dAValues[nnz]: gradient w.r.t. CSR non-zero values</li>
     *   <li>[1] dB:            gradient w.r.t. the dense matrix B</li>
     * </ul>
    ...
    public List<SDVariable> doDiff(List<SDVariable> grads) {
        ...
        SDVariable BVar      = arg(3);

        // Delegate to dedicated backward op: outputs are [dAValues, dB]
        SDVariable[] bpOut = new CsrSpmmBp(sameDiff, valuesVar, colIdxVar, rowPtrVar, BVar, dC, r, c, t)
                .outputVariables();
