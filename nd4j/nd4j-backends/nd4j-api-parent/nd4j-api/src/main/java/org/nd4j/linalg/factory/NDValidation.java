...
public class NDValidation {
    ...
    public static void validateFloatingPoint(String opName, String inputName, INDArray[] vars) {
        for (INDArray v : vars) {
            if (v == null) continue;
            if (!v.dataType().isFPType())
                throw new IllegalStateException("Input \"" + inputName + "\" for operation \"" + opName +
                        "\" must be a floating point type; got array with non-floating point data type " + v.dataType());
        }
    }
