        ...
        private void outputMeasureInfo(MeasureState state) {
            ...
            if (isEnd(state)) {
                ...
                sb.append(state.getLabel()).append("\t").append("avg: ").append(decimalFormat.format(finalAvg))
                                .append(" ms\t").append("total: ").append(integerFormat.format(finalTotal))
                                .append(" s\t").append("   tps: ").append(integerFormat.format(finalTps)).append("\t")
                                .append("running: ").append(count).append(" times");
                count = 0;
                timeUsed = 0;
                if (!state.getLabel().equals("_warmup_")) {
        ...
        private void outputMeasureInfo(MeasureState state) {
            ...
            if (isEnd(state)) {
                ...
                sb.append("bytes-usage: ").append(state.getLabel()).append("\t").append(format((bytesUsed / count)))
                                .append(" Bytes\t").append(format((bytesUsed / count) / (1024.0 * 1024.0)))
                                .append(" Mb\n");
                count = 0;
                bytesUsed = 0;
        ...
        private void outputMeasureInfo(MeasureState state) {
            ...
            if (isEnd(state)) {
                ...
                sb.append("memory-usage: ").append(state.getLabel()).append("\t")
                                .append(format(finalBytes / (1024.0 * 1024.0))).append(" Mb\n");
                count = 0;
                memoryUsed = 0;
