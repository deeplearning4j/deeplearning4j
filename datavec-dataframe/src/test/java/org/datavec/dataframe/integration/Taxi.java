...
public class Taxi {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println(String.format("loaded %d records in %d seconds", trips.rowCount(),
        ...
        out(String.format("Grouped %d records in %d ms", trips.rowCount(), stopwatch.elapsed(TimeUnit.MILLISECONDS)));

        stopwatch.reset().start();
        Table mean_total_fare = vg.reduce("total_amount", NumericReduceUtils.mean);
        out(String.format("Averaged %d records in %d ms", trips.rowCount(), stopwatch.elapsed(TimeUnit.MILLISECONDS)));
        out(mean_total_fare.print());

        stopwatch.reset().start();
        Table sum_total_fare = vg.reduce("total_amount", NumericReduceUtils.sum);
        out(String.format("Summed %d records in %d ms", trips.rowCount(), stopwatch.elapsed(TimeUnit.MILLISECONDS)));
