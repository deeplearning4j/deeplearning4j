...
public class BigDecimalMath {
    ...
    static public BigDecimal sqrt(final BigDecimal x, final MathContext mc) {
        ...
        for (;;) {
             ...
             */
            if (Math.abs(
                    BigDecimal.ONE.subtract(x.divide(s.pow(2, locmc), locmc)).doubleValue()) < eps)
                break;
    ...
    static public BigDecimal exp(BigDecimal x) {
        ...
        if (x.compareTo(BigDecimal.ZERO) < 0) {
            ...
            if (Math.pow(xDbl, TAYLOR_NTERM) < TAYLOR_NTERM * (TAYLOR_NTERM - 1.0)
                    * (TAYLOR_NTERM - 2.0) * xUlpDbl) {
                /*
                 ...
                 */
                int exSc = (int) (1.0
                        - Math.log10(TAYLOR_NTERM * (TAYLOR_NTERM - 1.0) * (TAYLOR_NTERM - 2.0)
    ...
    static public BigDecimal log(int n, final MathContext mc) {
        ...
        if (n <= 0)
            ...
            if (mc.getPrecision() < LOG2.precision())
                 ...
                 * a> Error propagation: the error in log(2) is twice the error
                 * in S(2,-5,...).
                 */
    ...
    static public BigDecimal powRound(final BigDecimal x, final int n) {
        ...
        if (n == 1)
            ...
            MathContext mc = new MathContext(
                    x.precision() - (int) Math.log10((double) (Math.abs(n))));
            if (n > 0)
                return x.pow(n, mc);
            else
    ...
    static public BigDecimal cos(final BigDecimal x) {
        if (x.compareTo(BigDecimal.ZERO) < 0)
            ...
            if (res.compareTo(p) > 0) {
                 ...
                 * higher up. throw new ProviderException(
                 * "Not implemented: cosine ") ;
                 */
                if (res.multiply(new BigDecimal("4")).compareTo(p) > 0) {
                    /*
        ...
        if (x.compareTo(BigDecimal.ZERO) == 0)
            ...
            if (xDbl > 0.8) {
                ...
                for (int i = 2;; i++) {
                    ...
                    fac = fac.multiply(new BigInteger("" + (2 * i)))
        ...
        if (x.compareTo(BigDecimal.ZERO) < 0)
            ...
            if (x.compareTo(BigDecimal.ONE) != 0) {
                ...
                for (int n = 2;; n++) {
                    ...
                    MathContext m = new MathContext(
                            err2prec(n * z.ulp().doubleValue() / 2. / z.doubleValue()));
                    c = c.round(m);
    ...
    static public BigDecimal zeta(final int n, final MathContext mc) {
        ...
        if (n % 2 == 0) {
             ...
             */
            MathContext mcloc = new MathContext(
                    2 + mc.getPrecision() + (int) (Math.log10((double) (n))));
    ...
    static public double psi(final double x) {
        ...
        if (x > 2.0) {
            ...
            return resul + 0.422784335098467139393487909917597568 + 0.5 / xmin1
                    - 1. / (1 - xmin1 * xmin1) - Math.PI / (2. * Math.tan(Math.PI * xmin1));
        }
    } /* psi */


    static protected BigDecimal broadhurstBBP(final int n, final int p, final int a[],
            MathContext mc) {
        /*
