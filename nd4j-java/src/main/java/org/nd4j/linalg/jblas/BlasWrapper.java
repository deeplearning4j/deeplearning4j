...
public class BlasWrapper implements org.nd4j.linalg.factory.BlasWrapper {
    ...
    public INDArray scal(double alpha, INDArray x) {
        ...
        for(int i = 0; i < x.length(); i++) {
            xLinear.putScalar(i,xLinear.getDouble(i) * alpha);
        }
        return x;
    }
    ...
    public IComplexNDArray scal(IComplexFloat alpha, IComplexNDArray x) {
        ...
        for(int i = 0; i < x.length(); i++) {
            xLinear.putScalar(i,xLinear.getComplex(i).mul(alpha));
        }
        return x;
    ...
    public IComplexNDArray axpy(IComplexNumber da, IComplexNDArray dx, IComplexNDArray dy) {
        ...
        IComplexNDArray dxLinear = dx.linearView();
        IComplexNDArray dyLinear = dy.linearView();
        for(int i = 0; i < dx.length(); i++) {
            dyLinear.putScalar(i,dxLinear.getComplex(i).mul(da).add(dyLinear.getComplex(i)));
        }
    ...
    public IComplexNumber dotc(IComplexNDArray x, IComplexNDArray y) {
        DataTypeValidation.assertSameDataType(x, y);
        return Nd4j.getExecutioner().execAndReturn(new Dot(x,y)).currentResultComplex();
    }
    ...
    public double nrm2(INDArray x) {
        return Nd4j.getExecutioner().execAndReturn(new Norm2(x)).currentResult().doubleValue();
    }

    @Override
    public double nrm2(IComplexNDArray x) {
        return Nd4j.getExecutioner().execAndReturn(new Norm2(x)).currentResultComplex().absoluteValue().doubleValue();
    ...
    public double asum(INDArray x) {
        return Nd4j.getExecutioner().execAndReturn(new Norm1(x)).currentResultComplex().absoluteValue().doubleValue();
    ...
    public int iamax(INDArray x) {
        return Nd4j.getExecutioner().execAndReturn(new IAMax(x)).currentResultComplex().absoluteValue().intValue();
