...
The plan covers every logical release matrix selectable through the single `build-deploy-cross-platform.yml` GitHub dispatcher, including cross-platform Rust/tokenizers and Java modules, compatibility artifacts, Vulkan/Vulkan-MLIR, Hexagon, TPU and ZLUDA. It schedules 16 platform/toolchain lanes rather than expanding 45 matrix variants into 45 instances. Within a lane, native configurations run serially on one checkout and reuse the Maven repository, downloaded dependencies, prepared toolchains, source tree and a 100 GiB compiler cache. The provisioner waits for the lane's signed-off `status.json` before booting the next host. macOS likewise runs base, compile, MPS and MPS+compile on one EC2 Mac dedicated host.

Release workers use pinned sccache 0.15.0 with a two-level `disk,s3` cache. Object files are published directly to the private release bucket under `deeplearning4j/releases/compiler-cache/v1`, so later VMs and later runs can reuse them without transferring a monolithic cache archive. The stable namespace is intentionally independent of run ID, branch, and commit; sccache's compiler, command, environment, and input hashes provide correctness. The EC2 instance role supplies credentials and is limited to the managed bucket. S3 server-side encryption is requested for every cache write. Cache startup, per-variant statistics, and server shutdown are present in the normal CloudWatch build log. Ordinary instance cleanup preserves the cache; the explicit `stop-everything --purge-storage` operation removes it with the rest of the managed bucket contents.
