...
try {
  ...
  if ($Shard.build.backend -eq 'cuda') {
    ...
    if ($Shard.build.cudaVersion -eq '13.1') {
      $CudaInstaller = Join-Path $WorkRoot 'cuda_13.1.2_windows_network.exe'
      ...
      $CudnnZip = Join-Path $WorkRoot "cudnn-$CudnnVersion-cuda13.zip"
      $CudnnDir = Join-Path $WorkRoot "cudnn-$CudnnVersion-cuda13"
