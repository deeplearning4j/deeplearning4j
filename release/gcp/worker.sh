...
# Compute Engine startup scripts are root/non-login shells. Establish the same
# basics explicitly before strict-mode expansion.
...
OBJECT_PREFIX="${ARTIFACT_PREFIX}/${RUN_ID}/${SHARD_ID}"

upload_if_present() {
  if [ -e "$1" ]; then
    python3 "${CLOUD_IO}" upload --bucket "${BUCKET}" --object "${OBJECT_PREFIX}/$2" --file "$1" || true
  fi
}

start_log_forwarder() {
  [ -z "${LOG_FORWARDER_PID}" ] || return 0
  phase cloud-logging-forwarder started
  python3 "${CLOUD_IO}" forward --project "${PROJECT}" --file "${BUILD_LOG}" \
    --stop-file "${LOG_FORWARDER_STOP}" --log-id "${LOG_ID}" --run-id "${RUN_ID}" --shard "${SHARD_ID}" \
    >"${LOG_FORWARDER_ERROR}" 2>&1 &
  LOG_FORWARDER_PID=$!
  phase cloud-logging-forwarder complete "logId=${LOG_ID} pid=${LOG_FORWARDER_PID}"
}
...
finish() {
  ...
  [ -s "${LOG_FORWARDER_ERROR}" ] && { printf '\nCloud Logging forwarder diagnostics:\n'; sed 's/^/[cloud-logging] /' "${LOG_FORWARDER_ERROR}"; }
...
watch_kill_switch() {
  while true; do
    ...
    printf 'Global release kill switch %s; stopping %s.\n' "${reason}" "${SHARD_ID}"
