...
- Compatibility lanes run concurrently; each VM drains a serial queue of matching shards while preserving its toolchains, Git objects, compiler cache, and durable per-shard Maven working trees; workers publish every Maven file to one unified remote Maven repository.
- Workers produce SDK archives, direct Maven-publish accounting, status documents, and checksummed shard manifests.
...
Every release VM uses pinned sccache 0.17.0 with a two-level local-disk and Azure Blob cache. Compiler objects are published directly under `deeplearning4j/releases/compiler-cache/v1` in the artifact container, making them reusable by later Azure VMs and runs. The first successful execution of each immutable shard/toolchain contract also publishes its populated local working set as a verified `sccache-l0` archive under `deeplearning4j/releases/toolchain-cache/v1`; later VMs download and attest that archive once during bootstrap, before the sccache server starts, and use the ordinary Azure cache only for keys absent from the snapshot. A restored snapshot is never recompressed or uploaded again. The `releases` container exposes anonymous blob read for Maven/browser consumption but does not allow anonymous listing or writes; the controller and control container remain private. The namespace is stable across branches, commits, and run IDs; sccache still hashes the compiler, command, environment, and inputs, so the prefetched working set does not bypass compiler-cache validation. The controller signs a read/write/create-only container SAS that expires with the run and places only that short-lived SAS connection string in the worker payload—the storage account key never leaves the controller. Cache prefetch timing, startup, per-variant statistics, publication timing, and shutdown flow through the same live Blob logs and `build-benchmark.json` as the build. Normal VM cleanup preserves the caches; `stop-everything --purge-storage` removes them because they are inside the managed artifact prefix.

Pinned worker dependencies use separate verified archives under `deeplearning4j/releases/toolchain-cache/v1`. Protobuf, protoc, CMake, Rust/cbindgen, Android NDK installations, Docker base images, sccache itself, and the Linux ROCm build SDK each have an independent identity derived from their complete version/platform/input contract. The worker uploads the content-addressed archive before its identity index, verifies compressed and expanded attestations on restore, rejects unsafe archive members, and atomically exposes a restored directory. Missing indexes are cache misses; authentication, transport, corruption, and incomplete-archive failures stop the build instead of silently rebuilding against an untrusted state. Existing managed host/target dependency snapshots continue to cover large libnd4j-built trees such as LLVM/MLIR, SLEEF, FlatBuffers, and OpenVINO.
...
`mirror configure` provisions one or more independent regional StorageV2 accounts and mirrors the complete block-blob container. On every run it reconciles configured priority prefixes with bounded Azure service-to-service requests, so artifact bytes never pass through the controller machine. A destination blob whose recorded immutable source version, Content-MD5, or release-owned `Dl4J_Sha256` digest is already current is skipped. The stable Maven repository is the default priority prefix; repeat `--priority-prefix PREFIX` to make other paths immediately available without narrowing the final mirror scope. Block blobs up to 32 MiB use Azure's synchronous whole-blob copy. Larger blobs use Azure's native asynchronous `Copy Blob` operation and are polled to completion; the controller does not split or relay their contents. Source ETags fence each copy, and Blob Storage preserves the copied blob's properties and metadata while the controller reapplies any index tags after asynchronous completion. The command then enables source change feed plus source/destination versioning and installs Azure Object Replication with the all-time creation cutoff. Azure therefore transfers every existing BlockBlob in the container and continuously replicates future versions without requiring the controller to enumerate an arbitrarily large compiler cache. The destination container is service-enforced read-only while the policy remains active. To refresh an existing priority prefix immediately, the command removes both halves of that mirror policy, waits for the data-plane lock to clear, reconciles changed objects, and recreates the all-time policy in a `finally` path even when copying fails. Azure Object Replication does not support append or page blobs, so build logs remain authoritative in the source region; Maven artifacts, browse indexes, dependency snapshots, and compiler-cache objects are block blobs and are covered.
...
Use `--skip-priority-bootstrap` only when immediate priority-prefix freshness is unnecessary. This does not narrow the mirror: the all-time Object Replication rule still copies every existing and future BlockBlob, but reconciliation is left to Azure's asynchronous replication schedule.

The command is idempotent: a completed destination policy is reused, and priority-prefix reconciliation skips objects already copied from the current source version. Inspect policy IDs, versioning, the stable Maven marker, replication state, and regional URLs with:
...
A successful `start` or `resume` automatically finalizes Maven accounting after VM and identity cleanup. The usable Maven 2 repository is created once and retained with normal Maven coordinates at:

```text
deeplearning4j/releases/maven-repository/
```

Each worker uploads every primary Maven file first and only then its MD5/SHA-1/SHA-256/SHA-512 sidecars directly to the stable repository over Azure's internal network. Incremental regional mirroring preserves the same publication phases—primaries, primary checksums, Maven metadata, metadata checksums, then the completion marker—while retaining parallel service-to-service copies within each phase. Large JARs never download to the controller. The worker emits hash/size accounting plus its small generated metadata documents; after VM cleanup, the controller verifies the Blob attestations, merges artifact-level and snapshot version-level `maven-metadata.xml`, reconciles snapshot entries against the physically retained Blob filenames so incomplete older metadata cannot hide classifiers, refreshes affected browse indexes, and writes `.dl4j/complete.json` last. Snapshot metadata uses Maven's `localCopy` form so resolved filenames retain the clean `VERSION-SNAPSHOT` spelling instead of publication-time names. The publisher writes `index.html` plus a trailing-slash alias at the repository root and every coordinate directory because Blob Storage does not synthesize directory listings or route `/path/` to `index.html`. Each marker records retained files, files published by the run, exact new/overwritten blob paths, and any `failedRunExecutions` from a partially selected run. Legacy per-run tarballs remain supported strictly as migration inputs; new Linux and Windows runs do not create or synchronize Maven repository archives.
...
Initial `mirror configure` reconciles the full current container through Azure service-to-service copy, then enables replication only for changes from the start of that reconciliation; it never replays historical blob versions over the verified mirror. For a targeted repair of an existing mirror, repeatable `--priority-blob` values perform exact-name copies without listing or copying the rest of the repository. Add `--force-priority-copy` when a destination must receive a fresh source version even though the payload digest already matches; this is intended for small metadata/checksum objects, not large unchanged artifacts.
