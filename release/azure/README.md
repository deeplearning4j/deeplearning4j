...
Repository files are uploaded as individual blobs with stable Maven filenames: classifier JARs, their POMs, standard artifact-level `maven-metadata.xml`, snapshot version-level `maven-metadata.xml`, and MD5/SHA-1/SHA-256/SHA-512 sidecars. Snapshot metadata uses Maven's `localCopy` form so the resolved filenames retain the clean `VERSION-SNAPSHOT` spelling instead of publication-time names. The publisher writes `index.html` plus a trailing-slash alias at the repository root and every coordinate directory because Blob Storage does not synthesize directory listings or route `/path/` to `index.html`. `.dl4j/repository-manifest.json` and its SHA-256 file follow the Maven tree, and `.dl4j/complete.json` is written last. Each marker records retained files, files published by the run, exact new/overwritten blob paths, and any `failedRunExecutions` from a partially selected run. Legacy per-run tarballs are migration inputs only; they are not Maven repositories and are not required for new runs.
