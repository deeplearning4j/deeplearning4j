 ...
 */
    ...
    int computeElementWiseStride(int rank, int *shape, int *stride, int isFOrder) {
        int oldnd;
        printf("Creating copy of shape\n");
        int *olddims = shape::copyOf(rank, shape);
        printf("Created copy of shape\n");
        printf("Creating copy of strides\n");
        int *oldstrides = shape::copyOf(rank, stride);
        printf("Created copy of strides\n");
        int np, op, last_stride;
        int oi, oj, ok, ni, nj, nk;
        printf("Creating new strides of rank %d\n",rank);
        int *newStrides = (int *) malloc(sizeof(int) * rank);
        printf("Created new strides\n");
        oldnd = 0;
        //set the shape to be 1 x length
        int newShapeRank = 2;
        printf("Creating new shape rank %d\n",newShapeRank);
        int *newShape = (int *) malloc(sizeof(int) * newShapeRank);
        printf("Created new shape of rank %d\n",rank);
        newShape[0] = 1;
        printf("About to compute new shape length with rank %d\n",rank);
        newShape[1] = shape::prod(shape, rank);
        printf("Initialized new shape\n");
        /*
