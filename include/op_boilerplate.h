...
#define DISPATCH_METAOP(NAME, SIGNATURE, LIST_A, LIST_B) if(false){} EVAL(_EXEC_META(_EXPAND_PACKED_META_CALL, NAME, (SIGNATURE), (LIST_A), LIST_B)) else{ printf("[ERROR] Unknown opNum=%d on %s:%d", opNum, __FILE__, __LINE__); }
#define DISPATCH_METAOP_INTERNAL(NAME, SIGNATURE, OPNUM_PAIR_B, ...) EVAL(_EXEC_META_E(_EXPAND_PACKED_META_CALL, NAME, SIGNATURE, OPNUM_PAIR_B, __VA_ARGS__))
...
#define ALPHA(A, B, C, D) EVAL(DISPATCH_METAOP_INTERNAL(A, B, D, UNPAREN(C)))
