...
#pragma unroll
			for(i = tid; i < resultLength; i++) {
				printf("TID %d initializing with values %f and %f\n",tid,dx[i],dy[i]);
				sPartials[tid] = opAtomic(dx[i], dy[i],&extraParamsVals);
				__syncthreads();
				for(j = 1; j < elementsPerReductionIndex; j++) {
					sPartials[tid] =  update(sPartials[tid],this->opAtomic(dx[i + tadElementWiseStride * j],dy[i + tadElementWiseStride * j], &extraParamsVals), &extraParamsVals);
					__syncthreads();
					printf("TID %d update with values %f and %f\n",tid,dx[i + tadElementWiseStride * j],dy[i + tadElementWiseStride * j]);

				}


				result[i] = postProcess(sPartials[tid],tadLength,&extraParamsVals);
			}
