...
public class DQNFactoryStdConv implements DQNFactory {
    ...
    public DQN buildDQN(int shapeInputs[], int numOutputs) {
        ...
        confB.layer(1, new ConvolutionLayer.Builder(4, 4).nOut(32).stride(2, 2).activation("relu").build());

        confB.layer(2, new DenseLayer.Builder().nOut(256).activation("relu").build());

        confB.layer(3, new OutputLayer.Builder(LossFunctions.LossFunction.MSE).activation("identity").nOut(numOutputs)
                        .build());
