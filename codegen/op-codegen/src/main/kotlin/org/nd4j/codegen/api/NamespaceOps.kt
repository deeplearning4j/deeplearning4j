...
data class NamespaceOps @JvmOverloads constructor(
     ...
     */
    fun checkInvariants() {
        var usedConfigs = mutableSetOf<Config>()
        ops.forEach { op ->
            usedConfigs.addAll(op.configs)
        }
        var unusedConfigs = configs.toSet() - usedConfigs
    ...
    fun op(name:String):Op {
        var op = ops.find { op -> op.opName.equals(name) } ?: throw java.lang.IllegalStateException("Operation $name not found")
        return op
    }
}