...
var broadcastingDoc = Mixin("broadcastingDoc"){
...
var transformCustom2 = Mixin("transformCustom2"){
...
var reduceVariableDimensions = Mixin("reduceVariable") {
    Input(DataType.NUMERIC, "in") { description = "Input variable" }
    Arg(DataType.BOOL,"keepDims"){ description = "Whether to keep the original  dimensions or produce a shrunk array with less dimensions"; defaultValue = false}
...
var reduceFloating = Mixin("reduceFloating"){
    useMixin(reduce)
    legacy = true
    javaPackage = "org.nd4j.linalg.api.ops.impl.reduce.floating"
}

var reduceFloatingVariable = Mixin("reduceFloatingVariable"){
...
var reduceSame = Mixin("reduceSame"){
    useMixin(reduce)
    legacy = true
    javaPackage = "org.nd4j.linalg.api.ops.impl.reduce.same"
}

var reduceSameVariable = Mixin("reduceSameVariable"){
    useMixin(reduceVariableDimensions)
    legacy = true
    javaPackage = "org.nd4j.linalg.api.ops.impl.reduce.same"
}

var reduceLong = Mixin("reduceLong"){
    useMixin(reduce)
    legacy = true
    javaPackage = "org.nd4j.linalg.api.ops.impl.reduce.longer"
}

var reduceLongVariable = Mixin("reduceLongVariable"){
    useMixin(reduceVariableDimensions)
    javaPackage = "org.nd4j.linalg.api.ops.impl.reduce.longer"
}

var reduce3 = Mixin("reduce3"){
    legacy = true
    ...
    var dims = Arg(DataType.INT, "dimensions"){ count = AtLeast(0);  isVargarg = true; description = "Dimensions to reduce over. If dimensions are not specified, full array reduction is performed" }
    Output(DataType.NUMERIC, "output"){ description = "Output variable" }
}

var reduce3Variable = Mixin("reduce3Variable"){
    legacy = true
    javaPackage = "org.nd4j.linalg.api.ops.impl.reduce3"
    Input(DataType.NUMERIC, "x") { description = "Input variable x" }
    Input(DataType.NUMERIC, "y") { description = "Input variable y" }
...
var indexAccum = Mixin("indexAccum"){
    ...
    var input = Input(DataType.NUMERIC, "in") { description = "Input variable" }
    var keepDims = Arg(DataType.BOOL, "keepDims") { description = "If true: keep the dimensions that are reduced on (as length 1). False: remove the reduction dimensions"; defaultValue = false }
    var dims = Arg(DataType.INT, "dimensions"){ count = AtLeast(1); isVargarg = true; description = "Dimensions to reduce over. If dimensions are not specified, full array reduction is performed" }
    Output(DataType.NUMERIC, "output"){ description = "Reduced array of rank (input rank - num dimensions)" }
...
var indexAccumVariable = Mixin("indexAccumVariable"){
    legacy = true
    javaPackage = "org.nd4j.linalg.api.ops.impl.indexaccum"
    var input = Input(DataType.NUMERIC, "in") { description = "Input variable" }
    var keepDims = Arg(DataType.BOOL, "keepDims") { description = "If true: keep the dimensions that are reduced on (as length 1). False: remove the reduction dimensions"; defaultValue = false }
    var dims = Input(DataType.NUMERIC, "dimensions"){  description = "Dimensions to reduce over. If dimensions are not specified, full array reduction is performed" }
    Output(DataType.NUMERIC, "output"){ description = "Reduced array of rank (input rank - num dimensions)" }
...
var indexAccumCustom = Mixin("indexAccumCustom"){
...
var indexAccumVariableCustom = Mixin("indexAccumVariableCustom"){
    javaPackage = "org.nd4j.linalg.api.ops.impl.indexaccum.custom"
    var input = Input(DataType.NUMERIC, "in") { description = "Input variable" }
    var keepDims = Arg(DataType.BOOL, "keepDims") { description = "If true: keep the dimensions that are reduced on (as length 1). False: remove the reduction dimensions"; defaultValue = false }
    var dims = Input(DataType.NUMERIC, "dimensions"){  description = "Dimensions to reduce over. If dimensions are not specified, full array reduction is performed" }
