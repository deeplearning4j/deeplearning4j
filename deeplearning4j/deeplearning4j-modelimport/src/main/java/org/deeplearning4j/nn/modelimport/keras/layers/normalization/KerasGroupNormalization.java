...
import org.deeplearning4j.nn.conf.layers.GroupNormalization;
...
/**
 * Imports a Keras GroupNormalization layer.
 *
 * GroupNormalization divides channels into groups and normalizes within each group.
 * It's an alternative to BatchNormalization that works well with small batch sizes.
 *
 * Keras config fields:
 * - groups: Number of groups for GroupNormalization (default: 32)
...
public class KerasGroupNormalization extends KerasLayer {
     ...
     */
    public KerasGroupNormalization(Map<String, Object> layerConfig)
     ...
     * @throws UnsupportedKerasConfigurationException Unsupported Keras config
     */
    public KerasGroupNormalization(Map<String, Object> layerConfig, boolean enforceTrainingConfig)
            throws InvalidKerasConfigurationException, UnsupportedKerasConfigurationException {
        super(layerConfig, enforceTrainingConfig);

        Map<String, Object> innerConfig = KerasLayerUtils.getInnerLayerConfigFromConfig(layerConfig, conf);

        // Parse groups (default 32)
        Object groupsObj = innerConfig.get("groups");
        this.groups = groupsObj != null ? ((Number) groupsObj).intValue() : 32;
        ...
        // Build GroupNormalization layer
        GroupNormalization.Builder builder = new GroupNormalization.Builder()
                .name(this.layerName)
                .groups(groups)
                .epsilon(epsilon)
                .center(center)
    ...
    /**
     * Get DL4J GroupNormalization layer.
     *
     * @return GroupNormalization
     */
    public GroupNormalization getGroupNormalizationLayer() {
        return (GroupNormalization) this.layer;
    }
    ...
    public InputType getOutputType(InputType... inputType) throws InvalidKerasConfigurationException {
        if (inputType.length > 1)
            throw new InvalidKerasConfigurationException(
                    "Keras GroupNormalization layer accepts only one input (received " + inputType.length + ")");
        return inputType[0];
    }
