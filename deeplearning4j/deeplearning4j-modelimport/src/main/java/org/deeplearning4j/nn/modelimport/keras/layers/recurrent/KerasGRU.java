...
/**
 * Imports a Keras GRU layer as a DL4J SimpleRnn layer.
...
public class KerasGRU extends KerasLayer {
     ...
     */
    public KerasGRU(Integer kerasVersion) throws UnsupportedKerasConfigurationException {
        super(kerasVersion);
    }
     ...
     * @throws UnsupportedKerasConfigurationException Unsupported Keras config
     */
    public KerasGRU(Map<String, Object> layerConfig)
     ...
     * @throws UnsupportedKerasConfigurationException Unsupported Keras config
     */
    public KerasGRU(Map<String, Object> layerConfig, Map<String, ? extends KerasLayer> previousLayers)
            throws InvalidKerasConfigurationException, UnsupportedKerasConfigurationException {
     ...
     * @throws UnsupportedKerasConfigurationException Unsupported Keras config
     */
    public KerasGRU(Map<String, Object> layerConfig, boolean enforceTrainingConfig)
            throws InvalidKerasConfigurationException, UnsupportedKerasConfigurationException {
    ...
    public KerasGRU(Map<String, Object> layerConfig, boolean enforceTrainingConfig,
        ...
        log.warn("Keras GRU layer is being imported using SimpleRnn as DL4J does not have a native GRU implementation. " +
                "GRU-specific gating mechanisms are not fully replicated.");
        ...
        this.returnSequences = (Boolean) innerConfig.getOrDefault(conf.getLAYER_FIELD_RETURN_SEQUENCES(), false);

        // GRU specific config
        if (innerConfig.containsKey("reset_after")) {
            this.resetAfter = (Boolean) innerConfig.get("reset_after");
        }
        ...
        // Using SimpleRnn as a fallback since DL4J doesn't have native GRU
    ...
    /**
     * Get DL4J layer (SimpleRnn used as GRU fallback).
     *
     * @return Layer
     */
    public Layer getGRULayer() {
        return this.layer;
    }
    ...
    public InputType getOutputType(InputType... inputType) throws InvalidKerasConfigurationException {
        if (inputType.length > 1)
            throw new InvalidKerasConfigurationException(
                    "Keras GRU layer accepts only one input (received " + inputType.length + ")");
        InputPreProcessor preProcessor = getInputPreprocessor(inputType);
        if (preProcessor != null)
            return preProcessor.getOutputType(inputType[0]);
        else
            return this.getGRULayer().getOutputType(-1, inputType[0]);
    }
    ...
    public void setWeights(Map<String, INDArray> weights) throws InvalidKerasConfigurationException {
        ...
        if (weights.containsKey(conf.getKERAS_PARAM_NAME_W()))
            ...
            throw new InvalidKerasConfigurationException(
                    "Keras GRU layer does not contain parameter " + conf.getKERAS_PARAM_NAME_W());
        ...
        if (weights.containsKey(conf.getKERAS_PARAM_NAME_RW()))
            ...
            throw new InvalidKerasConfigurationException(
                    "Keras GRU layer does not contain parameter " + conf.getKERAS_PARAM_NAME_RW());
