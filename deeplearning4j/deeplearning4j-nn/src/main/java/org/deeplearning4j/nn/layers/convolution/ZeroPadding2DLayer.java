 ...
 * @author Alex Black
 */
public class ZeroPadding2DLayer extends AbstractLayer<org.deeplearning4j.nn.conf.layers.convolutional.ZeroPadding2DLayer> {

    private int[] padding; //[padTop, padBottom, padLeft, padRight]

    public ZeroPadding2DLayer(NeuralNetConfiguration conf) {
        super(conf);
        this.padding = ((org.deeplearning4j.nn.conf.layers.convolutional.ZeroPadding2DLayer) conf.getLayer()).getPadding();
    }
    ...
    @Override
    public Layer clone() {
        return new ZeroPadding2DLayer(conf.clone());
    }
