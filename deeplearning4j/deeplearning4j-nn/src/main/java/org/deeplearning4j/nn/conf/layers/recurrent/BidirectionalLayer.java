...
/**
 * BidirectionalLayer is a "wrapper" layer: it wraps any uni-directional RNN layer to make it bidirectional.<br> Note that
...
public class BidirectionalLayer extends Layer {
    ...
    private BidirectionalLayer(BidirectionalLayer.Builder builder) {
        super(builder);
    }

    /**
     * Create a BidirectionalLayer wrapper, with the default Mode (CONCAT) for the specified layer
     *
     * @param layer layer to wrap
     */
    public BidirectionalLayer(@NonNull Layer layer) {
        this(Mode.CONCAT, layer);
    }

    /**
     * Create a BidirectionalLayer wrapper for the specified layer
     *
     * @param mode Mode to use to combine activations. See {@link Mode} for details
     * @param layer layer to wrap
     */
    public BidirectionalLayer(@NonNull Mode mode, @NonNull Layer layer) {
        if (!(layer instanceof BaseRecurrentLayer || layer instanceof LastTimeStepLayer
                        || layer instanceof BaseWrapperLayer)) {
            throw new IllegalArgumentException("Cannot wrap a non-recurrent layer: "
                            + "config must extend BaseRecurrentLayer or LastTimeStepLayer " + "Got class: "
                            + layer.getClass());
        }
        this.fwd = layer;
    ...
    public long getNOut() {
        if (this.fwd instanceof LastTimeStepLayer) {
            return ((FeedForwardLayer) ((LastTimeStepLayer) this.fwd).getUnderlying()).getNOut();
        } else {
    ...
    public long getNIn() {
        if (this.fwd instanceof LastTimeStepLayer) {
            return ((FeedForwardLayer) ((LastTimeStepLayer) this.fwd).getUnderlying()).getNIn();
        } else {
    ...
    public org.deeplearning4j.nn.api.Layer instantiate(NeuralNetConfiguration conf,
        ...
        org.deeplearning4j.nn.layers.recurrent.BidirectionalLayer ret = new org.deeplearning4j.nn.layers.recurrent.BidirectionalLayer(conf, f, b, layerParamsView);
    ...
    public InputType getOutputType(int layerIndex, InputType inputType) {
        ...
        if (fwd instanceof LastTimeStepLayer) {
    ...
    @Getter
    @Setter
    public static class Builder extends Layer.Builder<BidirectionalLayer.Builder> {
        ...
        public BidirectionalLayer build() {
            return new BidirectionalLayer(this);
        }
    }
}
