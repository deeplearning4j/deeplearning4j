...
public class LocalResponseNormalizationLayer
                extends AbstractLayer<org.deeplearning4j.nn.conf.layers.normalization.LocalResponseNormalizationLayer> {
    protected static final Logger log =
                    LoggerFactory.getLogger(
                            org.deeplearning4j.nn.conf.layers.normalization.LocalResponseNormalizationLayer.class);

    protected LocalResponseNormalizationHelper helper = null;
    protected int helperCountFail = 0;

    public LocalResponseNormalizationLayer(NeuralNetConfiguration conf, INDArray input) {
        super(conf, input);
        initializeHelper();
    }
    ...
    @Override
    public Layer clone() {
        return new LocalResponseNormalizationLayer(conf.clone());
    }

    public LocalResponseNormalizationLayer(NeuralNetConfiguration conf) {
        super(conf);
        initializeHelper();
    }
        ...
        if (helper != null && (helperCountFail == 0 || !layerConf().isCudnnAllowFallback())){
            ...
            try {
                ...
                if(layerConf().isCudnnAllowFallback()){
                    helperCountFail++;
                    log.warn("CuDNN LocalResponseNormalizationLayer backprop execution failed - falling back on built-in implementation",t);
                } else {
                    throw new RuntimeException("Error during LocalResponseNormalizationLayer CuDNN helper backprop - isCudnnAllowFallback() is set to false", t);
                }
            }
            if (ret != null) {
                return ret;
