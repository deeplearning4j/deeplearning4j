 ...
 */
@Slf4j
public class SpaceToDepthLayer extends AbstractLayer<org.deeplearning4j.nn.conf.layers.convolutional.SpaceToDepthLayer> {

    public SpaceToDepthLayer(NeuralNetConfiguration conf) {
        super(conf);
    }

    public SpaceToDepthLayer(NeuralNetConfiguration conf, INDArray input) {
        super(conf, input);
    }
    ...
    private int isNHWC() {return layerConf().getDataFormat().equals(
            org.deeplearning4j.nn.conf.layers.convolutional.SpaceToDepthLayer.DataFormat.NHWC)? 1: 0;}
