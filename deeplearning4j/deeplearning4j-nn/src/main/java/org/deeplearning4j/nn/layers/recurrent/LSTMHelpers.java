...
public class LSTMHelpers {
    ...
    static public FwdPassReturn activateHelper(final BaseRecurrentLayer layer, final NeuralNetConfiguration conf,
        ...
        if (originalPrevMemCellState == null) {
            prevMemCellState = workspaceMgr.create(ArrayType.FF_WORKING_MEM, inputWeights.dataType(), new long[]{miniBatchSize, hiddenLayerSize}, 'f');
        } else {
            prevMemCellState = workspaceMgr.dup(ArrayType.FF_WORKING_MEM, originalPrevMemCellState,'f');
        }
