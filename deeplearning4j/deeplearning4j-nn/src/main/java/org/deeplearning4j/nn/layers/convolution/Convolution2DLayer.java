 ...
 */
public class Convolution2DLayer extends BaseLayer<org.deeplearning4j.nn.conf.layers.convolutional.Convolution2DLayer> {
    protected static final Logger log = LoggerFactory.getLogger(Convolution2DLayer.class);
    ...
    public Convolution2DLayer(NeuralNetConfiguration conf) {
        super(conf);
        initializeHelper();
        convolutionMode = ((org.deeplearning4j.nn.conf.layers.convolutional.Convolution2DLayer) conf().getLayer()).getConvolutionMode();
    }

    public Convolution2DLayer(NeuralNetConfiguration conf, INDArray input) {
        super(conf, input);
        initializeHelper();
    }
    ...
    /**
     * preOutput4d: Used so that Convolution2DLayer subclasses (such as Convolution1DLayer) can maintain their standard
    ...
    protected void validateInputRank() {
        ...
        if (input.rank() != 4) {
            ...
            throw new DL4JInvalidInputException("Got rank " + input.rank()
                    + " array as input to Convolution2DLayer (layer name = " + layerName + ", layer index = "
