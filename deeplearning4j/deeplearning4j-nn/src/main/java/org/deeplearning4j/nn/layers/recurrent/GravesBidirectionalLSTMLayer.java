 ...
 * READ THIS FIRST
 *
 * Bdirectional LSTMLayer layer implementation.
 ...
 *
 * A high level description of bidirectional LSTMLayer can be found from
 * "Hybrid Speech Recognition with Deep BidirectionalLayer LSTMLayer"
 ...
 */
@Slf4j
public class GravesBidirectionalLSTMLayer
                extends BaseRecurrentLayer<org.deeplearning4j.nn.conf.layers.recurrent.GravesBidirectionalLSTMLayer> {

    protected FwdPassReturn cachedPassForward;
    protected FwdPassReturn cachedPassBackward;

    public GravesBidirectionalLSTMLayer(NeuralNetConfiguration conf) {
        super(conf);
    }

    public GravesBidirectionalLSTMLayer(NeuralNetConfiguration conf, INDArray input) {
        super(conf, input);
    }
    ...
    public Pair<INDArray, MaskState> feedForwardMaskArray(INDArray maskArray, MaskState currentMaskState,
                    int minibatchSize) {
        //BidirectionalLayer RNNs operate differently to standard RNNs from a masking perspective
        //Specifically, the masks are applied regardless of the mask state
        //For example, input -> RNN -> BidirectionalLayer-RNN: we should still mask the activations and errors in the bi-RNN
