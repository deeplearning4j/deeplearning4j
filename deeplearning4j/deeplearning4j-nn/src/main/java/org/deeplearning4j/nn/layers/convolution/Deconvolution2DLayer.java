    ...
    protected Pair<INDArray, INDArray> preOutput(boolean training , boolean forBackprop, LayerWorkspaceMgr workspaceMgr) {
        ...
        if (input.rank() != 4) {
            ...
            if (layerName == null)
                layerName = "(not named)";
            throw new DL4JInvalidInputException("Got rank " + input.rank()
                    + " array as input to Deconvolution2DLayer (layer name = " + layerName + ", layer index = "
        ...
        if (input.size(1) != inDepth && input.size(3) == inDepth) {
            ...
            if (layerName == null)
                layerName = "(not named)";
            throw new DL4JInvalidInputException("Cannot do forward pass in Deconvolution2DLayer layer (layer name = " + layerName
