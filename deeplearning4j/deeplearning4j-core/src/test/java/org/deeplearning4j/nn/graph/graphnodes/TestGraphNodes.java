...
public class TestGraphNodes {
    ...
    @Test
    public void testDotProductNode1D() {
        Nd4j.getRandom().setSeed(12345);
        ...
        assertEquals(18.0000, out.getDouble(0), 1e-5);
        ...
        assertArrayEquals(new int[]{54, 54, 54}, p.getSecond()[0].toIntVector());
        assertArrayEquals(new int[]{36, 36, 36}, p.getSecond()[1].toIntVector());
    }

    @Test
    public void testDotProductNode2D() {
        Nd4j.getRandom().setSeed(12345);
    ...
    @Test
    public void testDotProductNode4D() {
        Nd4j.getRandom().setSeed(12345);

        int[] dim1 = new int[]{1,2,3};
        GraphVertex dotProduct = new DotProductVertex(null, "test_vertex", -1, dim1);

        float[] data1 = new float[36], data2 = new float[36];
        for (int i = 0; i < 36; ++i) {
            data1[i] = 2;
            data2[i] = 3;
        }
        INDArray in1 = Nd4j.createFromArray(data1).reshape(new int[]{3,2,3,2});
        INDArray in2 = Nd4j.createFromArray(data2).reshape(new int[]{3,2,3,2});
