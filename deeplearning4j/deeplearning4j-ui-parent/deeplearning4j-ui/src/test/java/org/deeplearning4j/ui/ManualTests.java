...
public class ManualTests {
    ...
    public void testCNNActivationsVisualization() throws Exception {
        ...
        MultiLayerConfiguration.Builder builder = new NeuralNetConfiguration.Builder().seed(seed)
                        ...
                        .layer(0, new Convolution2DLayer.Builder(4, 4).name("cnn1").nIn(nChannels).stride(1, 1).nOut(20)
                                        .build())
                        .layer(1, new Subsampling2DLayer.Builder(Subsampling2DLayer.PoolingType.MAX, new int[] {2, 2})
                                        .name("pool1").build())
                        .layer(2, new Convolution2DLayer.Builder(3, 3).name("cnn2").stride(1, 1).nOut(40).build())
                        .layer(3, new Subsampling2DLayer.Builder(Subsampling2DLayer.PoolingType.MAX, new int[] {2, 2})
                                        .name("pool2").build())
                        .layer(4, new Convolution2DLayer.Builder(3, 3).name("cnn3").stride(1, 1).nOut(60).build())
                        .layer(5, new Subsampling2DLayer.Builder(Subsampling2DLayer.PoolingType.MAX, new int[] {2, 2})
                                        .name("pool3").build())
                        .layer(6, new Convolution2DLayer.Builder(2, 2).name("cnn3").stride(1, 1).nOut(80).build())
    ...
    public void testCNNActivationsFrozen() throws Exception {
        ...
        MultiLayerConfiguration.Builder builder = new NeuralNetConfiguration.Builder().seed(seed)
                ...
                .weightInit(WeightInit.XAVIER)
                .updater(new Nesterovs(0.01, 0.9)).list()
                .layer(0, new FrozenLayer(new Convolution2DLayer.Builder(5, 5)
                        //nIn and nOut specify depth. nIn here is the nChannels and nOut is the number of filters to be applied
                        .nIn(nChannels).stride(1, 1).nOut(20).activation(Activation.IDENTITY).build()))
                .layer(1, new FrozenLayer(new Subsampling2DLayer.Builder(Subsampling2DLayer.PoolingType.MAX).kernelSize(2, 2)
                        .stride(2, 2).build()))
