...
class TupleStreamDataSetIteratorTest extends SolrCloudTestCase {
            ...
            for (int b = 0; b <= 256; b += increment) {
                ...
                for (int g = 0; g <= 256; g += increment) {
                    ...
                    for (int r = 0; r <= 256; r += increment) {
                        ...
                        final float luminance = (b * 0.0722f + g * 0.7152f + r * 0.2126f) / (255 * 3.0f);
                        final SolrInputDocument doc = sdoc("id", Integer.toString(docIds.get(docIdsIdx++)), "channel_b_f", Float.toString(b / 255f), "channel_g_f", Float.toString(g / 255f), "channel_r_f", Float.toString(r / 255f), "luminance_f", Float.toString(luminance));
    ...
    @DisplayName("Counting Iteration Listener")
    ...
    void iterateTest() throws Exception {
        doIterateTest(true);
        doIterateTest(false);
    }

    private void doIterateTest(boolean withIdKey) throws Exception {
        try (final TupleStreamDataSetIterator tsdsi = new TupleStreamDataSetIterator(123, /* batch */
        (withIdKey ? "greeting" : null), /* idKey */
        new String[] { "pie" }, new String[] { "answer" }, "tuple(greeting=\"hello world\",pie=3.14,answer=42)", null)) {
    ...
    @Test
    @DisplayName("Model Fit Test")
    void modelFitTest() throws Exception {
        ...
        for (int ii = 1; ii <= 5; ++ii) {
            ...
            batch *= 2;
            try (final TupleStreamDataSetIterator tsdsi = new TupleStreamDataSetIterator(batch, "id", /* idKey */
            new String[] { "channel_b_f", "channel_g_f", "channel_r_f" }, new String[] { "luminance_f" }, "search(mySolrCollection," + "q=\"id:*\"," + "fl=\"id,channel_b_f,channel_g_f,channel_r_f,luminance_f\"," + "sort=\"id asc\"," + "qt=\"/export\")", cluster.getZkClient().getZkServerAddress())) {
                model.fit(tsdsi);
            }
