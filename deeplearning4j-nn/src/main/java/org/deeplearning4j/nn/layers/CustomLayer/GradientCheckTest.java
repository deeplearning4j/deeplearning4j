...
public class GradientCheckTest {
    ...
    public void elementWiseMultiplicationLayerTest(){
        ...
        log.info("params before learning: "+netGraph.getLayer(0).paramTable());
...
//        the ides is simple: since we do a element wise multiplication layer (just a scaling), we want the cos sim is mainly decided by the fourth value, if everything runs well, we will get a large weight for the fourth value
        ...
        INDArray features = Nd4j.create(new double[][]{{1,2,3,4},{1,2,3,1},{1,2,3,0},{1,2,3,2}});
        INDArray labels = Nd4j.create(new double[][]{{1,1,1,8},{1,1,1,2},{1,1,1,1},{1,1,1,4}});
        ...
        String msg = "elementWiseMultiplicationLayerTest() - score did not (sufficiently) decrease during learning - activationFn="
                + "Id" + ", lossFn=" + "Cos-sim" + ", outputActivation=" + "Id"
                + ", doLearningFirst=" + "true" + " (before=" + scoreBefore
                + ", scoreAfter=" + scoreAfter + ")";
        assertTrue(msg, scoreAfter < 0.8 * scoreBefore);

//        expectation: large weight for the fourth weight
        log.info("params after learning: "+netGraph.getLayer(0).paramTable());
