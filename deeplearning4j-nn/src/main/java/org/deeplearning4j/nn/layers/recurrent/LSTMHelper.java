...
public interface LSTMHelper {
    ...
    Gradients backpropGradient(final org.deeplearning4j.nn.conf.layers.Layer conf, final IActivation gateActivationFn,
    ...
    FwdPassReturn activate(final Layer layer, final org.deeplearning4j.nn.conf.layers.Layer conf, final IActivation gateActivationFn, //Activation function for the gates - sigmoid or hard sigmoid (must be found in range 0 to 1)
                           final INDArray input, final INDArray recurrentWeights, //Shape: [hiddenLayerSize,4*hiddenLayerSize+3]; order: [wI,wF,wO,wG,wFF,wOO,wGG]
                           final INDArray inputWeights, //Shape: [n^(L-1),4*hiddenLayerSize]; order: [wi,wf,wo,wg]
