...
public class LegacyAsyncMultiDataSetIterator implements MultiDataSetIterator {
    ...
    public LegacyAsyncMultiDataSetIterator(MultiDataSetIterator iterator, int queueLength) {
        this(iterator, queueLength, new LinkedBlockingQueue<MultiDataSet>(queueLength), true);
    }

    public LegacyAsyncMultiDataSetIterator(MultiDataSetIterator iterator, int queueLength, boolean useWorkspace) {
        this(iterator, queueLength, new LinkedBlockingQueue<MultiDataSet>(queueLength), useWorkspace);
    }

    public LegacyAsyncMultiDataSetIterator(MultiDataSetIterator iterator, int queueLength, BlockingQueue<MultiDataSet> queue) {
        this(iterator, queueLength, queue, true);
    }

    public LegacyAsyncMultiDataSetIterator(MultiDataSetIterator iterator, int queueLength, BlockingQueue<MultiDataSet> queue, boolean useWorkspace) {
        if (queueLength <= 0)
