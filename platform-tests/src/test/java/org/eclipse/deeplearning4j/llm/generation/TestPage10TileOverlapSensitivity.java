...
@Slf4j
public class TestPage10TileOverlapSensitivity {
    ...
    @Test
    @DisplayName("Page-10 overlap sensitivity: overlapping tile crops isolate residual partial-text loss")
    public void testPage10TileOverlapSensitivity() throws Exception {
        ensureLoaded();
        ...
        variants.put("OVERLAP_0", buildOverlappedSplit(0));
        variants.put("OVERLAP_16", buildOverlappedSplit(16));
        variants.put("OVERLAP_32", buildOverlappedSplit(32));
        variants.put("OVERLAP_48", buildOverlappedSplit(48));
        ...
        try {
            ...
            GenerationResult baseline = results.get("OVERLAP_0");
            assertTrue(baseline != null && normalize(baseline.getText()).contains("heroes are set apart"),
                    "Baseline overlap=0 path no longer contains the expected paragraph fragment. "
                            + summarize(results));

            boolean anyChanged = false;
            for (Map.Entry<String, GenerationResult> entry : results.entrySet()) {
                if (!"OVERLAP_0".equals(entry.getKey())
            ...
            assertTrue(anyChanged,
                    "Tile-overlap variants did not change the token stream at all. " + summarize(results));
        } finally {
    ...
    private static synchronized void ensureLoaded() throws Exception {
        ...
        loaded = true;
        log.info("Tile-overlap sensitivity inputs ready: resized={}x{} grid={}x{} frames={}",
    ...
    private static ImageTiler.SplitImageResult buildOverlappedSplit(int overlap) {
        int numRows = baseSplitResult.numRows;
        int numCols = baseSplitResult.numCols;
        if (numRows <= 0 || numCols <= 0) {
            return baseSplitResult;
        }
        ...
        for (int r = 0; r < numRows; r++) {
            for (int c = 0; c < numCols; c++) {
                int startX = Math.max(0, c * optimalWidth - overlap);
                int startY = Math.max(0, r * optimalHeight - overlap);
                int endX = Math.min(width, (c + 1) * optimalWidth + overlap);
                int endY = Math.min(height, (r + 1) * optimalHeight + overlap);
        ...
        PreparedFrame globalFrame = prepareFrame(resizedForTiling, TARGET_SIZE);
    ...
    private static PreparedFrame prepareFrame(BufferedImage source, int maxSize) {
        ImageTiler.ResizeResult resized = ImageTiler.resizeToFit(source, maxSize, maxSize);
