...
public class GradientCheckTestsComputationGraph extends BaseDL4JTest {
    ...
    void testGradientCNNL1L2MLN(CNN2DFormat format,Nd4jBackend backend) {
        ...
        for (int i = 0; i < l2vals.length; i++) {
            ...
            if (PRINT_RESULTS) {
                log.info("{}- activationFn={}, lossFn={}, outputActivation={}, doLearningFirst={}", testName, afn, lf, outputActivation, doLearningFirst);
            }
        ...
        for (Activation afn : activFns) {
            for (boolean doLearningFirst : characteristic) {
                for (int i = 0; i < lossFunctions.length; i++) {
                    ...
                    if (PRINT_RESULTS) {
                        log.info("{} - activationFn={}, lossFn={}, outputActivation={}, doLearningFirst={}", name, afn, lf, outputActivation, doLearningFirst);
                    }
                    boolean gradOK = GradientCheckUtil.checkGradients(mln, DEFAULT_EPS, DEFAULT_MAX_REL_ERROR, DEFAULT_MIN_ABS_ERROR, PRINT_RESULTS, RETURN_ON_FIRST_FAILURE, input, labels);
                    assertTrue(gradOK);
                    TestUtils.testModelSerialization(mln);
    ...
    public void testBasicIris() {
        ...
        if (PRINT_RESULTS) {
            log.info("testBasicIris()");
        }

        boolean gradOK = GradientCheckUtil.checkGradients(new GraphConfig().net(graph).inputs(new INDArray[]{input})
    ...
    public void testBasicIrisWithMerging() {
        ...
        if (PRINT_RESULTS) {
            log.info("testBasicIrisWithMerging()");
        }
    ...
    public void testBasicIrisWithElementWiseNode() {
        ...
        for (ElementWiseVertex.Op op : ops) {
            ...
            if (PRINT_RESULTS) {
                log.info("testBasicIrisWithElementWiseVertex(op={})", op);
            }
        ...
        for(boolean firstSmaller : new boolean[]{false, true}) {
            for (ElementWiseVertex.Op op : ops) {
                ...
                for (int mb : new int[]{1, 5}) {
                    ...
                    boolean gradOK = GradientCheckUtil.checkGradients(new GraphConfig().net(graph).inputs(new INDArray[]{in})
                            .labels(new INDArray[]{labels}));
                    assertTrue(gradOK, msg);
                    TestUtils.testModelSerialization(graph);
                }
    ...
    public void testRNNWithMerging() {
        for(RNNFormat format : RNNFormat.values()) {
            ...
            ComputationGraph graph = new ComputationGraph(conf);
            graph.init();
            log.debug("Configuration for {} {}", format, conf);
    ...
    public void testLSTMWithLastTimeStepVertex() {
        ...
        if (PRINT_RESULTS) {
            log.info("testLSTMWithLastTimeStepVertex()");
        ...
        gradOK = GradientCheckUtil.checkGradients(new GraphConfig().net(graph).inputs(new INDArray[]{input})
    ...
    public void testLSTMWithDuplicateToTimeSeries() {
        ...
        if (PRINT_RESULTS) {
            log.info("testLSTMWithDuplicateToTimeSeries()");

        }

        boolean gradOK = GradientCheckUtil.checkGradients(new GraphConfig().net(graph).inputs(new INDArray[]{input1, input2})
                .labels(new INDArray[]{labels}));
    ...
    public void testLSTMWithReverseTimeSeriesVertex() {
        ...
        if (PRINT_RESULTS) {
            log.info("testLSTMWithReverseTimeSeriesVertex()");
    ...
    public void testMultipleInputsLayer() {
        ...
        for (int mb : minibatchSizes) {
            ...
            boolean gradOK = GradientCheckUtil.checkGradients(new GraphConfig().net(graph).inputs(inputs)
    ...
    public void testMultipleOutputsMergeVertex() {
        ...
        for (int mb : minibatchSizes) {
            ...
            boolean gradOK = GradientCheckUtil.checkGradients(new GraphConfig().net(graph).inputs(input)
                    .labels(new INDArray[]{out}));
    ...
    public void testBasicIrisTripletStackingL2Loss() {
        ...
        if (PRINT_RESULTS) {
            log.info("testBasicIrisTripletStackingL2Loss()");

        }

        boolean gradOK = GradientCheckUtil.checkGradients(new GraphConfig().net(graph).inputs(new INDArray[]{pos, anc, neg})
                .labels(new INDArray[]{labels}));
    ...
    public void testBasicCenterLoss() {
        ...
        for (boolean train : trainFirst) {
            for (double lambda : new double[] {0.0, 0.5, 2.0}) {
                ...
                boolean gradOK = GradientCheckUtil.checkGradients(new GraphConfig().net(graph).inputs(new INDArray[]{example})
                        .labels(new INDArray[]{labels}));
    ...
    public void testBasicL2() {
        ...
        for (int minibatch : mbSizes) {
            ...
            boolean gradOK = GradientCheckUtil.checkGradients(new GraphConfig().net(graph).inputs(new INDArray[]{in1, in2})
                    .labels(new INDArray[]{labels}));
    ...
    public void testBasicTwoOutputs() {
        ...
        log.info("Num layers: {}", graph.getNumLayers());
        log.info("Num params: {}", graph.numParams());
    ...
    public void testL2NormalizeVertex2d() {
        ...
        for(long[] definition : definitions) {
            ...
            for (int minibatch : mbSizes) {
                ...
                boolean gradOK = GradientCheckUtil.checkGradients(new GraphConfig().net(graph).inputs(new INDArray[]{in1})
    ...
    public void testGraphEmbeddingLayerSimple() {
        ...
        if (PRINT_RESULTS) {
            log.info("testGraphEmbeddingLayerSimple");
        }

        boolean gradOK = GradientCheckUtil.checkGradients(new GraphConfig().net(cg).inputs(new INDArray[]{input})
                .labels(new INDArray[]{labels}));
