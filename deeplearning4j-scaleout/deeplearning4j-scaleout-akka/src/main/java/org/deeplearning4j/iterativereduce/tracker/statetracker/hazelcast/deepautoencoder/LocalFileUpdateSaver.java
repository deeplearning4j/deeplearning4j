...
public class LocalFileUpdateSaver implements UpdateSaver<UpdateableEncoderImpl> {
    ...
    @Override
    public UpdateableEncoderImpl load(String id) throws Exception {
        String path = paths.remove(id);
        File load = new File(path);
        UpdateableEncoderImpl u =  SerializationUtils.readObject(load);
        load.delete();
        return u;
    }
    ...
    @Override
    public void save(String id,UpdateableEncoderImpl save) throws Exception {
       if(save.get() == null)
