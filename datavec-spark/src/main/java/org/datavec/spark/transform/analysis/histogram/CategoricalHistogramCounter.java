...
public class CategoricalHistogramCounter implements HistogramCounter {
    ...
    public HistogramCounter add(Writable w) {
        ...
        if(counts.containsKey(value)) counts.put(value, counts.get(value)+1);
    ...
    public HistogramCounter merge(HistogramCounter other) {
        if(!(other instanceof CategoricalHistogramCounter)) throw new IllegalArgumentException("Input must be CategoricalHistogramCounter; got " +  other);

        CategoricalHistogramCounter o = (CategoricalHistogramCounter)other;

        for(Map.Entry<String,Integer> entry : o.counts.entrySet()){
            String key = entry.getKey();
            if(counts.containsKey(key)) counts.put(key, counts.get(key)+entry.getValue());
            else counts.put(key, entry.getValue());
        }
