...
public class ClusterSet {
    ...
    return !getPointDistribution().containsKey(point.getId()) ||
        !getPointDistribution().get(point.getId()).equals(newCluster.getId());
  }
	...
	public org.arbiter.berkeley.Pair<Cluster, Double> nearestCluster(Point point) {
		...
		return new org.arbiter.berkeley.Pair<>(nearestCluster, minDistance);
	...
	public List<Cluster> getMostPopulatedClusters(int count) {
		...
		Collections.sort(mostPopulated, new Comparator<Cluster>() {
			public int compare(Cluster o1, Cluster o2) {
				return new Integer(o1.getPoints().size()).compareTo(o2.getPoints().size());
			}
		});
