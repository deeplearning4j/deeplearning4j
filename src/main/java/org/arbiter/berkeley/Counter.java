...
public class Counter<E> implements Serializable {
	...
	org.arbiter.berkeley.MapFactory<E, Double> mf;
	double deflt = 0.0;
	...
	public Counter(boolean identityHashMap) {
		this(identityHashMap ? new org.arbiter.berkeley.MapFactory.IdentityHashMapFactory<E, Double>()
				: new org.arbiter.berkeley.MapFactory.HashMapFactory<E, Double>());
	}

	public Counter(org.arbiter.berkeley.MapFactory<E, Double> mf) {
		this.mf = mf;
		entries = mf.buildMap();
	}
	...
	private void keepKeysHelper(int keepN, boolean top) {
		...
		int n = 0;
		for (E e : org.arbiter.berkeley.Iterators.able(top ? asPriorityQueue() : asMinPriorityQueue())) {
