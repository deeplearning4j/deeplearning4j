...
/*
    ...
    public void testSequenceRecordReaderFunctionCSV() throws Exception {
        ...
        List<Pair<String, InputStream>> origData = sc.binaryFiles(path);
        assertEquals(3, origData.size()); //3 CSV files

        SequenceRecordReaderFunction srrf = new SequenceRecordReaderFunction(new CSVSequenceRecordReader(1, ",")); //CSV, skip 1 line
        List<List<List<Writable>>> rdd = origData.stream().map(input -> srrf.apply(input)).collect(Collectors.toList());
        List<List<List<Writable>>> listSpark = rdd;
    ...
    public void testSequenceRecordReaderFunctionVideo() throws Exception {
        ...
        assertEquals(4, origData.size()); //4 video files
