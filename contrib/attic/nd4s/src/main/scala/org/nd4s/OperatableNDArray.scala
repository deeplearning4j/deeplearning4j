...
trait OperatableNDArray[A <: INDArray] {
  ...
  def +(that: INDArray)(implicit ev: NDArrayEvidence[A, _]): A =
    ev.add(underlying, that)

  def -(that: INDArray)(implicit ev: NDArrayEvidence[A, _]): A =
    ev.sub(underlying, that)

  /** element-by-element multiplication */
  def *(that: INDArray)(implicit ev: NDArrayEvidence[A, _]): A =
    ev.mul(underlying, that)

  /** matrix multiplication */
  def **(that: INDArray)(implicit ev: NDArrayEvidence[A, _]): A =
    ev.mmul(underlying, that)

  /** matrix multiplication using Numpy syntax for arrays */
  def dot(that: INDArray)(implicit ev: NDArrayEvidence[A, _]): A =
    ev.mmul(underlying, that)

  def /(that: INDArray)(implicit ev: NDArrayEvidence[A, _]): A =
    ev.div(underlying, that)

  /** right division ... is this the correct symbol? */
  def \(that: INDArray)(implicit ev: NDArrayEvidence[A, _]): A =
    ev.rdiv(underlying, that)

  // --- In-place INDArray opertors
  /** In-place addition */
  def +=(that: INDArray)(implicit ev: NDArrayEvidence[A, _]): A =
    ev.addi(underlying, that)

  /** In-place subtraction */
  def -=(that: INDArray)(implicit ev: NDArrayEvidence[A, _]): A =
    ev.subi(underlying, that)

  /** In-placeelement-by-element multiplication */
  def *=(that: INDArray)(implicit ev: NDArrayEvidence[A, _]): A =
    ev.muli(underlying, that)

  /** In-place matrix multiplication */
  def **=(that: INDArray)(implicit ev: NDArrayEvidence[A, _]): A =
    ev.mmuli(underlying, that)

  /** In-place division */
  def /=(that: INDArray)(implicit ev: NDArrayEvidence[A, _]): A =
    ev.divi(underlying, that)

  /** In-place right division */
  def \=(that: INDArray)(implicit ev: NDArrayEvidence[A, _]): A =
    ev.rdivi(underlying, that)

  // --- Number operators
  def +(that: Number)(implicit ev: NDArrayEvidence[A, _]): A =
    ev.add(underlying, that)
  ...
  def %=(that: Number)(implicit ev: NDArrayEvidence[A, _]): A =
    ev.remainderi(underlying, that)

  /** right division ... is this the correct symbol? */
  def \=(that: Number)(implicit ev: NDArrayEvidence[A, _]): A =
    ev.rdivi(underlying, that)

  def get[B](i: Int)(implicit ev: NDArrayEvidence[A, B]): B =
    ev.get(underlying, i)

  def get[B](i: Int, j: Int)(implicit ev: NDArrayEvidence[A, B]): B =
  ...
  def apply[B](i: Int, j: Int)(implicit ev: NDArrayEvidence[A, B]): B =
    get(i, j)
  ...
  def normMaxT[B](implicit ev: NDArrayEvidence[A, B]): B =
    ev.normMax(underlying)
  ...
  def stdT[B](implicit ev: NDArrayEvidence[A, B]): B =
    ev.standardDeviation(underlying)
