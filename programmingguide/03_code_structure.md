title: DeepLearning4j: Code Structure
layout: default
...
There are two classes that can be used for building neural networks: MultiLayerNetwork and ComputationGraph. We will provide a brief overview on each of these networks.
...
The MultiLayerConfiguraiton can be used to define the parameter and structure of the neural network. This is one example of a configuration:
...
The writing code to train a neural network is easy once its configuration is defined. Typically, two loops wil be needed: one to iterate through the number of epochs and another to iterate through batches of the training data. Note that the latter loop will implicitly be done behind the scenes with either MultiLayerNetwork or ComptuationGraph. For now, we will assume the training split of the data is already contained in DataSetIterators called trainData. The process of converting data into this format will be convered in the next chapter.
...
Notice that the code to train a neural network is very concise. This is because MultiLayerNetwork and ComputationGraph handles the details and intricacies so we don't have to worry about it. 
...
Once the neural network is fully trained, the remaining task is to evaluate the neural network on the testing split of the data. We assume that the testing split of the data is contained in a DataSetIterator called testData. An example code for doing this is shown below.

    Evaluation eval = model.evaluate(testData);
    log.info(eval.stats());

Note this similar code can be used on the validation test to evaluate the neural network between epochs within the training loop as well.

Specific metrics such as AUC (area under curve) for a ROC (receiving operator characteristic) curve can be evaluated as well if the output is appropriate. An example of this is shown below
...
In the next few chapters, we will see specific examples of this process for convolutional, recurrent, and feedforward networks.