...
//        Bidirectional.Mode[] modes = new Bidirectional.Mode[]{Bidirectional.Mode.CONCAT, Bidirectional.Mode.ADD,
//                Bidirectional.Mode.AVERAGE, Bidirectional.Mode.MUL};
        Bidirectional.Mode[] modes = new Bidirectional.Mode[]{Bidirectional.Mode.ADD,
                Bidirectional.Mode.AVERAGE, Bidirectional.Mode.MUL, Bidirectional.Mode.CONCAT};

        Random r = new Random(12345);
        for( int mb : new int[]{1}) {
            for(boolean inputMask : new boolean[]{false, true}){
                for(boolean simple : new boolean[]{true}) {
                        ...
                        MultiLayerConfiguration conf = new NeuralNetConfiguration.Builder()
                                ...
                                .layer(new Bidirectional(m,
                                        (simple ?
                                                new SimpleRnn.Builder().nIn(3).nOut(3).build() :
                                                new LSTM.Builder().nIn(3).nOut(3).build())))
                                .layer(new RnnOutputLayer.Builder().lossFunction(LossFunctions.LossFunction.MCXENT)
                                        .activation(Activation.SOFTMAX).nOut(nOut)
                                        .nIn(m == Bidirectional.Mode.CONCAT ? 6 : 3)
                                        .build())
                                .build();
        ...
        for( int mb : new int[]{3}) {
            ...
            for (Bidirectional.Mode m : modes) {
                ...
                MultiLayerConfiguration conf = new NeuralNetConfiguration.Builder()
                        ...
                        .list()
                        .layer(new LSTM.Builder().nIn(nIn).nOut(3).build())
                        .layer(new Bidirectional(m, new SimpleRnn.Builder().nIn(3).nOut(3).build()))
                        .layer(new RnnOutputLayer.Builder().lossFunction(LossFunctions.LossFunction.MCXENT)
                                .activation(Activation.SOFTMAX).nOut(nOut)
                                .nIn(m == Bidirectional.Mode.CONCAT ? 6 : 3)
                                .build())
        ...
        for( int mb : new int[]{1, 3}) {
            for(boolean inputMask : new boolean[]{false, true}){
                for(int l=0; l<l1s.length; l++ ) {
                    ...
                    String name = "testSimpleRnn() - mb=" + mb + ", maskType=" + maskType + ", l1=" + l1s[l]
                            + ", l2=" + l2s[l];
