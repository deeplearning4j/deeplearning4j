...
public class RnnGradientChecks {
    ...
    public void testBidirectionalWrapper(){
        ...
        Bidirectional.Mode[] modes = new Bidirectional.Mode[]{Bidirectional.Mode.CONCAT, Bidirectional.Mode.SUM,
                Bidirectional.Mode.AVERAGE, Bidirectional.Mode.MUL};

        Random r = new Random(12345);
        for( int mb : new int[]{1, 3}) {
//            for (int maskingType = 0; maskingType < 3; maskingType++) {
            for (int maskingType = 0; maskingType < 1; maskingType++) {
                ...
                for (Bidirectional.Mode m : modes) {
                    ...
                    boolean gradOK = GradientCheckUtil.checkGradients(net, DEFAULT_EPS, DEFAULT_MAX_REL_ERROR,
                            DEFAULT_MIN_ABS_ERROR, PRINT_RESULTS, RETURN_ON_FIRST_FAILURE, in, labels);
                    assertTrue(gradOK);
                }
            }
        }
