 ...
 */
@Slf4j
public class LossLayerGradientCheck {
        ...
        for( int i=0; i<lossFunctions.length; i++ ){
            for( int j=0; j<minibatchSizes.length; j++ ) {
                ...
                MultiLayerConfiguration conf = new NeuralNetConfiguration.Builder()
                        ...
                        .list()
                    // TODO how do we ensure nIn is always equal to nOut for the purpose of these tests?
                        .layer(0, new DenseLayer.Builder().nIn(nOut[i]).nOut(nOut[i]).activation("tanh").build())
