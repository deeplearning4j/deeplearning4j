...
class EqualsTest : public NDArrayTests {
 public:
  ...
  const sd::LongType secondShapeBuffer[8] = {2, 2, 1, 6, 1, 0, 6, 99};
...
TEST_F(EqualsTest, Eps) {
  ...
  functions::reduce3::Reduce3<float, float>::execScalar(opNum, data, firstShapeBuffer, extraArgs, dataSecond,
                                                        secondShapeBuffer, val.buffer(), val.shapeInfo());
