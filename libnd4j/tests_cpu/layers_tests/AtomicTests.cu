...
static void multiplyLauncher(void *vbuffer, uint64_t length, void *vresult) {
  multiplyKernel<T>
      <<<256, 256, 1024, *sd::LaunchContext::defaultContext()->getCudaStream()>>>(vbuffer, length, vresult);
...
static void multiplyHost(NDArray &input, NDArray &output) {
  BUILD_SINGLE_SELECTOR(input.dataType(), multiplyLauncher,
                        (input.specialBuffer(), input.lengthOf(), output.specialBuffer()), SD_NUMERIC_TYPES);
}

static void sumHost(NDArray &input, NDArray &output) {
  BUILD_SINGLE_SELECTOR(input.dataType(), sumLauncher,
                        (input.specialBuffer(), input.lengthOf(), output.specialBuffer()), SD_NUMERIC_TYPES);
}

static void subHost(NDArray &input, NDArray &output) {
  BUILD_SINGLE_SELECTOR(input.dataType(), subLauncher,
                        (input.specialBuffer(), input.lengthOf(), output.specialBuffer()), SD_FLOAT_TYPES);
}

static void divHost(NDArray &input, NDArray &output) {
  BUILD_SINGLE_SELECTOR(input.dataType(), divLauncher,
                        (input.specialBuffer(), input.lengthOf(), output.specialBuffer()), SD_FLOAT_TYPES);
}

TEST_F(AtomicTests, test_multiply) {
  std::vector<sd::DataType> dtypes = {sd::DataType::FLOAT32, sd::DataType::DOUBLE, sd::DataType::INT16,
                                      sd::DataType::HALF};
...
TEST_F(AtomicTests, test_sum) {
  std::vector<sd::DataType> dtypes = {sd::DataType::FLOAT32, sd::DataType::DOUBLE, sd::DataType::BFLOAT16,
