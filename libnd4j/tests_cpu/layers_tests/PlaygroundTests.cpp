...
void bench_conv(int outter_loop, const char *msg, const std::vector<NDArray *> &inList,
  ...
  sd_printf("%s:\nTime: %lld us; avg %f   Loop count %d\n", msg, values[values.size() / 2], sum / (double)values.size(),
            outter_loop);
}
...
TEST_F(PlaygroundTests, test_conv2d_bench) {
  ...
  for (auto h : helpers) {
    ...
    // Running conv2d onednn with kernels: 3 3 strides: 2 2 padding: 0 0 dilation: 1 1 paddingMode 0 weightFormat 0
    // isNCHW
    // 0
    int kH = 3, kW = 3, sH = 2, sW = 2, pH = 0, pW = 0, dH = 1, dW = 1, paddingMode = 0, weightFormat = 0;

    // op.execute( std::vector<NDArray *>{&in0, &w0},  std::vector<NDArray *>{&out0},{kH, kW, sH, sW, pH, pW, dH, dW,
    // paddingMode, weightFormat});
