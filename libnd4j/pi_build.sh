...
if [ "${TARGET_OS}" = "android" ];then
	...
	XTRA_MVN_ARGS="${XTRA_MVN_ARGS}"
	 ...
	 if [ "${CURRENT_TARGET}" == "jetson-arm64" ];then
		...
		XTRA_MVN_ARGS="${XTRA_MVN_ARGS} -Pcuda -pl !nd4j/nd4j-backends/nd4j-backends-impls/nd4j-cuda-platform -Dlibnd4j.cpu.compile.skip=true -Djavacpp.version=1.5.6 -Dcuda.version=${CUDA_VER} -Dlibnd4j.cuda=${CUDA_VER} -Dlibnd4j.chip=cuda -Dlibnd4j.compute=5.3 "
