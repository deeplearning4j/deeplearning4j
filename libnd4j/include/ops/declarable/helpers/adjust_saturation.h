...
namespace helpers {
    template <typename T>
    static FORCEINLINE void rgb_to_hsv(graph::LaunchContext* context, T r, T g, T b, T* h, T* s, T* v) {
    ...
    template <typename T>
    static FORCEINLINE void hsv_to_rgb(graph::LaunchContext* context, T h, T s, T v, T* r, T* g, T* b) {
        T c = s * v;
        T m = v - c;
        T dh = h * 6;
        T rr, gg, bb;
