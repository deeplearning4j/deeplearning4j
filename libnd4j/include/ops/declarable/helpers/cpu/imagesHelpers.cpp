...
template <typename T>
FORCEINLINE static void hsvToRgb(const NDArray* input, NDArray* output, const int dimC) {
    // auto op = nd4j::ops::helpers::hsvToRgb<T>;
    return tripleTransformer<T>(input, output, dimC, nd4j::ops::helpers::hsvToRgb<T>);
}

template <typename T>
FORCEINLINE static void rgbToHsv(const NDArray* input, NDArray* output, const int dimC) {
...
void transformHsvRgb(nd4j::LaunchContext* context, const NDArray* input, NDArray* output, const int dimC) {
    BUILD_SINGLE_SELECTOR(input->dataType(), hsvToRgb, (input, output, dimC), FLOAT_TYPES);
}

void transformRgbHsv(nd4j::LaunchContext* context, const NDArray* input, NDArray* output, const int dimC) {
    BUILD_SINGLE_SELECTOR(input->dataType(), rgbToHsv, (input, output, dimC), FLOAT_TYPES);
}
