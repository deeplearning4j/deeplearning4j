        ...
        for (auto bh = start; bh < stop; ++bh) {
            ...
            for (LongType chunk = 0; chunk < numChunks; ++chunk) {
                ...
                for (LongType s = 0; s < C; ++s) {
                    ...
                    const float interDecay = std::exp(-decay * (s + 1));
                    ...
                    for (LongType s2 = 0; s2 < upper; ++s2) {
                        ...
                        for (LongType d = 0; d < D; ++d)
                            dot += qRow[d] * kRow[d];
                        A[s2] = dot * std::exp(-decay * static_cast<float>(s - s2));
                    }
                ...
                const float chunkDecay = std::exp(-decay * C);
                ...
                for (LongType s = 0; s < C; ++s) {
                    const float posDecay = std::exp(-decay * (C - 1 - s));
