...
#pragma omp simd
            ...
            template <typename T>
            void skipgram_(void *syn0, void *syn1, void *syn1Neg, void *expTable, int *indices, int8_t *codes, double alpha, const int hsRounds, const int nsRounds, const int vocabSize, const int vectorLength, const int expLength) {
                ...
                if (hsRounds > 0) {
                    for (int r = 0; r < hsRounds; r++) {
                        ...
                        hSoftmax_<T>(syn0, syn1, expTable, neu1e, alpha, vectorLength, 1, expLength, true);
                    }
                }
            ...
            BUILD_SINGLE_TEMPLATE(template void skipgram_, (void *syn0, void *syn1, void *syn1Neg, void *expTable, int *indices, int8_t *codes, double alpha, const int hsRounds, const int nsRounds, const int vocabSize, const int vectorLength, const int expLength), FLOAT_TYPES);

            void skipgram(NDArray &syn0, NDArray &syn1, NDArray &syn1Neg, NDArray &expTable, NDArray &indices, NDArray &codes, NDArray &alpha, NDArray &randomValue, NDArray &inferenceVector) {
                auto xType = syn0.dataType();

                auto hsRounds = indices.lengthOf();
                auto nsRounds = 0;

                BUILD_SINGLE_SELECTOR(xType, skipgram_, (syn0.buffer(), syn1.buffer(), syn1Neg.buffer(), expTable.buffer(), reinterpret_cast<int *>(indices.buffer()), reinterpret_cast<int8_t*>(codes.buffer()), alpha.e<double>(0), hsRounds, nsRounds, (int) syn0.sizeAt(0), (int) syn0.sizeAt(1), (int) expTable.lengthOf()), FLOAT_TYPES);
            }
        }
    }
}