...
//////////////////////////////////////////////////////////////////////
SD_INLINE sd::Status outputSize(sd::LaunchContext *context, const int inSize, const int k, const int d, const int s,
                                bool isSameMode, int *outSize, int *padding_before, int *padding_after) {
  ...
  if (isSameMode) {
    *outSize = (inSize + s - 1) / s;
    const int padding_needed = sd::math::sd_max<int>(0, (*outSize - 1) * s + kEff - inSize);
...
SD_INLINE sd::Status dilation_hw(sd::LaunchContext *context, sd::LongType const *in, sd::LongType const *wh,
                                 std::vector<int> &strides, std::vector<int> &rates, bool isSameMode, int *sH, int *sW,
