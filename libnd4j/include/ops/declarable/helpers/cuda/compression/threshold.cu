...
static NDArray thresholdEstimate_(const NDArray &updates, const float threshold) {
  ...
  BUILD_SINGLE_SELECTOR(xType, encoderKernelP1Generic,
                        (launchDims, LaunchContext::defaultContext()->getCudaStream(), updates.specialBuffer(),
                         updates.lengthOf(), tmp.specialBuffer(), threshold),
                        SD_FLOAT_TYPES);
...
void thresholdEncode(NDArray &updates, NDArray &encoded, float threshold) {
  ...
  do {
    int numPrefixBlocks =
        sd::math::sd_max<int>(1, sd::math::sd_ceil<float, int>((float)numElts / (2.0f * prefixThreads)));
  ...
  // filling offsets
  encodeThresholdP2Int_(reinterpret_cast<void **>(pointers.data()), reinterpret_cast<int *>(blocks.specialBuffer()),
                        numBlocks, reinterpret_cast<int *>(offsets.specialBuffer()));

  NDArray::registerSpecialUse({&blocks, &offsets}, {});
  pm.synchronize();

  encodeThresholdP3_(updates.specialBuffer(), updates.shapeInfo(), reinterpret_cast<int *>(offsets.specialBuffer()),
                     updates.lengthOf(), reinterpret_cast<int *>(encoded.specialBuffer()));
...
void thresholdDecode(const NDArray &encoded, NDArray &updates) {
  ...
  BUILD_SINGLE_SELECTOR(xType, decoderKernelGeneric,
                        (launchDims, LaunchContext::defaultContext()->getCudaStream(), encoded.specialBuffer(),
                         updates.lengthOf(), updates.specialBuffer()),
                        SD_FLOAT_TYPES);
