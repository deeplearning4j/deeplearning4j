...
PLATFORM_IMPL(abs, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(rank <= 6, 0, "ABS_MKLDNN OP: the rank of input must be less or equal 6, but got rank = %i instead !",
               rank);

  absMKLDNN(input, output);
...
PLATFORM_CHECK(abs, ENGINE_CPU) {
  ...
  Requirements req("ONEDNN ABS OP");
...
PLATFORM_IMPL(abs_bp, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(rank <= 6 && dLdzRank <= 6, 0,
               "ABS_BP_MKLDNN OP: the rank of input and dLdz must be less or equal 6, but got input rank = %i and dLdz "
               "rank = %i instead !",
               rank, dLdzRank);

  absBpMKLDNN(input, dLdz, dLdx);
...
PLATFORM_CHECK(abs_bp, ENGINE_CPU) {
  ...
  Requirements req("ONEDNN ABS BP OP");
  req.expectTrue(block.isUseONEDNN(), IS_USE_ONEDNN_MSG) &&
