...
PLATFORM_IMPL(softplus, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(rank <= 6, 0, "SOFTPLUS_MKLDNN OP: the rank of input must be less or equal 6, but got rank = %i instead !",
               rank);

  softplusMKLDNN(input, output);
...
PLATFORM_CHECK(softplus, ENGINE_CPU) {
  ...
  Requirements req("ONEDNN SOFTPLUS OP");
...
PLATFORM_IMPL(softplus_bp, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(rank <= 6 && dLdzRank <= 6, 0,
               "SOFTPLUS_BP_MKLDNN OP: the rank of input and dLdz must be less or equal 6, but got input rank = %i and dLdz "
               "rank = %i instead !",
               rank, dLdzRank);

  softplusBpMKLDNN(input, dLdz, dLdx);
...
PLATFORM_CHECK(softplus_bp, ENGINE_CPU) {
  ...
  Requirements req("ONEDNN SOFTPLUS BP OP");
