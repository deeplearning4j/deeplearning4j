...
//////////////////////////////////////////////////////////////////////
static void sigmoidMKLDNN(NDArray* x, NDArray* z) {
  ...
  // operation primitive description - using logistic which is sigmoid
  ...
  dnnl::eltwise_forward::primitive_desc op_prim_desc(engine, dnnl::prop_kind::forward_inference,
                                                      algorithm::eltwise_logistic, x_mkl_md, z_mkl_md, 0.f, 0.f);
...
PLATFORM_IMPL(sigmoid, ENGINE_ONEDNN) {
  auto input = INPUT_VARIABLE(0);
  auto output = OUTPUT_VARIABLE(0);

  const sd::LongType rank = input->rankOf();
  REQUIRE_TRUE(rank <= 6, 0, "SIGMOID_MKLDNN OP: the rank of input must be less or equal 6, but got rank = %i instead !",
               rank);

  sigmoidMKLDNN(input, output);

  return sd::Status::OK;
}

PLATFORM_CHECK(sigmoid, ENGINE_ONEDNN) {
  auto x = INPUT_VARIABLE(0);
  auto z = OUTPUT_VARIABLE(0);

  Requirements req("ONEDNN SIGMOID OP");
...
//////////////////////////////////////////////////////////////////////
static void sigmoidBpMKLDNN(NDArray* x, NDArray* dLdz, NDArray* dLdx) {
  dnnl::memory::dims shape = *x->getShapeAsFlatVector();
  ...
  dnnl::eltwise_forward::primitive_desc op_ff_prim_desc(engine, dnnl::prop_kind::forward_training,
                                                         algorithm::eltwise_logistic, x_mkl_md, x_mkl_md, 0.f, 0.f);

  // OneDNN 3.x API for backward: primitive_desc(engine, algorithm, diff_src_md, diff_dst_md, data_md, alpha, beta, hint_fwd_pd)
  dnnl::eltwise_backward::primitive_desc op_prim_desc(engine, algorithm::eltwise_logistic,
...
PLATFORM_IMPL(sigmoid_bp, ENGINE_ONEDNN) {
  auto input = INPUT_VARIABLE(0);
  ...
  REQUIRE_TRUE(rank <= 6 && dLdzRank <= 6, 0,
               "SIGMOID_BP_MKLDNN OP: the rank of input and dLdz must be less or equal 6, but got input rank = %i and dLdz "
               "rank = %i instead !",
               rank, dLdzRank);

  sigmoidBpMKLDNN(input, dLdz, dLdx);

  return sd::Status::OK;
}

PLATFORM_CHECK(sigmoid_bp, ENGINE_ONEDNN) {
  auto x = INPUT_VARIABLE(0);
  auto dLdz = INPUT_VARIABLE(1);
  auto dLdx = OUTPUT_VARIABLE(0);

  Requirements req("ONEDNN SIGMOID BP OP");
