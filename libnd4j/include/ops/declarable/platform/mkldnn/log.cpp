...
//
// OneDNN implementation of natural logarithm: ln(x)
//
...
//////////////////////////////////////////////////////////////////////
static void logMKLDNN(NDArray* x, NDArray* z) {
  ...
  // log: ln(x)
  dnnl::eltwise_forward::desc op_desc(dnnl::prop_kind::forward_inference, algorithm::eltwise_log, x_mkl_md, 0, 0);
...
PLATFORM_IMPL(Log, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(rank <= 6, 0, "LOG_MKLDNN OP: the rank of input must be less or equal 6, but got rank = %i instead !",
               rank);

  logMKLDNN(input, output);
...
PLATFORM_CHECK(Log, ENGINE_CPU) {
  ...
  Requirements req("ONEDNN LOG OP");
