...
//////////////////////////////////////////////////////////////////////
static void reluMKLDNN(NDArray* x, NDArray* z, float negativeSlope) {
  ...
  dnnl::eltwise_forward::desc op_desc(dnnl::prop_kind::forward_inference, algorithm::eltwise_relu, x_mkl_md, negativeSlope, 0);
...
PLATFORM_IMPL(relu, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(rank <= 6, 0, "RELU_MKLDNN OP: the rank of input must be less or equal 6, but got rank = %i instead !",
               rank);

  // Get the negative slope (cutoff) from T arguments, default is 0
  float negativeSlope = block.numT() > 0 ? static_cast<float>(block.getTArguments()->at(0)) : 0.0f;

  reluMKLDNN(input, output, negativeSlope);
...
PLATFORM_CHECK(relu, ENGINE_CPU) {
  ...
  Requirements req("ONEDNN RELU OP");
...
//////////////////////////////////////////////////////////////////////
static void reluBpMKLDNN(NDArray* x, NDArray* dLdz, NDArray* dLdx, float negativeSlope) {
  dnnl::memory::dims shape = x->getShapeAsFlatVector();
  ...
  // operation primitive description
  // forward
  dnnl::eltwise_forward::desc op_ff_desc(dnnl::prop_kind::forward_inference, algorithm::eltwise_relu, x_mkl_md, negativeSlope, 0);
  dnnl::eltwise_forward::primitive_desc op_ff_prim_desc(op_ff_desc, engine);

  // backward description
  dnnl::eltwise_backward::desc op_desc(algorithm::eltwise_relu, dLdz_mkl_md, x_mkl_md, negativeSlope, 0);
...
PLATFORM_IMPL(relu_bp, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(rank <= 6 && dLdzRank <= 6, 0,
               "RELU_BP_MKLDNN OP: the rank of input and dLdz must be less or equal 6, but got input rank = %i and dLdz "
               "rank = %i instead !",
               rank, dLdzRank);

  // relu_bp doesn't take a scalar parameter in the standard op, so use 0
  reluBpMKLDNN(input, dLdz, dLdx, 0.0f);
...
PLATFORM_CHECK(relu_bp, ENGINE_CPU) {
  ...
  Requirements req("ONEDNN RELU BP OP");
  req.expectTrue(block.isUseONEDNN(), IS_USE_ONEDNN_MSG) &&
