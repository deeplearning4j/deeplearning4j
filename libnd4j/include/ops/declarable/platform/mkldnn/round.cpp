...
//
// OneDNN implementation of round: round to nearest integer
//
...
//////////////////////////////////////////////////////////////////////
static void roundMKLDNN(NDArray* x, NDArray* z) {
  ...
  dnnl::eltwise_forward::desc op_desc(dnnl::prop_kind::forward_inference, algorithm::eltwise_round, x_mkl_md, 0, 0);
...
PLATFORM_IMPL(rint, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(rank <= 6, 0, "ROUND_MKLDNN OP: the rank of input must be less or equal 6, but got rank = %i instead !",
               rank);

  roundMKLDNN(input, output);
...
PLATFORM_CHECK(rint, ENGINE_CPU) {
  ...
  Requirements req("ONEDNN ROUND OP");
