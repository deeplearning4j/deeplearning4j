...
//
// OneDNN implementation of square root: sqrt(x)
//
...
PLATFORM_IMPL(Sqrt, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(rank <= 6, 0, "SQRT_MKLDNN OP: the rank of input must be less or equal 6, but got rank = %i instead !",
               rank);

  sqrtMKLDNN(input, output);
...
PLATFORM_CHECK(Sqrt, ENGINE_CPU) {
  ...
  Requirements req("ONEDNN SQRT OP");
