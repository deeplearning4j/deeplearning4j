...
static void conv2dMKLDNN(sd::graph::Context &block, const NDArray *input, const NDArray *weights,
    ...
    int bS, iC, iH, iW, oC, oH, oW;                             // batch size, input channels, input height/width,
output channels, output height/width; int indIOioC, indIiH, indWoC, indWiC, indWkH, indOoH;       // corresponding
indexes ConvolutionUtils::getSizesAndIndexesConv2d(isNCHW, *input, *output, bS, iC, iH, iW, oC, oH, oW, indIOioC,
indIiH, indWiC, indWoC, indWkH, indOoH);
    ...
    onednnUtils::getMKLDNNMemoryDescConv2d(kH, kW, sH, sW, pH, pW, dH, dW, paddingMode, isNCHW,
...
static void conv2dBpMKLDNN(sd::graph::Context &block,
                            ...
                            const int kH, const int kW, const int sH,const int sW, int pH, int pW, const int dH, const
int dW, const int paddingMode, const int isNCHW) {

    int bS, iC, iH, iW, oC, oH, oW;                             // batch size, input channels, input height/width,
output channels, output height/width; int indIOioC, indIiH, indWoC, indWiC, indWkH, indOoH;       // corresponding
indexes ConvolutionUtils::getSizesAndIndexesConv2d(isNCHW, *input, *gradO, bS, iC, iH, iW, oC, oH, oW, indIOioC, indIiH,
...
user_weights_md(empty), user_diff_weights_md(empty), user_bias_md(empty), user_dst_md(empty);
    ...
    auto conv_desc = gradB != nullptr
                     ? convolution_forward::desc(prop_kind::forward, algorithm::convolution_auto, conv_src_md,
conv_weights_md, conv_bias_md, conv_dst_md, conv_strides, conv_dilation, conv_padding, conv_padding_r) :
...
conv_strides, conv_dilation, conv_padding, conv_padding_r);
    ...
    auto conv_prim_desc = convolution_forward::primitive_desc(conv_desc, onednnUtils::getEngine(
...
conv_src_md, conv_diff_weights_md, conv_dst_md, conv_strides, conv_dilation, conv_padding, conv_padding_r);
