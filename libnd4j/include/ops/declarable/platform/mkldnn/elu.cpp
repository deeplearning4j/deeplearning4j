...
//////////////////////////////////////////////////////////////////////
static void eluMKLDNN(NDArray* x, NDArray* z, float alpha) {
  ...
  // operation primitive description - elu with alpha parameter
  dnnl::eltwise_forward::desc op_desc(dnnl::prop_kind::forward_inference, algorithm::eltwise_elu, x_mkl_md, alpha, 0);
...
PLATFORM_IMPL(elu, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(rank <= 6, 0, "ELU_MKLDNN OP: the rank of input must be less or equal 6, but got rank = %i instead !",
               rank);

  // Get alpha parameter, default is 1.0
  float alpha = block.numT() > 0 ? static_cast<float>(T_ARG(0)) : 1.0f;

  eluMKLDNN(input, output, alpha);
...
PLATFORM_CHECK(elu, ENGINE_CPU) {
  ...
  Requirements req("ONEDNN ELU OP");
...
//////////////////////////////////////////////////////////////////////
static void eluBpMKLDNN(NDArray* x, NDArray* dLdz, NDArray* dLdx, float alpha) {
  dnnl::memory::dims shape = x->getShapeAsFlatVector();
  ...
  // forward
  dnnl::eltwise_forward::desc op_ff_desc(dnnl::prop_kind::forward_inference, algorithm::eltwise_elu, x_mkl_md, alpha, 0);
  dnnl::eltwise_forward::primitive_desc op_ff_prim_desc(op_ff_desc, engine);

  // backward description
  dnnl::eltwise_backward::desc op_desc(algorithm::eltwise_elu, dLdz_mkl_md, x_mkl_md, alpha, 0);
...
PLATFORM_IMPL(elu_bp, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(rank <= 6 && dLdzRank <= 6, 0,
               "ELU_BP_MKLDNN OP: the rank of input and dLdz must be less or equal 6, but got input rank = %i and dLdz "
               "rank = %i instead !",
               rank, dLdzRank);
  ...
  eluBpMKLDNN(input, dLdz, dLdx, alpha);
...
PLATFORM_CHECK(elu_bp, ENGINE_CPU) {
  ...
  Requirements req("ONEDNN ELU BP OP");
  req.expectTrue(block.isUseONEDNN(), IS_USE_ONEDNN_MSG) &&
