...
//
// OneDNN implementation of hardtanh: clip(x, -1, 1)
//
...
//////////////////////////////////////////////////////////////////////
static void hardtanhMKLDNN(NDArray* x, NDArray* z) {
  ...
  dnnl::eltwise_forward::desc op_desc(dnnl::prop_kind::forward_inference, algorithm::eltwise_clip, x_mkl_md, -1.0f, 1.0f);
...
PLATFORM_IMPL(hardtanh, ENGINE_CPU) {
  auto input = INPUT_VARIABLE(0);
  auto output = OUTPUT_VARIABLE(0);

  const sd::LongType rank = input->rankOf();
  REQUIRE_TRUE(rank <= 6, 0, "HARDTANH_MKLDNN OP: the rank of input must be less or equal 6, but got rank = %i instead !",
               rank);

  hardtanhMKLDNN(input, output);

  return sd::Status::OK;
}

PLATFORM_CHECK(hardtanh, ENGINE_CPU) {
  auto x = INPUT_VARIABLE(0);
  auto z = OUTPUT_VARIABLE(0);

  Requirements req("ONEDNN HARDTANH OP");
...
//////////////////////////////////////////////////////////////////////
static void hardtanhBpMKLDNN(NDArray* x, NDArray* dLdz, NDArray* dLdx) {
  dnnl::memory::dims shape = x->getShapeAsFlatVector();
  ...
  dnnl::eltwise_forward::desc op_ff_desc(dnnl::prop_kind::forward_inference, algorithm::eltwise_clip, x_mkl_md, -1.0f, 1.0f);
  dnnl::eltwise_forward::primitive_desc op_ff_prim_desc(op_ff_desc, engine);

  dnnl::eltwise_backward::desc op_desc(algorithm::eltwise_clip, dLdz_mkl_md, x_mkl_md, -1.0f, 1.0f);
...
PLATFORM_IMPL(hardtanh_bp, ENGINE_CPU) {
  auto input = INPUT_VARIABLE(0);
  ...
  REQUIRE_TRUE(rank <= 6 && dLdzRank <= 6, 0,
               "HARDTANH_BP_MKLDNN OP: the rank of input and dLdz must be less or equal 6, but got input rank = %i and dLdz "
               "rank = %i instead !",
               rank, dLdzRank);

  hardtanhBpMKLDNN(input, dLdz, dLdx);

  return sd::Status::OK;
}

PLATFORM_CHECK(hardtanh_bp, ENGINE_CPU) {
  auto x = INPUT_VARIABLE(0);
  auto dLdz = INPUT_VARIABLE(1);
  auto dLdx = OUTPUT_VARIABLE(0);

  Requirements req("ONEDNN HARDTANH BP OP");
  req.expectTrue(block.isUseONEDNN(), IS_USE_ONEDNN_MSG) &&
