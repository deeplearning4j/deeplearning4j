...
//////////////////////////////////////////////////////////////////////
static void mishMKLDNN(NDArray* x, NDArray* z) {
  ...
  // operation primitive description - mish: x * tanh(softplus(x)) = x * tanh(log(1 + exp(x)))
  dnnl::eltwise_forward::desc op_desc(dnnl::prop_kind::forward_inference, algorithm::eltwise_mish, x_mkl_md, 0, 0);
...
PLATFORM_IMPL(mish, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(rank <= 6, 0, "MISH_MKLDNN OP: the rank of input must be less or equal 6, but got rank = %i instead !",
               rank);

  mishMKLDNN(input, output);
...
PLATFORM_CHECK(mish, ENGINE_CPU) {
  ...
  Requirements req("ONEDNN MISH OP");
...
//////////////////////////////////////////////////////////////////////
static void mishBpMKLDNN(NDArray* x, NDArray* dLdz, NDArray* dLdx) {
  dnnl::memory::dims shape = x->getShapeAsFlatVector();
  ...
  // forward
  dnnl::eltwise_forward::desc op_ff_desc(dnnl::prop_kind::forward_inference, algorithm::eltwise_mish, x_mkl_md, 0, 0);
  dnnl::eltwise_forward::primitive_desc op_ff_prim_desc(op_ff_desc, engine);

  // backward description
  dnnl::eltwise_backward::desc op_desc(algorithm::eltwise_mish, dLdz_mkl_md, x_mkl_md, 0, 0);
...
PLATFORM_IMPL(mish_bp, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(rank <= 6 && dLdzRank <= 6, 0,
               "MISH_BP_MKLDNN OP: the rank of input and dLdz must be less or equal 6, but got input rank = %i and dLdz "
               "rank = %i instead !",
               rank, dLdzRank);

  mishBpMKLDNN(input, dLdz, dLdx);
...
PLATFORM_CHECK(mish_bp, ENGINE_CPU) {
  ...
  Requirements req("ONEDNN MISH BP OP");
  req.expectTrue(block.isUseONEDNN(), IS_USE_ONEDNN_MSG) &&
