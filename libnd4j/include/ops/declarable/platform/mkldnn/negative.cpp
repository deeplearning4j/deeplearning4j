...
//
// OneDNN implementation of negation: -x
// Uses linear transformation with alpha=-1, beta=0
//
...
PLATFORM_IMPL(Neg, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(rank <= 6, 0, "NEG_MKLDNN OP: the rank of input must be less or equal 6, but got rank = %i instead !",
               rank);

  negativeMKLDNN(input, output);
...
PLATFORM_CHECK(Neg, ENGINE_CPU) {
  ...
  Requirements req("ONEDNN NEG OP");
