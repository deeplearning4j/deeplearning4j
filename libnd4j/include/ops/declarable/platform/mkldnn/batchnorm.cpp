...
namespace nd4j {
    namespace ops {
        PLATFORM_IMPL(batchnorm_new) {
            ...
            if (streams.empty()) {
                streams.emplace_back(MKLDNNStream("batchnorm_new"));
            }
            ...
            if (streams[0].checkAndReset({input, mean, variance, gamma, beta}, {output}, {(float)epsilon}, axes)) {
                ...
                auto batchnorm_desc = batch_normalization_forward::desc(prop_kind::forward_inference, batchnorm_src_md, epsilon, normalization_flags::use_global_stats | (applyScale || applyOffset ? normalization_flags::use_scale_shift : (normalization_flags) 0));
                ...
                auto batchnorm_mean_memory = mkldnn::memory(batchnorm_prim_desc.mean_desc(), engine, mean->buffer());
                auto batchnorm_variance_memory = mkldnn::memory(batchnorm_prim_desc.variance_desc(), engine, variance->buffer());
                ...
                if (m.get_desc() != user_src_memory.get_desc()) {
                    ...
                    reorder(user_src_memory, batchnorm_src_memory).execute(stream, user_src_memory, batchnorm_src_memory);
                }
                ...
                if (applyScale || applyOffset) {
                    ...
                    batch_normalization_forward(batchnorm_prim_desc).execute(stream, {{MKLDNN_ARG_SRC, batchnorm_src_memory}, {MKLDNN_ARG_MEAN, batchnorm_mean_memory}, {MKLDNN_ARG_VARIANCE, batchnorm_variance_memory}, {MKLDNN_ARG_WEIGHTS, batchnorm_weights_memory}, {MKLDNN_ARG_DST, batchnorm_dst_memory}});
                } else {
                    batch_normalization_forward(batchnorm_prim_desc).execute(stream, {{MKLDNN_ARG_SRC, batchnorm_src_memory}, {MKLDNN_ARG_MEAN, batchnorm_mean_memory}, {MKLDNN_ARG_VARIANCE, batchnorm_variance_memory}, {MKLDNN_ARG_DST, batchnorm_dst_memory}});
                }

                if (batchnorm_prim_desc.dst_desc() != user_dst_memory.get_desc()) {
                    reorder(batchnorm_dst_memory, user_dst_memory).execute(stream, batchnorm_dst_memory, user_dst_memory);
                }
        ...
        PLATFORM_CHECK(batchnorm_new) {
            ...
            return block.isUseMKLDNN() && nd4j::MKLDNNStream::isSupported({input, mean, variance, gamma, beta, output}) && axes.size() == 1;
        }
    }
}