...
PLATFORM_IMPL(gelu, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(rank <= 6, 0, "GELU_MKLDNN OP: the rank of input must be less or equal 6, but got rank = %i instead !",
               rank);

  geluMKLDNN(input, output);
...
PLATFORM_CHECK(gelu, ENGINE_CPU) {
  ...
  Requirements req("ONEDNN GELU OP");
...
PLATFORM_IMPL(gelu_bp, ENGINE_CPU) {
  auto input = INPUT_VARIABLE(0);
  ...
  REQUIRE_TRUE(rank <= 6 && dLdzRank <= 6, 0,
               "GELU_BP_MKLDNN OP: the rank of input and dLdz must be less or equal 6, but got input rank = %i and dLdz "
               "rank = %i instead !",
               rank, dLdzRank);

  geluBpMKLDNN(input, dLdz, dLdx);

  return sd::Status::OK;
}

PLATFORM_CHECK(gelu_bp, ENGINE_CPU) {
  auto x = INPUT_VARIABLE(0);
  auto dLdz = INPUT_VARIABLE(1);
  auto dLdx = OUTPUT_VARIABLE(0);

  Requirements req("ONEDNN GELU BP OP");
  req.expectTrue(block.isUseONEDNN(), IS_USE_ONEDNN_MSG) &&
