...
static void lreluMKLDNN(NDArray* x, NDArray* z, float alpha) {
  ...
  // operation primitive description - relu with negative slope (leaky relu)
  dnnl::eltwise_forward::desc op_desc(dnnl::prop_kind::forward_inference, algorithm::eltwise_relu, x_mkl_md, alpha, 0);
...
PLATFORM_IMPL(lrelu, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(rank <= 6, 0, "LRELU_MKLDNN OP: the rank of input must be less or equal 6, but got rank = %i instead !",
               rank);

  // Get alpha parameter (negative slope), default is 0.01
  float alpha = block.numT() > 0 ? static_cast<float>(T_ARG(0)) : 0.01f;

  lreluMKLDNN(input, output, alpha);
...
static void lreluBpMKLDNN(NDArray* x, NDArray* dLdz, NDArray* dLdx, float alpha) {
  ...
  // forward
  dnnl::eltwise_forward::desc op_ff_desc(dnnl::prop_kind::forward_inference, algorithm::eltwise_relu, x_mkl_md, alpha, 0);
  dnnl::eltwise_forward::primitive_desc op_ff_prim_desc(op_ff_desc, engine);

  // backward description
  dnnl::eltwise_backward::desc op_desc(algorithm::eltwise_relu, dLdz_mkl_md, x_mkl_md, alpha, 0);
...
PLATFORM_IMPL(lrelu_bp, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(rank <= 6 && dLdzRank <= 6, 0,
               "LRELU_BP_MKLDNN OP: the rank of input and dLdz must be less or equal 6, but got input rank = %i and dLdz "
               "rank = %i instead !",
               rank, dLdzRank);
  ...
  lreluBpMKLDNN(input, dLdz, dLdx, alpha);
