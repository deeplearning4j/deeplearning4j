...
//
// OneDNN implementation of HardSwish: x * relu6(x + 3) / 6
//
...
//////////////////////////////////////////////////////////////////////
static void hardSwishMKLDNN(NDArray* x, NDArray* z) {
  ...
  // HardSwish: x * relu6(x + 3) / 6 = x * min(max(0, x + 3), 6) / 6
  dnnl::eltwise_forward::desc op_desc(dnnl::prop_kind::forward_inference, algorithm::eltwise_hardswish, x_mkl_md, 0, 0);
...
PLATFORM_IMPL(hardswish, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(rank <= 6, 0, "HARDSWISH_MKLDNN OP: the rank of input must be less or equal 6, but got rank = %i instead !",
               rank);

  hardSwishMKLDNN(input, output);
...
PLATFORM_CHECK(hardswish, ENGINE_CPU) {
  ...
  Requirements req("ONEDNN HARDSWISH OP");
...
//////////////////////////////////////////////////////////////////////
static void hardSwishBpMKLDNN(NDArray* x, NDArray* dLdz, NDArray* dLdx) {
  dnnl::memory::dims shape = x->getShapeAsFlatVector();
  ...
  dnnl::eltwise_forward::desc op_ff_desc(dnnl::prop_kind::forward_inference, algorithm::eltwise_hardswish, x_mkl_md, 0, 0);
  dnnl::eltwise_forward::primitive_desc op_ff_prim_desc(op_ff_desc, engine);

  dnnl::eltwise_backward::desc op_desc(algorithm::eltwise_hardswish, dLdz_mkl_md, x_mkl_md, 0, 0);
...
PLATFORM_IMPL(hardswish_bp, ENGINE_CPU) {
  auto input = INPUT_VARIABLE(0);
  ...
  REQUIRE_TRUE(rank <= 6 && dLdzRank <= 6, 0,
               "HARDSWISH_BP_MKLDNN OP: the rank of input and dLdz must be less or equal 6");

  hardSwishBpMKLDNN(input, dLdz, dLdx);

  return sd::Status::OK;
}

PLATFORM_CHECK(hardswish_bp, ENGINE_CPU) {
  auto x = INPUT_VARIABLE(0);
  auto dLdz = INPUT_VARIABLE(1);
  auto dLdx = OUTPUT_VARIABLE(0);

  Requirements req("ONEDNN HARDSWISH BP OP");
  req.expectTrue(block.isUseONEDNN(), IS_USE_ONEDNN_MSG) &&
