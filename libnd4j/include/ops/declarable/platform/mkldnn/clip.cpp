...
//
// OneDNN implementation of clip_by_value operation: clamp(x, min, max)
//
...
PLATFORM_IMPL(clipbyvalue, ENGINE_CPU) {
  ...
  clipMKLDNN(input, output, minVal, maxVal);
...
PLATFORM_CHECK(clipbyvalue, ENGINE_CPU) {
  ...
  Requirements req("ONEDNN CLIP OP");
...
PLATFORM_IMPL(clipbyvalue_bp, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(rank <= 6 && dLdzRank <= 6, 0,
               "CLIP_BP_MKLDNN OP: the rank of input and dLdz must be less or equal 6");

  float minVal = block.numT() > 0 ? static_cast<float>(T_ARG(0)) : 0.0f;
  float maxVal = block.numT() > 1 ? static_cast<float>(T_ARG(1)) : 6.0f;

  clipBpMKLDNN(input, dLdz, dLdx, minVal, maxVal);
...
PLATFORM_CHECK(clipbyvalue_bp, ENGINE_CPU) {
  ...
  Requirements req("ONEDNN CLIP BP OP");
