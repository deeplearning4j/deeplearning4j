...
static void conv2dCUDNN(const LaunchContext* context, NDArray* input, NDArray* weights, NDArray* bias,
  ...
  // cuDNN streams that escape from the capture graph, invalidating it (cudaStreamCaptureStatusInvalidated).
  // Use the heuristic _v7 variant instead, which selects an algorithm without running GPU benchmarks.
  ...
  if (count == 0)
    THROW_EXCEPTION("conv2dCUDNN: cudnnGetConvolutionForwardAlgorithm failed as the count is 0");
  algo = algoPerf.algo;
...
static void conv2dBpCUDNN(const LaunchContext* context, NDArray* input, NDArray* weights,
  ...
  if (count == 0)
    THROW_EXCEPTION("conv2dBpCUDNN: cudnnGetConvolutionBackwardFilterAlgorithm failed as the count is 0");
  ...
  if (count == 0)
    THROW_EXCEPTION("conv2dBpCUDNN: cudnnGetConvolutionBackwardDataAlgorithm failed as the count is 0");
