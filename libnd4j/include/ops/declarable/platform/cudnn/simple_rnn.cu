...
// @author Adam Gibson
//
// cuDNN-based simple RNN (RELU or TANH activation)
//
...
// Copy weights for simple RNN: 1 gate only
void copySimpleRnnWeights(const cudaStream_t &stream, bool isBidirectional, uint8_t *weightsSpace, size_t weightsSize,
                          uint8_t *inputWeightsData, uint8_t *recurrentWeightsData, uint8_t *biasesData,
                          LongType inputSize, int hiddenSize, int dataTypeSize) {
  int pseudo_layer_count = isBidirectional ? 2 : 1;
  uint8_t *wptr = weightsSpace;
  auto wEnd = wptr + weightsSize;

  // Simple RNN has 1 gate only
  auto input_pseudo_size = inputSize * hiddenSize * dataTypeSize;
  auto hidden_pseudo_size = hiddenSize * hiddenSize * dataTypeSize;
  ...
  // copy biases (1 gate for simple RNN)
  auto bias_size = hiddenSize * dataTypeSize;
  for (int i = 0; i < pseudo_layer_count; i++) {
    ...
    wptr += bias_size;
    // refill next with zeros (cuDNN expects double bias)
...
void cudnn_simple_rnn_old(LaunchContext *contextPtr, NDArray *input, NDArray *inputWeights,
                          ...
                          bool isBidirectional, bool useTanh) {
  sd_debug("cudnn simple_rnn api %s \n", "v6");
  ...
  const int dimC[rankOf] = {static_cast<int>(numLayers * numDirections), static_cast<int>(batchSize),
                            static_cast<int>(hiddenSize)};
  ...
  // RNN for simple RNN - use TANH or RELU mode
  RnnDesc rnnDesc;
  cudnnRNNMode_t rnnCellMode = useTanh ? CUDNN_RNN_TANH : CUDNN_RNN_RELU;
  ...
  copySimpleRnnWeights(stream, isBidirectional, (uint8_t *)weightsSpace, weightsSize, inputWeightsData,
                       recurrentWeightsData, biasesData, inputSize, hiddenSize, dataTypeSize);
  ...
  if (training) {
    CHECK_CUDNN_FAILURE_MSG(
        ...
        cudnnRNNForwardTraining(handle, rnnDesc, (int)maxSeqLength, xDescList.getDescriptors(), xData, hxDesc,
                                prevActData, nullptr, nullptr, wDesc, weightsSpace, yDescList.getDescriptors(), yData,
                                hyDesc, finalTimeStepActivationsData, nullptr, nullptr, workSpace, workSpaceSizeInBytes,
    ...
    CHECK_CUDNN_FAILURE_MSG(
        ...
        cudnnRNNForwardInference(handle, rnnDesc, (int)maxSeqLength, xDescList.getDescriptors(), xData, hxDesc,
                                 prevActData, nullptr, nullptr, wDesc, weightsSpace, yDescList.getDescriptors(), yData,
                                 hyDesc, finalTimeStepActivationsData, nullptr, nullptr, workSpace,
...
void cudnn_simple_rnn_v8(LaunchContext *contextPtr, NDArray *input, NDArray *seqLengthArray, NDArray *inputWeights,
                         NDArray *recurrentWeights, NDArray *biases, NDArray *prevAct, NDArray *outputActivations,
                         NDArray *finalTimeStepActivations, int maxSeqLength, int batchSize, int inputSize,
                         int hiddenSize, bool isBidirectional, bool useTanh) {
  sd_debug("cudnn simple_rnn api %s \n", "v8");
  ...
  CHECK_CUDNN_FAILURE_MSG(
      ...
      cudnnRNNForward(handle, rnnDesc, fwdMode, (const int32_t *)argSeqNdArray->specialBuffer(), xDataDesc, xData,
                      yDataDesc, yData, hDesc, prevActData, finalTimeStepActivationsData, nullptr, nullptr, nullptr,
                      weightsSize, weightsSpace, workSpaceSizeInBytes, workSpace, reserveSpaceSizeInBytes,
                      reserveSpace));
...
PLATFORM_IMPL(simple_rnn, ENGINE_CUDA) {
  ...
  auto hI = INPUT_VARIABLE(1);  // initial hidden state [bS, nOut]
  auto Wx = INPUT_VARIABLE(2);  // input-to-hidden weights [nIn, nOut]
  auto Wh = INPUT_VARIABLE(3);  // hidden-to-hidden weights [nOut, nOut]
  auto b = INPUT_VARIABLE(4);   // biases [nOut]

  auto h = OUTPUT_VARIABLE(0);  // output activations [time, bS, nOut]

  // activation: 0 = tanh, 1 = relu (default: tanh)
  const int activation = block.getIArguments()->size() > 0 ? I_ARG(0) : 0;
  bool useTanh = (activation == 0);
  ...
  bool isBidirectional = false;  // Standard simple RNN is unidirectional
...
//////////////////////////////////////////////////////////////////////////
PLATFORM_CHECK(simple_rnn, ENGINE_CUDA) {
  auto x = INPUT_VARIABLE(0);   // input [time, bS, nIn]
  auto hI = INPUT_VARIABLE(1);  // initial hidden state [bS, nOut]
  auto Wx = INPUT_VARIABLE(2);  // input-to-hidden weights [nIn, nOut]
  auto Wh = INPUT_VARIABLE(3);  // hidden-to-hidden weights [nOut, nOut]
  ...
  Requirements req("CUDNN SIMPLE_RNN OP");
