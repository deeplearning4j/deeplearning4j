...
PLATFORM_IMPL(linear, ENGINE_CPU) {
  ...
  fc.configure(&inTensor, &wTensor, bPtr, &outTensor);

  // Import or allocate input
...
//////////////////////////////////////////////////////////////////////////
PLATFORM_CHECK(linear, ENGINE_CPU) {
  auto input = INPUT_VARIABLE(0);
  auto weights = INPUT_VARIABLE(1);
  auto output = OUTPUT_VARIABLE(0);

  Requirements req("ARMCOMPUTE LINEAR OP");
  req.expectEq(makeInfoVariable(input->dataType(), TYPE_MSG_INPUT), DataType::FLOAT32) &&
      ...
      req.expectEq(makeInfoVariable(weights->ordering(), ORDERING_MSG_INPUT), 'c') &&
