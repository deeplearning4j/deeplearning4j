...
PLATFORM_IMPL(squeeze, ENGINE_CPU) {
  ...
  // Configure reshape (squeeze is just a reshape)
  arm_compute::NEReshapeLayer reshape;
  reshape.configure(&inTensor, &outTensor);
...
PLATFORM_CHECK(squeeze, ENGINE_CPU) {
  ...
  Requirements req("ARMCOMPUTE SQUEEZE OP");
