...
PLATFORM_IMPL(hardsigmoid, ENGINE_CPU) {
  ...
  // Default: alpha = 0.2, beta = 0.5 (from Keras/TensorFlow)
  // In ARM Compute: HARD_SIGMOID with alpha and beta parameters
  float alpha = 0.2f;
  float beta = 0.5f;

  arm_compute::ActivationLayerInfo info(arm_compute::ActivationLayerInfo::ActivationFunction::HARD_SWISH, alpha, beta);
...
PLATFORM_CHECK(hardsigmoid, ENGINE_CPU) {
  ...
  Requirements req("ARMCOMPUTE HARDSIGMOID OP");
