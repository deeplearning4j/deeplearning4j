...
PLATFORM_IMPL(reduce_min, ENGINE_CPU) {
  ...
  if (dimensions.size() == 1) {
    ...
    reduce.configure(&inTensor, &outTensor, armAxis, arm_compute::ReductionOperation::MIN, keepDims);
    ...
    for (size_t i = 0; i < dimensions.size(); i++) {
      ...
      arm_compute::NEReductionOperation reduce;
      reduce.configure(currentIn, currentOut, armAxis, arm_compute::ReductionOperation::MIN, keepDims);
...
PLATFORM_CHECK(reduce_min, ENGINE_CPU) {
  ...
  Requirements req("ARMCOMPUTE REDUCE_MIN OP");
