...
PLATFORM_IMPL(argmin, ENGINE_CPU) {
  ...
  // ARM Compute argmin output is U32 or S32
  ...
  // Configure argmin
  arm_compute::NEArgMinMaxLayer argmin;
  argmin.configure(&inTensor, armAxis, &outTensor, arm_compute::ReductionOperation::ARG_IDX_MIN);
...
PLATFORM_CHECK(argmin, ENGINE_CPU) {
  ...
  Requirements req("ARMCOMPUTE ARGMIN OP");
