...
PLATFORM_IMPL(slice, ENGINE_CPU) {
  ...
  if (block.width() > 2) {
    ...
    for (sd::LongType i = 0; i < beginArr->lengthOf(); i++) {
      begin.push_back(beginArr->e<int>(i));
      size.push_back(sizeArr->e<int>(i));
    }
  } else {
    for (int i = 0; i < input->rankOf(); i++) {
      begin.push_back(INT_ARG(i));
      size.push_back(INT_ARG(i + input->rankOf()));
    }
  }
  ...
  for (int i = input->rankOf() - 1; i >= 0; i--) {
    starts.set(input->rankOf() - 1 - i, begin[i]);
    ends.set(input->rankOf() - 1 - i, begin[i] + size[i]);
  }
  ...
  // Configure slice
  arm_compute::NESlice slice;
  slice.configure(&inTensor, &outTensor, starts, ends);
...
PLATFORM_CHECK(slice, ENGINE_CPU) {
  ...
  Requirements req("ARMCOMPUTE SLICE OP");
