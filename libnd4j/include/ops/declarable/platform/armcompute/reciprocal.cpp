...
PLATFORM_IMPL(reciprocal, ENGINE_CPU) {
  ...
  // Configure reciprocal (1/x) - use rsqrt then square, or implement with power(-1)
  // Actually, we can create a ones tensor and divide
  Arm_Tensor onesTensor;
  onesTensor.allocator()->init(inInfo);

  arm_compute::NEElementwiseDivision reciprocal;

  // First allocate ones tensor and fill with 1.0
  onesTensor.allocator()->allocate();
  auto onesPtr = reinterpret_cast<float*>(onesTensor.buffer());
  for (size_t i = 0; i < input->lengthOf(); i++) {
    onesPtr[i] = 1.0f;
  }

  reciprocal.configure(&onesTensor, &inTensor, &outTensor);
...
PLATFORM_CHECK(reciprocal, ENGINE_CPU) {
  ...
  Requirements req("ARMCOMPUTE RECIPROCAL OP");
