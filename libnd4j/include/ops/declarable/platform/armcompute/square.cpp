...
PLATFORM_IMPL(square, ENGINE_CPU) {
  ...
  // Configure square using pixel-wise multiplication (x * x)
  arm_compute::NEPixelWiseMultiplication square;
  square.configure(&inTensor, &inTensor, &outTensor, 1.0f, arm_compute::ConvertPolicy::SATURATE,
...
PLATFORM_CHECK(square, ENGINE_CPU) {
  ...
  Requirements req("ARMCOMPUTE SQUARE OP");
