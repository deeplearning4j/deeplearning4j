...
PLATFORM_IMPL(log_softmax, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(dim < rank && dim >= 0, 0,
               "LOG_SOFTMAX ARMCOMPUTE OP: the value of input integer parameter (dimension) must be less than input array rank "
  ...
  ArmFunction<arm_compute::NELogSoftmaxLayer> logSoftmax;
  logSoftmax.configure(input, output, Arm_DataLayout::UNKNOWN, beta, armAxis);
  logSoftmax.run();
...
PLATFORM_CHECK(log_softmax, ENGINE_CPU) {
  ...
  Requirements req("ARMCOMPUTE LOG_SOFTMAX OP");
