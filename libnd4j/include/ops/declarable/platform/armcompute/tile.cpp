...
PLATFORM_IMPL(tile, ENGINE_CPU) {
  ...
  if (block.width() > 1) {
    ...
    for (sd::LongType i = 0; i < reps->lengthOf(); i++) {
      multiples.push_back(static_cast<uint32_t>(reps->e<int>(i)));
    }
  } else {
    for (int i = 0; i < block.numI(); i++) {
      multiples.push_back(static_cast<uint32_t>(INT_ARG(i)));
    }
  }

  // ARM Compute uses reversed axis ordering, so reverse the multiples
...
PLATFORM_CHECK(tile, ENGINE_CPU) {
  ...
  Requirements req("ARMCOMPUTE TILE OP");
