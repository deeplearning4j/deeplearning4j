...
PLATFORM_IMPL(softmax, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(dim < rank && dim >= 0, 0,
               "SOFTMAX ARMCOMPUTE OP: the value of input integer parameter (dimension) must be less than input array rank "
               "%i, but got dimension = %i instead !",
               rank, dim);

  // ARM Compute uses axis in reverse order (from the end)
  // In ARM Compute, axis 0 is the innermost (last) dimension
  int armAxis = rank - 1 - dim;

  // ARM Compute beta parameter (scaling factor, default 1.0)
  float beta = 1.0f;

  ArmFunction<arm_compute::NESoftmaxLayer> softmax;
  softmax.configure(input, output, Arm_DataLayout::UNKNOWN, beta, armAxis);
  softmax.run();
...
PLATFORM_CHECK(softmax, ENGINE_CPU) {
  ...
  Requirements req("ARMCOMPUTE SOFTMAX OP");
