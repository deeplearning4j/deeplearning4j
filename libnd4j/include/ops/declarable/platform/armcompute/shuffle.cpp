...
//////////////////////////////////////////////////////////////////////////
// Channel Shuffle operation for ShuffleNet-like architectures
PLATFORM_IMPL(shuffle, ENGINE_CPU) {
  auto input = INPUT_VARIABLE(0);   // [bS, C, H, W] or [bS, H, W, C]
  auto output = OUTPUT_VARIABLE(0);

  int groups = INT_ARG(0);
  int isNCHW = block.getIArguments()->size() > 1 ? INT_ARG(1) : 1;

  // ARM Compute has channel shuffle operation
  Arm_DataLayout layout = isNCHW ? Arm_DataLayout::NCHW : Arm_DataLayout::NHWC;
  ...
  arm_compute::NEChannelShuffleLayer shuffle;
  shuffle.configure(&inTensor, &outTensor, groups);
...
//////////////////////////////////////////////////////////////////////////
PLATFORM_CHECK(shuffle, ENGINE_CPU) {
  auto input = INPUT_VARIABLE(0);
  auto output = OUTPUT_VARIABLE(0);

  Requirements req("ARMCOMPUTE SHUFFLE OP");
