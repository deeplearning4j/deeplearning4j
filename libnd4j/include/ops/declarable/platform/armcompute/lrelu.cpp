...
PLATFORM_IMPL(lrelu, ENGINE_CPU) {
  ...
  // Leaky ReLU alpha (negative slope), default is 0.01
  float alpha = block.numT() > 0 ? static_cast<float>(T_ARG(0)) : 0.01f;

  arm_compute::ActivationLayerInfo info(arm_compute::ActivationLayerInfo::ActivationFunction::LEAKY_RELU, alpha);
...
PLATFORM_CHECK(lrelu, ENGINE_CPU) {
  ...
  Requirements req("ARMCOMPUTE LRELU OP");
