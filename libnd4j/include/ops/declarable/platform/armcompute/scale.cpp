...
//////////////////////////////////////////////////////////////////////////
// Scale operation - resize/scale an image
PLATFORM_IMPL(resize_bilinear, ENGINE_CPU) {
  ...
  arm_compute::NEScale scale;
  arm_compute::ScaleKernelInfo scaleInfo(arm_compute::InterpolationPolicy::BILINEAR,
                                          arm_compute::BorderMode::REPLICATE,
                                          arm_compute::PixelValue(0.0f),
                                          arm_compute::SamplingPolicy::CENTER,
                                          false);  // align_corners
  scale.configure(&inTensor, &outTensor, scaleInfo);
...
PLATFORM_CHECK(resize_bilinear, ENGINE_CPU) {
  ...
  Requirements req("ARMCOMPUTE RESIZE_BILINEAR OP");
  req.expectEq(makeInfoVariable(input->dataType(), TYPE_MSG_INPUT), DataType::FLOAT32) &&
      req.expectEq(makeInfoVariable(output->dataType(), TYPE_MSG_OUTPUT), DataType::FLOAT32) &&
      req.expectEq(makeInfoVariable(input->rankOf(), RANK_MSG_INPUT), 4) &&  // NHWC format
