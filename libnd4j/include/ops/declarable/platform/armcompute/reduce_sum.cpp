...
PLATFORM_IMPL(reduce_sum, ENGINE_CPU) {
  ...
  // For sum reduction, we need to use NEReductionOperation with SUM
  // ARM Compute axis is in reverse order
  unsigned int armAxis = dimensions.size() > 0 ? (rank - 1 - dimensions[0]) : 0;

  // Configure reduction layer
  arm_compute::NEReductionOperation reduceSum;
  reduceSum.configure(&inTensor, &outTensor, armAxis, arm_compute::ReductionOperation::SUM, keepDims);
...
PLATFORM_CHECK(reduce_sum, ENGINE_CPU) {
  ...
  req.expectEq(makeInfoVariable(input->dataType(), TYPE_MSG_INPUT), DataType::FLOAT32) &&
      ...
      req.expectLessEq(makeInfoVariable(static_cast<int>(dimensions.size()), "dimensions#size"), 1) &&  // NEReductionOperation supports single axis only
