...
//////////////////////////////////////////////////////////////////////////
// LogSigmoid: f(x) = log(sigmoid(x)) = log(1/(1+exp(-x))) = -log(1+exp(-x))
PLATFORM_IMPL(logsigmoid, ENGINE_CPU) {
  ...
  Arm_Tensor inTensor, outTensor, sigmoidTensor;
  inTensor.allocator()->init(inInfo);
  outTensor.allocator()->init(outInfo);
  sigmoidTensor.allocator()->init(outInfo);

  // First compute sigmoid
  arm_compute::ActivationLayerInfo sigmoidInfo(
      arm_compute::ActivationLayerInfo::ActivationFunction::LOGISTIC);
  arm_compute::NEActivationLayer sigmoidLayer;
  ...
  sigmoidTensor.allocator()->allocate();
...
PLATFORM_CHECK(logsigmoid, ENGINE_CPU) {
  ...
  Requirements req("ARMCOMPUTE LOGSIGMOID OP");
