...
PLATFORM_IMPL(prelu, ENGINE_CPU) {
  ...
  auto alpha = INPUT_VARIABLE(1);  // PReLU alpha parameters
  auto output = OUTPUT_VARIABLE(0);

  // Create ARM Compute tensor info
  auto inInfo = getArmTensorInfo(*input, Arm_DataLayout::UNKNOWN);
  auto alphaInfo = getArmTensorInfo(*alpha, Arm_DataLayout::UNKNOWN);
  auto outInfo = getArmTensorInfo(*output, Arm_DataLayout::UNKNOWN);

  Arm_Tensor inTensor, alphaTensor, outTensor;
  inTensor.allocator()->init(inInfo);
  alphaTensor.allocator()->init(alphaInfo);
  outTensor.allocator()->init(outInfo);

  // Configure PReLU layer
  arm_compute::NEPReluLayer prelu;
  prelu.configure(&inTensor, &alphaTensor, &outTensor);
  ...
  // Import or allocate memory for alpha
  if (!alpha->hasPaddedBuffer() && !alphaTensor.info()->has_padding()) {
    alphaTensor.allocator()->import_memory(alpha->buffer());
  } else {
    alphaTensor.allocator()->allocate();
    copyToTensor(*alpha, alphaTensor);
  }
...
PLATFORM_CHECK(prelu, ENGINE_CPU) {
  ...
  Requirements req("ARMCOMPUTE PRELU OP");
  req.expectEq(makeInfoVariable(input->dataType(), TYPE_MSG_INPUT0), DataType::FLOAT32) &&
      ...
      req.expectGreater(makeInfoVariable(input->rankOf(), RANK_MSG_INPUT0), 0) &&
