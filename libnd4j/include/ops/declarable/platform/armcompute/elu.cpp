...
PLATFORM_IMPL(elu, ENGINE_CPU) {
  ...
  // ELU alpha parameter, default is 1.0
  float alpha = block.numT() > 0 ? static_cast<float>(T_ARG(0)) : 1.0f;

  arm_compute::ActivationLayerInfo info(arm_compute::ActivationLayerInfo::ActivationFunction::ELU, alpha);
...
PLATFORM_CHECK(elu, ENGINE_CPU) {
  ...
  Requirements req("ARMCOMPUTE ELU OP");
