...
PLATFORM_IMPL(relu6, ENGINE_CPU) {
  ...
  // ReLU6 is bounded ReLU: min(max(0, x), 6)
  // In ARM Compute, this is LU_BOUNDED_RELU with alpha=6, beta=0
  arm_compute::ActivationLayerInfo info(arm_compute::ActivationLayerInfo::ActivationFunction::LU_BOUNDED_RELU, 6.0f, 0.0f);
...
PLATFORM_CHECK(relu6, ENGINE_CPU) {
  ...
  Requirements req("ARMCOMPUTE RELU6 OP");
