...
PLATFORM_IMPL(reduce_max, ENGINE_CPU) {
  ...
  if (dimensions.size() == 1) {
    ...
    reduce.configure(&inTensor, &outTensor, armAxis, arm_compute::ReductionOperation::MAX, keepDims);
    ...
    // Multiple axes - do sequential reductions
    // Import input
    if (!input->hasPaddedBuffer() && !inTensor.info()->has_padding()) {
      inTensor.allocator()->import_memory(input->buffer());
    } else {
    ...
    // Sort dimensions in descending order for proper reduction
    ...
    for (size_t i = 0; i < dimensions.size(); i++) {
      ...
      arm_compute::NEReductionOperation reduce;
      reduce.configure(currentIn, currentOut, armAxis, arm_compute::ReductionOperation::MAX, keepDims);
...
PLATFORM_CHECK(reduce_max, ENGINE_CPU) {
  ...
  Requirements req("ARMCOMPUTE REDUCE_MAX OP");
