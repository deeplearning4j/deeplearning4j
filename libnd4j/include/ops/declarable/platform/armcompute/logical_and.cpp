...
PLATFORM_IMPL(boolean_and, ENGINE_CPU) {
  ...
  // Create tensor info with U8 type for boolean operations
  Arm_TensorShape xShape, yShape, outShape;
  xShape.set_num_dimensions(x->rankOf());
  yShape.set_num_dimensions(y->rankOf());
  ...
  for (int i = 0; i < y->rankOf(); i++) {
    yShape[y->rankOf() - 1 - i] = y->sizeAt(i);
  }
  ...
  Arm_TensorInfo yInfo(yShape, 1, arm_compute::DataType::U8);
  ...
  arm_compute::NELogicalAnd logicalAnd;
  logicalAnd.configure(&xTensor, &yTensor, &outTensor);
...
PLATFORM_CHECK(boolean_and, ENGINE_CPU) {
  ...
  Requirements req("ARMCOMPUTE BOOLEAN_AND OP");
  req.expectEq(makeInfoVariable(x->dataType(), TYPE_MSG_INPUT), DataType::BOOL) &&
      req.expectEq(makeInfoVariable(y->dataType(), TYPE_MSG_INPUT), DataType::BOOL) &&
