...
// @author Adam Gibson
//
// CUDA utilities for llama.cpp/GGML integration
//
...
static std::vector<ggml_backend_t> s_cudaBackends;
...
struct ggml_backend* getCudaBackend(int deviceId) {
    ...
    if (s_cudaBackends.size() <= static_cast<size_t>(deviceId)) {
        s_cudaBackends.resize(deviceId + 1, nullptr);
    }

    // Create backend if not already created
    if (s_cudaBackends[deviceId] == nullptr) {
        s_cudaBackends[deviceId] = ggml_backend_cuda_init(deviceId);
    }

    return s_cudaBackends[deviceId];
}
