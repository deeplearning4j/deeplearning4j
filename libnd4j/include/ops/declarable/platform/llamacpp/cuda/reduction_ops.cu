...
// @author Adam Gibson
//
// CUDA implementations of reduction operations using GGML kernels
//
...
//////////////////////////////////////////////////////////////////////////
// REDUCE_SUM - Sum all elements (CUDA)
...
PLATFORM_CHECK(reduce_sum, ENGINE_CUDA) {
    ...
    Requirements req("LLAMACPP CUDA REDUCE_SUM OP");
...
//////////////////////////////////////////////////////////////////////////
// REDUCE_SUM_ROWS - Sum along rows (CUDA)
PLATFORM_IMPL(reduce_sum_rows, ENGINE_CUDA) {
    auto input = INPUT_VARIABLE(0);
    auto output = OUTPUT_VARIABLE(0);
...
PLATFORM_CHECK(reduce_sum_rows, ENGINE_CUDA) {
    auto input = INPUT_VARIABLE(0);
    auto output = OUTPUT_VARIABLE(0);

    Requirements req("LLAMACPP CUDA REDUCE_SUM_ROWS OP");
    req.expectTrue(block.isUseLLAMACPP(), IS_USE_LLAMACPP_MSG);
    req.expectTrue(makeInfoVariable(llamacppUtils::hasCudaBackend(), "CUDA backend available"), NO_MSG);
    req.expectTrue(makeInfoVariable([input, output] {
        return llamacppUtils::isSupportedType(input->dataType()) &&
