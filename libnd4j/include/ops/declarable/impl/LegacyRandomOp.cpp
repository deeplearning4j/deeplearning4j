  ...
  */
  switch (opNum) {
    ...
    case sd::random::GaussianDistribution: {
      ...
      for (int e = 0; e < input->lengthOf(); e++) shape[e] = input->e<sd::LongType>(e);
    ...
    case sd::random::Linspace: {
      ...
      z->linspace(start->e<double>(0),
                  (finish->e<double>(0) - start->e<double>(0)) / (numOfElements->e<sd::LongType>(0) - 1.));
    } break;
    default: {
...
sd::ResultSet LegacyRandomOp::execute(sd::graph::RandomGenerator& rng, std::initializer_list<NDArray*> inputs,
...
sd::ResultSet LegacyRandomOp::execute(sd::graph::RandomGenerator& rng, std::vector<NDArray*>& inputs,
                                      std::vector<double>& tArgs, std::vector<int>& iArgs, bool isInplace) {
...
sd::Status LegacyRandomOp::validateDataTypes(Context& block) {
  if (block.isFastPath()) {
    ...
    for (auto v : fpo) {
      if (v != nullptr) {
        if (!v->isR()) return sd::Status::BAD_ARGUMENTS;
      }
    }
  } else {
    ...
    if (block.getVariableSpace()->hasVariable(pair)) {
      ...
      if (!var->hasNDArray()) return sd::Status::BAD_ARGUMENTS;

      auto arr = var->getNDArray();
      if (!arr->isR()) return sd::Status::BAD_ARGUMENTS;
    }
  }
