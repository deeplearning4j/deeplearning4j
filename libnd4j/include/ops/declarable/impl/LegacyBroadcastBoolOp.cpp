...
namespace nd4j {
    namespace ops {
        Nd4jStatus LegacyBroadcastBoolOp::validateAndExecute(Context &block) {
            ...
            if (x == z)
                NativeOpExcutioner::execBroadcast(nullptr, opNum, x->buffer(), x->shapeInfo(), y->buffer(), y->shapeInfo(), z->buffer(), z->shapeInfo(), dims.data(), dims.size(), tad.tadOnlyShapeInfo, tad.tadOffsets, tad.tadOnlyShapeInfo, tad.tadOffsets);
            else {
                ...
                NativeOpExcutioner::execBroadcast(nullptr, opNum, x->buffer(), x->shapeInfo(), y->buffer(), y->shapeInfo(), z->buffer(), z->shapeInfo(), dims.data(), dims.size(), tad.tadOnlyShapeInfo, tad.tadOffsets, tadZ.tadOnlyShapeInfo, tadZ.tadOffsets);
            }
