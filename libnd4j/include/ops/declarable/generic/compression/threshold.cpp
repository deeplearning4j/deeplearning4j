...
CUSTOM_OP_IMPL(encode_threshold, 1, 2, true, 1, 0) {
  ...
  REQUIRE_TRUE(x->lengthOf() <= DataTypeUtils::max<int>(), 0,
               "encode_threshold: gradients array must have length <= SD_MAX_INT");
  REQUIRE_TRUE(encoded->lengthOf() >= 4, 0,
               "encode_threshold: array for encoded updates can't have less than 4 elements");
  //            REQUIRE_TRUE(x->platformBuffer() == updated->platformBuffer(), 0, "encode_threshold: gradients array
  //            must be the same at input and output");
  ...
  if (encoded->lengthOf() == 4) return sd::Status::OK;
...
DECLARE_SHAPE_FN(encode_threshold) {
  ...
  return SHAPELIST(x->shapeInfo(),
                   sd::ConstantShapeHelper::getInstance().vectorShapeInfo(elements + 4, DataType::INT32));
}
...
CUSTOM_OP_IMPL(decode_threshold, 2, 1, true, 0, 0) {
  ...
  REQUIRE_TRUE(updates->lengthOf() == encoded->e<int>(1), 0,
               "decode_threshold: updates array must have length equal to [%i]", encoded->e<int>(1));
