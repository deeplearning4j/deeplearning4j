...
namespace nd4j {
    namespace ops {
        CUSTOM_OP_IMPL(segment_prod, 2, 1, false, 0, 0) {
            ...
            REQUIRE_TRUE(helpers::segmentIndicesValidate(idxSegments, expected, wrong), 0, "segment_prod: segment indices should be arranged, but %2.1f > %2.1f", expected.getScalar<float>(0), wrong.getScalar<float>(0));
