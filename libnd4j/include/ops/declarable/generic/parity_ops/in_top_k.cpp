...
namespace nd4j {
    namespace ops {
        ...
        DECLARE_SHAPE_FN(in_top_k) {
            ...
            if (shape::order(in) == 'c')
                shape::shapeBuffer(shape::rank(in), block.dataType(),  shape::shapeOf(in), newshape);
            else 
                shape::shapeBufferFortran(shape::rank(in), block.dataType(), shape::shapeOf(in), newshape);
