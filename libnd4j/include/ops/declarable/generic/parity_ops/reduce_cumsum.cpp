...
namespace ops {
    ...
    CUSTOM_OP_IMPL(reduce_cumsum, 1, 1, false, 0, 0) {
        NDArray<T>* input = INPUT_VARIABLE(0);
        NDArray<T>* output = OUTPUT_VARIABLE(0);
        std::vector<int> axes = *block.getIArguments();
        std::unique_ptr<NDArray<T>> inputCumulated(input->dup());
        std::unique_ptr<NDArray<T>> tempMean(input->dup());
        const bool keepDims = block.getTArguments()->size() > 0 ? (bool)T_ARG(0) : false;

        input->template reduceAlongDimension<simdOps::Mean<T>>(tempMean.get(), axes, keepDims);
        ...
        std::unique_ptr<ResultSet<T>> outList(NDArrayFactory<T>::allTensorsAlongDimension(inputCumulated.get(), dimensions));
        //output->
        for (Nd4jLong e = 0; e < outList->size(); ++e) {
            outList->at(e)->assign(tempMean.get());
        }
        inputCumulated->template applyPairwiseTransform<simdOps::ReverseSubtract<T>>(input, nullptr);
...
//            REQUIRE_TRUE(item > -input->shapeInfo()[0] || item <input->shapeInfo()[0], 0, "REDUCE_MEAN OP: the input dimension to reduce along must be in range (-%i, %i), but got %i instead !" , input->rankOf(), input->rankOf(), item);
        ...
        inputCumulated->template reduceAlongDimension<simdOps::Sum<T>>(output, axes, keepDims);

        return ND4J_STATUS_OK;
    }

    DECLARE_SHAPE_FN(reduce_cumsum) {    
    ...
    DECLARE_SHAPE_FN(reduce_cumsum_bp) {    
    ...
    CUSTOM_OP_IMPL(reduce_cumsum_bp, 2, 1, false, 0, 0) {
