...
//////////////////////////////////////////////////////////////////////
        CUSTOM_OP_IMPL(matmul, 2, 1, false, 0, -2) {
            ...
            if(x->isScalar() || x->rankOf() < 2) {
                nd4j_debug("Reshape x from rank %d\n",x->rankOf());
                x = new NDArray(x->reshape(x->ordering(),{1,1}));
                nd4j_debug("Post Reshape x from rank %d\n",x->rankOf());
            }

            if(y->isScalar() || y->rankOf() < 2) {
                nd4j_debug("Reshape y from rank %d\n",y->rankOf());
                y = new NDArray(y->reshape(y->ordering(),{1,1}));
                nd4j_debug("Reshape y from rank %d\n",y->rankOf());
            }
...
//////////////////////////////////////////////////////////////////////
        CUSTOM_OP_IMPL(matmul_bp, 3, 2, false, 0, -2) {
            nd4j_debug("In matmul_bp execution %d\n",0);
            auto x = INPUT_VARIABLE(0);
            auto y = INPUT_VARIABLE(1);
            auto eps = INPUT_VARIABLE(2);
            auto dldx = OUTPUT_VARIABLE(0);
            auto dldy = OUTPUT_VARIABLE(1);
            nd4j_debug("In matmul_bp execution obtained variables %d\n",0);
