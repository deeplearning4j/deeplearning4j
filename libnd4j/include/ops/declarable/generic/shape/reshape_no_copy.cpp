...
DECLARE_SHAPE_FN(reshape_no_copy) {
  ...
  // Note: do NOT special-case scalar.reshape(1) or scalar.reshape(-1) to preserve rank-0.
  // The standard behavior (matching NumPy/PyTorch) is that reshape([1]) always produces
  // a rank-1 array with shape [1], even when the input is a rank-0 scalar.
  // The only valid way to get a rank-0 scalar output from reshape is to pass an empty
  // shape specification (handled in the newShape.empty() block above).
