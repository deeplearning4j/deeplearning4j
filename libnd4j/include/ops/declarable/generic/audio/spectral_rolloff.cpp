...
CUSTOM_OP_IMPL(spectral_rolloff, 1, 1, false, -2, -2) {
    auto input = INPUT_VARIABLE(0);
    auto output = OUTPUT_VARIABLE(0);

    int sampleRate = block.numI() > 0 ? INT_ARG(0) : 22050;
    int fftSize = block.numI() > 1 ? INT_ARG(1) : 2048;
    double rolloffPercent = block.numT() > 0 ? T_ARG(0) : 0.85;

    REQUIRE_TRUE(input->rankOf() == 3, 0,
                 "spectral_rolloff: input must be rank 3 [batch, freqBins, numFrames], got %d", input->rankOf());
    REQUIRE_TRUE(sampleRate > 0, 0, "spectral_rolloff: sampleRate must be positive, got %d", sampleRate);
    REQUIRE_TRUE(fftSize > 0, 0, "spectral_rolloff: fftSize must be positive, got %d", fftSize);
    REQUIRE_TRUE(rolloffPercent > 0.0 && rolloffPercent <= 1.0, 0,
                 "spectral_rolloff: rolloffPercent must be in (0, 1]");

    helpers::spectralRolloff(block.launchContext(), input, sampleRate, fftSize, rolloffPercent, output);

    return sd::Status::OK;
}

DECLARE_SHAPE_FN(spectral_rolloff) {
