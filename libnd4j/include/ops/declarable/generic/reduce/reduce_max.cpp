...
DECLARE_TYPES(reduce_max) { getOpDescriptor()->setAllowedInputTypes(ANY)->setSameMode(true); }
...
CUSTOM_OP_IMPL(reduce_max_bp, -1, 1, false, 0, 0) {
  ...
  *gradI = 0;
  ...
  if (gradO->lengthOf() == 1) {
    auto indOfMaxElem = input->indexReduceNumber(indexreduce::IndexMax);
    gradI->p(indOfMaxElem.t<LongType>(0), gradO->e(0));
  } else {
    auto indicesArr = input->applyIndexReduce(indexreduce::IndexMax, &dimensions);
