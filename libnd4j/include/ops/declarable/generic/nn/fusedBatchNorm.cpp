...
CUSTOM_OP_IMPL(fused_batch_norm, 3, 1, false, 0, 2) {
    ...
    if(!isTraining){
        ...
        mean = new NDArray(scale->ordering(), shape, scale->dataType(), block.getWorkspace());
        variance = new NDArray(scale->ordering(), shape, scale->dataType(), block.getWorkspace());
    }
    ...
    NDArray xAffected(x->ordering(), {restSize, iD}, x->dataType(), block.getWorkspace());
    xAffected.assign(x);
