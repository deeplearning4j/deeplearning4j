...
CUSTOM_OP_IMPL(sru_bp, 8, 4, true, 0, 0) {
  ...
  //  x = x * mask
  if (applyMask) x->applyBroadcast(broadcast::Multiply, {0, 1}, *mask, *x);  // apply mask
  ...
  if (applyMask) gradX->applyBroadcast(broadcast::Multiply, {0, 1}, *mask, *gradX);  // apply mask
...
//     auto input   = INPUT_VARIABLE(0);                // X, input 3d tensor [bS x K x N], N - number of time steps, bS
//     - batch size, K - number of features auto weights = INPUT_VARIABLE(1);                // W, 2d tensor of weights
//     [3K x K] auto bias    = INPUT_VARIABLE(2);                // B, row of biases with twice length [1 x 2*K] auto
//     init    = INPUT_VARIABLE(3);                // C_{0}, 2d tensor of initial state [bS x K] at time t=0 NDArray*
//     mask    = nullptr;                          // optional,  2d tensor of dropout mask [bS x K]
...
//     for (int t = 0; t < N; ++t) {
//         xt = xmt({0,0, 0,0,     t,t+1}); xt.reshapei(xt.ordering(), {bS, K});       // [bS x  K x N] -> [bS x K x 1]
//         -> [bS x K] zt =  wi({0,0, 0,    K, t,t+1}); zt.reshapei(zt.ordering(), {bS, K});       // [bS x 3K x N] ->
//         [bS x K x 1] -> [bS x K] ft =  wi({0,0, K,  2*K, t,t+1}); ft.reshapei(ft.ordering(), {bS, K});       // [bS x
//         3K x N] -> [bS x K x 1] -> [bS x K] rt =  wi({0,0, 2*K,3*K, t,t+1}); rt.reshapei(rt.ordering(), {bS, K}); //
//         [bS x 3K x N] -> [bS x K x 1] -> [bS x K]
...
//     auto x   = INPUT_VARIABLE(0);                // X, input 3d tensor [bS x inSize x time], time - number of time
//     steps, bS - batch size, inSize - number of features auto w = INPUT_VARIABLE(1);                // W, 2d tensor of
//     weights [3K x inSize] auto b    = INPUT_VARIABLE(2);                // B, row of biases with twice length [1 x
//     2*inSize] auto c0    = INPUT_VARIABLE(3);                // C_{0}, 2d tensor of initial state [bS x inSize] at
//     time t=0 NDArray* mask    = nullptr;                          // optional,  2d tensor of dropout mask [bS x
//     inSize]
...
//     auto x        = INPUT_VARIABLE(0);                                   // X, input 3d tensor [bS x inSize x time],
//     time - number of time steps, bS - batch size, inSize - number of features auto w        = INPUT_VARIABLE(1); //
//     W, 2d tensor of weights [3*inSize x inSize] auto b        = INPUT_VARIABLE(2); // B, row of biases with twice
//     length [1 x 2*inSize] auto c0       = INPUT_VARIABLE(3);                                   // C_{0}, 2d tensor of
//     initial state [bS x inSize] at time t=0 auto c        = INPUT_VARIABLE(4);                                   //
//     C, [bS x inSize x time] auto inGradCt = INPUT_VARIABLE(5);                                   // [bS x inSize]
//     auto inGradH  = INPUT_VARIABLE(6);                                   // [bS x inSize x time]
//     auto mask     = block.width() > 7 ? INPUT_VARIABLE(7) : nullptr;     // optional,  2d tensor of dropout mask [bS
//     x inSize]
...
//     // input shapes validation
//     const int rank = 3;
//     REQUIRE_TRUE(x->rankOf()  == rank,   0, "SRU_BP operation: wrong rank of input array, expected is %i, but got %i
//     instead !", rank, x->rankOf()); REQUIRE_TRUE(w->rankOf()  == rank-1, 0, "SRU_BP operation: wrong rank of weights
//     array, expected is %i, but got %i instead !", rank-1, w->rankOf()); REQUIRE_TRUE(b->rankOf()  <= 2,      0,
//     "SRU_BP operation: wrong rank of biases  array, expected is <=2, but got %i instead !", b->rankOf());
//     REQUIRE_TRUE(c0->rankOf() == rank-1, 0, "SRU_BP operation: wrong rank of initial state array, expected is %i, but
//     got %i instead !", rank-1, c0->rankOf()); REQUIRE_TRUE(c->rankOf()  == rank,   0, "SRU_BP operation: wrong rank
//     of cell states array, expected is %i, but got %i instead !", rank, c->rankOf()); REQUIRE_TRUE(inGradCt->rankOf()
//     == rank-1, 0, "SRU_BP operation: wrong rank of array of cell state gradient, expected is %i, but got %i instead
//     !", rank-1, inGradCt->rankOf()); REQUIRE_TRUE(inGradH->rankOf()  == rank,   0, "SRU_BP operation: wrong rank of
//     array of cell outputs gradients, expected is %i, but got %i instead !", rank, inGradH->rankOf()); if(mask)
//         REQUIRE_TRUE(mask->rankOf() == rank-1, 0, "SRU_BP operation: wrong rank of mask array, expected is %i, but
//         got %i instead !", rank-1, mask->rankOf());
...
//     REQUIRE_TRUE(wShape  == wCorrectShape,  0, "SRU_BP operation: wrong shape of weights array, expected is %s, but
//     got %s instead !", wCorrectShape.c_str(), wShape.c_str());
//     // REQUIRE_TRUE(bShape  == bCorrectShape,  0, "SRU_BP operation: wrong shape of biases  array, expected is %s,
//     but got %s instead !", bCorrectShape.c_str(), bShape.c_str()); REQUIRE_TRUE(c0Shape == c0CorrectShape, 0, "SRU_BP
//     operation: wrong shape of initial state array, expected is %s, but got %s instead !", c0CorrectShape.c_str(),
//     c0Shape.c_str()); REQUIRE_TRUE(cShape == cCorrectShape, 0, "SRU_BP operation: wrong shape of cell states array,
//     expected is %s, but got %s instead !", cCorrectShape.c_str(), cShape.c_str()); REQUIRE_TRUE(inGradCtShape ==
//     inGradCtCorrectShape, 0, "SRU_BP operation: wrong shape of array of cell state gradient, expected is %s, but got
//     %s instead !", inGradCtCorrectShape.c_str(), inGradCtShape.c_str()); REQUIRE_TRUE(inGradHShape ==
//     inGradHCorrectShape, 0, "SRU_BP operation: wrong shape of array of cell outputs gradients, expected is %s, but
//     got %s instead !", inGradHCorrectShape.c_str(), inGradHShape.c_str()); if(mask) {
//         const std::string maskShape = ShapeUtils::shapeAsString(mask);
//         REQUIRE_TRUE(maskShape == c0CorrectShape, 0, "SRU_BP operation: wrong shape of mask array, expected is %s,
//         but got %s instead !", c0CorrectShape.c_str(), maskShape.c_str());
//     }

//     const auto bF = (*b)({0,0,  0,       inSize});                                 // biases for forget gate [1 x
//     inSize] const auto bR = (*b)({0,0,  inSize,2*inSize});                                 // biases for reset  gate
//     [1 x inSize] NDArray gradBias(x->ordering(),   {bS, 2*inSize, time}, x->dataType(), block.launchContext());
...
//         auto ct_1 = t ? (*c)({ 0,0, 0,0, t-1,t}) : *c0;                                                // previous
//         c_{t-1}
