...
CUSTOM_OP_IMPL(tri, -2, 1, false, 0, 1) {
  ...
  BUILD_SINGLE_SELECTOR(output->dataType(), output->fillAsTriangular, (1., diag + 1, 0, *output, 'l'),
                        SD_COMMON_TYPES);  // fill with unities lower triangular block of matrix
  BUILD_SINGLE_SELECTOR(output->dataType(), output->fillAsTriangular, (0., 0, diag, *output, 'u'),
                        SD_COMMON_TYPES);  // fill with zeros upper triangular block of matrix
