...
CUSTOM_OP_IMPL(lstmBlock, 9, 7, false, 2, 2) {
    ...
    REQUIRE_TRUE(W->sizeAt(0)==(inSize+numUnits), 0, "lstmBlock: Weights size(0) must be equal to inSize + numUnits, got %i", W->sizeAt(0));
    REQUIRE_TRUE(W->sizeAt(1)==(4*numUnits), 0, "lstmBlock: Weights size(1) must be equal to 4*numUnits, got %i", W->sizeAt(1));
    REQUIRE_TRUE(b->rankOf()==1 && b->sizeAt(0)==(4*numUnits), 0, "lstmBlock: Biases must be rank 1, size 4*numUnits");
...
DECLARE_SHAPE_FN(lstmBlock) {
    ...
    REQUIRE_TRUE(W[1]==(inSize+numUnits), 0, "lstmBlock: Weights size(0) must be equal to inSize + numUnits, got %i", W[1]);
    REQUIRE_TRUE(W[2]==(4*numUnits), 0, "lstmBlock: Weights size(1) must be equal to 4*numUnits, got %i", W[2]);
    REQUIRE_TRUE(shape::rank(b)==1 && b[1]==(4*numUnits), 0, "lstmBlock: Biases must be rank 1, size 4*numUnits");
	REQUIRE_TRUE(shape::rank(i)==3 && shape::rank(c)==3 && shape::rank(f)==3 && shape::rank(o)==3 && shape::rank(z)==3 && shape::rank(h)==3 && shape::rank(y)==3,
	             0, "lstmBlock: Output arrays must all be rank 3);
