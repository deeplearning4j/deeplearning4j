...
//
// dense → CSC sparse conversion op.
//
// Input:
...
// Outputs:
//   [0] cscValues  [nnz],     same float dtype as input
//   [1] cscRowIdx  [nnz],     INT32
//   [2] cscColPtr  [cols+1],  INT32
//
// nnz is DATA-DEPENDENT: DECLARE_SHAPE_FN computes it via a coherence-managed
// reduction (reduce::CountNonZero or reduce::MatchCondition mode-7), never
// via a manual syncToHost + host loop.
//
...
CUSTOM_OP_IMPL(dense_to_csc, 1, 3, false, 1, 0) {
  auto dense = INPUT_VARIABLE(0);

  REQUIRE_TRUE(dense->rankOf() == 2, 0,
               "dense_to_csc: input must be 2D (matrix), got rank %d", dense->rankOf());
...
DECLARE_SHAPE_FN(dense_to_csc) {
  ...
  const LongType cols = dense->sizeAt(1);
...
#endif  // NOT_EXCLUDED(OP_dense_to_csc)
