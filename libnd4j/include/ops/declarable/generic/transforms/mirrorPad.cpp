...
CUSTOM_OP_IMPL(mirror_pad, 2, 1, false, 0, 1) {
    ...
    if(input->rankOf() <= 1) {  // when in put scalar or vector;
        ...
        REQUIRE_TRUE( (paddings->getScalar<Nd4jLong>(0) <= (input->lengthOf() - includeBorder)) && (paddings->getScalar<Nd4jLong>(1) <= (input->lengthOf() - includeBorder)), 0, "MIRROR_PAD OP: wrong content of paddings array, its elements must be no grater then length of input array (being vector or scalar) for symmetric mode (or length-1 for reflect mode) !");
    }
    else {
        ...
        for(int i = 0; i < input->rankOf(); ++i)
            REQUIRE_TRUE( (paddings->getScalar<Nd4jLong>(i,0) <= (input->sizeAt(i) - includeBorder)) && (paddings->getScalar<Nd4jLong>(i,1) <= (input->sizeAt(i) - includeBorder)), 0, "MIRROR_PAD OP: wrong content of paddings array, its elements must be no grater then corresponding dimension of input array for symmetric mode (or dimension-1 for reflect mode) !");
    }
...
DECLARE_SHAPE_FN(mirror_pad) {
    ...
    outShapeInfo[0] = rank;
    if(rank == 1) {
        outShapeInfo[1] = input->lengthOf() + paddings->getScalar<Nd4jLong>(0) + paddings->getScalar<Nd4jLong>(1);
        outShapeInfo[2] = 1;
        outShapeInfo[3] = 0;
        outShapeInfo[4] = 1;
        outShapeInfo[5] = 99;
        ...
        for(int i = 0; i < rank; ++i)
            outShapeInfo[i+1] = input->sizeAt(i) + paddings->getScalar<Nd4jLong>(i,0) + paddings->getScalar<Nd4jLong>(i,1);
