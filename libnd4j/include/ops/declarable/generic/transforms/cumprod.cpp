...
CONFIGURABLE_OP_IMPL(cumprod, 1, 1, true, 0, 2) {
  ...
  if (block.getIArguments()->size() == 2 && block.width() == 1) {
    // all at once case
    helpers::prefix(block.launchContext(), scalar::Multiply, input, output, exclusive, reverse);
  } else {
...
CUSTOM_OP_IMPL(cumprod_bp, 2, 1, false, 0, 2) {
  ...
  if (!exclusive && !reverse) {
    if (dims.size())
      helpers::prefix(block.launchContext(), scalar::Add, &val, output, dims, true, false);
    else
      helpers::prefix(block.launchContext(), scalar::Add, &val, output, false, true);

  } else if (!exclusive && reverse) {
    if (dims.size())
      helpers::prefix(block.launchContext(), scalar::Add, &val, output, dims, false, false);
    else
      helpers::prefix(block.launchContext(), scalar::Add, &val, output, false, false);
  } else if (exclusive && !reverse) {
    if (dims.size())
      helpers::prefix(block.launchContext(), scalar::Add, &val, output, dims, true, true);
    else
      helpers::prefix(block.launchContext(), scalar::Add, &val, output, true, true);
  } else {
    if (dims.size())
      helpers::prefix(block.launchContext(), scalar::Add, &val, output, dims, true, false);
    else
      helpers::prefix(block.launchContext(), scalar::Add, &val, output, true, false);
  }
