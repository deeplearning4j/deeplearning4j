...
CUSTOM_OP_IMPL(mean_sqerr_loss, 3, 1, false, 0, 1) {
    ...
    if(!weights->isScalar())
    	for (int i = 0; i < weights->rankOf(); ++i)
        	REQUIRE_TRUE(!(weights->shapeOf()[i] != labels->shapeOf()[i] && weights->shapeOf()[i] != 1), 0, "MEAN_SQERR_LOSS OP: shape of weights array %s is not broadcastable to labels array shape %s !", ShapeUtils::shapeAsString(weights).c_str(), ShapeUtils::shapeAsString(labels).c_str());
	...
	NDArray weightedLosses(labels->getShapeInfo(), false, block.getWorkspace());
