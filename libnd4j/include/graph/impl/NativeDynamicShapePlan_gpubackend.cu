...
static void cleanupCaptureTlsState(bool freeHostPtrs, void* prevCaptureStream) {
  ...
  // Clear outer composite-capture scope stream — any composite capture (success or
  // abort) ends here, so async ops in subsequent slot-by-slot execution must not
  // see this stream.
...
// Force-terminate any in-progress CUDA stream capture on `stream`. Mirrors the
// abortCapture body: the CUDA runtime is the authority on capture state — the
// handle's endCapture() is state-gated and can refuse while the stream is
// still capturing, which would poison every later launch on that stream.
static void terminateActiveStreamCapture(cudaStream_t stream, const char* site) {
  if (stream == nullptr) return;
  cudaStreamCaptureStatus capStatus = cudaStreamCaptureStatusNone;
  cudaError_t capErr = cudaStreamGetCaptureInfo_v2(
      stream, &capStatus, nullptr, nullptr, nullptr, nullptr);
  if (capErr != cudaSuccess || capStatus == cudaStreamCaptureStatusNone) return;
  cudaGraph_t discardedGraph = nullptr;
  cudaError_t endErr = cudaStreamEndCapture(stream, &discardedGraph);
  if (endErr == cudaSuccess && discardedGraph != nullptr) {
    cudaGraphDestroy(discardedGraph);
  }
  if (endErr != cudaSuccess) cudaGetLastError();  // clear sticky error
  DSP_DIAG(EXECUTE, "terminateActiveStreamCapture[%s]: ended orphaned stream "
           "capture (status=%d endErr=%d) — stream unpoisoned",
           site, (int)capStatus, (int)endErr);
}
...
void NativeDynamicShapePlan::abortCapture(GraphSegment& seg,
  ...
  // ── ALWAYS terminate an in-progress stream capture before cleanup ──────
  // Failure paths can reach this helper with cudaStreamBeginCapture still
  // active on the work stream (e.g. capture-workspace OOM mid-island). If the
  // stream is left capturing, EVERY later launch on it is illegal (CUDA 901)
  // and TritonGraphBackend::executeSegment sees cudaStreamIsCapturing==Active
  // forever — decode silently no-ops in an endless capture pass.
  {
    cudaStream_t abortStream = (stream != nullptr)
        ? *static_cast<cudaStream_t*>(stream)
        : nullptr;
    if (abortStream == nullptr) {
      auto* defaultStreamPtr = LaunchContext::defaultContext()->getCudaStream();
      if (defaultStreamPtr != nullptr) abortStream = *defaultStreamPtr;
    }
...
static void dumpSegFinalArgmax(const GraphSegment& seg,
  ...
  if (finalOutputSlot < 0 || finalOutputSlot >= totalOutputSlots) {
    finalOutputSlot = dsp::firstSegmentOutputSlot(
        slots, numSlots, seg.def.startSlot, seg.def.endSlot, totalOutputSlots);
  }
  if (finalOutputSlot < 0 || finalOutputSlot >= totalOutputSlots ||
...
static bool instantiateAndStoreMergedCapture(
  ...
  DSP_DIAG(EXECUTE, "%s: group=%d [%d-%d] INSTANTIATE_BEGIN nodes=%zu "
  ...
  // ── Pre-instantiate: verify stream is NOT still capturing ────────────
  {
    ...
    if (capErr != cudaSuccess || capStat != cudaStreamCaptureStatusNone) {
      DSP_DIAG(EXECUTE, "%s: group=%d ABORT — stream %p still in capture mode "
               "(capStat=%d capErr=%d). Cannot instantiate.",
  ...
  // The pre-capture SBS warmup has already executed the complete segment and
  ...
  DSP_DIAG(EXECUTE, "%s: group=%d [%d-%d] INSTANTIATE_COMPLETE nodes=%zu graphExec=%p "
           ...
           diagPrefix, mergedGroupId, startSlot, endSlot, nodeCount,
           (void*)nativeHandle->getGraphExec());
...
static Status setGpuBackendFailureDetail(GraphSegment& seg, const std::string& detail) {
  ...
  if (existing == nullptr || existing[0] == '\0') {
    std::string message = detail + " [GPU graph segment " +
...
static bool isGapRangeCaptureSafe(NativeSlot* slots, int startSlot, int endSlot, bool mergeViews) {
  ...
  // exercised for any otherwise capturable gap.
...
Status NativeDynamicShapePlan::compositeReplay(
  ...
  if (!syncResult.ok() || syncResult.effectiveExternals == nullptr) {
    ...
    return setGpuBackendFailureDetail(
        seg, "composite replay input staging failed: syncStatus=" +
                 std::to_string(static_cast<int>(syncResult.status)) +
                 ", cudaError=" + std::to_string(syncResult.cudaError) +
                 " (" + cudaGetErrorString(
  ...
  if ((!seg.exec.needsArgRefresh() || mergedAddressValidationRequired) &&
    ...
    bool skipAddrCheck = !mergedAddressValidationRequired &&
                         !hasDeviceManagedExternalInputs(effectiveExternals, numExt) &&
  ...
  if (!driftDetected &&
    ...
    bool skipSlotCheck = !mergedAddressValidationRequired &&
                         (seg.exec.slotAddrStableCount >= kAddrStableSkipThreshold) &&
                         ((seg.exec.executionCount % kAddrRecheckInterval) != 0);
    if (skipSlotCheck) {
  ...
  // GDR_DRIFT_TRIPWIRE: the wiring-walk hash (computeSegmentSlotAddrHash) visits
  // only slots reachable via op output wiring. If wiring is incomplete at this
  // point (null outputSlotIndices after a boundary clear / plan rebuild), the
  // hash degenerates to the bare FNV offset basis and "validates" as stable —
  ...
  if (!driftDetected &&
      ...
      seg.exec.capturedSlotAddrHash != 0) {
    int visitedSlots = 0;
    int wiringNullSlots = 0;
    dsp::forEachSegmentOutputSlot(
        slots_, numSlots_, seg.def.startSlot, seg.def.endSlot,
        totalOutputSlots_, [&](int opIndex, int, int) {
          visitedSlots++;
          const auto& w = slots_[opIndex].wiring;
          if (w.outputSlotIndices == nullptr || w.numOutputs <= 0) wiringNullSlots++;
        });
    if (visitedSlots == 0) {
      driftDetected = true;
      seg.exec.markArgsStale();
      DSP_DIAG(EXECUTE,
               "GDR_DRIFT_TRIPWIRE: seg[%d-%d] wiring-walk visited ZERO output slots "
               "in range width=%d — hash is vacuous, forcing arg refresh (execCount=%d)",
               seg.def.startSlot, seg.def.endSlot,
               seg.def.endSlot - seg.def.startSlot + 1,
               seg.exec.executionCount);
    } else if (wiringNullSlots > 0) {
      DSP_DIAG(EXECUTE,
               "GDR_DRIFT_TRIPWIRE_PARTIAL: seg[%d-%d] %d of %d visited slots have "
               "null output wiring — hash coverage incomplete (execCount=%d)",
               seg.def.startSlot, seg.def.endSlot, wiringNullSlots, visitedSlots,
               seg.exec.executionCount);
    }
  }
  ...
  if (!useFastReplay) {
   ...
   if (tritonBackend != nullptr) {
     ...
     seg.exec.recordReplayAddressKeys(
         computeSegmentInputAddrKey(seg, effectiveExternals, numExt),
         computeSlotAddrHash(slots_, numSlots_, outputSlots_, seg.def.startSlot,
                             seg.def.endSlot, totalOutputSlots_),
         "composite_replay_arg_refresh");
   }
 }
#endif
    ...
    if (unit.mergedGroupId >= 0 && unit.isMergedLeader) {
      ...
      if (gapSlotsExecutedSinceArgCopy) {
        ...
        if (tritonBackend2 != nullptr) {
          Status refreshStatus = tritonBackend2->refreshArgTablesForReplay(
              seg, effectiveExternals, numExt, outputSlots_, totalOutputSlots_, stream);
          if (refreshStatus != Status::OK) {
            seg.exec.markArgsStale();
            DSP_DIAG(EXECUTE,
                     "COMPOSITE_REPLAY: post-gap arg-table refresh failed status=%s (%d) "
                     "seg[%d-%d] — aborting before next replay unit",
                     statusName_gpu(refreshStatus), static_cast<int>(refreshStatus),
                     seg.def.startSlot, seg.def.endSlot);
            return refreshStatus;
          }
      ...
      if (mgId < 0 || mgId >= static_cast<int>(sched.mergedReplayHandles.size()) ||
        ...
        int fallbackStart = unit.startSlot;
        int fallbackEnd = unit.endSlot;
        if (mgId >= 0 && mgId < static_cast<int>(sched.mergedGroupSlotRanges.size())) {
          fallbackStart = sched.mergedGroupSlotRanges[mgId].minSlot;
          fallbackEnd = sched.mergedGroupSlotRanges[mgId].maxSlot;
        }
        DSP_DIAG(EXECUTE,
                 "MERGED_REPLAY: group %d handle not ready — executing full group "
                 "range [%d-%d] slot-by-slot",
                 mgId, fallbackStart, fallbackEnd);
        // A merged handle represents the complete island+gap group. If it is
        // unavailable, execute every slot in that full range in program order;
        // running only the leader silently omits the group's other islands/gaps.
        for (int s = fallbackStart; s <= fallbackEnd; s++) {
          if (s < 0 || s >= numSlots_) continue;
          auto slotStatus = executeSlot(s, effectiveExternals, numExt, stream);
          if (slotStatus != Status::OK) {
            DSP_DIAG(EXECUTE,
                     "MERGED_REPLAY: fallback slot %d FAILED status=%s (%d)",
                     s, statusName_gpu(slotStatus), static_cast<int>(slotStatus));
            setGpuBackendFailureDetail(
                seg, "merged replay fallback slot " + std::to_string(s) +
                         " (" + slots_[s].ident.opName + ") returned " +
                         statusName_gpu(slotStatus) + " (" +
    ...
    if (unit.kind == REPLAY_UNIT_GAP) {
        ...
        for (auto& active : cachedSlots) {
          switch (active.action) {
            ...
            case ActiveSlotAction::BATCHED_GEMM: {
              ...
              // (stream VALUE), so pass cudaStr (the dereferenced value), NOT stream (the
              // raw cudaStream_t* pointer). Using the pointer address as a stream handle
              // causes an invalid-resource CUDA error on the cast kernel launch →
              // context poison → SIGSEGV on the next cudaLaunchKernel.
              Status batchStatus = executeBatchedGemmGroup(active.batchedGemmGroupIdx, effectiveExternals, numExt, static_cast<void*>(cudaStr));
            ...
            case ActiveSlotAction::EXECUTE: {
              ...
              if (useGapFastPath) {
                ...
                if (needsShapeAwareGapExec && executeCount_ >= 5) {
                  DSP_DIAG_SLOT(SHAPE, active.slotIdx,
                      "GAP_DYNAMIC_FULL_EXEC: slot=%d op=%s executeCount=%d "
                      "isDynamic=%d valueShape=%d",
                      active.slotIdx, activeSlot.ident.opName.c_str(), executeCount_,
                      activeSlot.flags.isDynamicShape ? 1 : 0,
              ...
              if (slotStatus != Status::OK) {
                DSP_DIAG(EXECUTE,
                         ...
                         active.slotIdx, statusName_gpu(slotStatus),
                         static_cast<int>(slotStatus));
                setGpuBackendFailureDetail(
                    seg, "composite replay gap slot " +
                             std::to_string(active.slotIdx) + " (" +
                             slots_[active.slotIdx].ident.opName + ") returned " +
                             statusName_gpu(slotStatus) + " (" +
                             std::to_string(static_cast<int>(slotStatus)) + ")");
                return slotStatus;  // replayGuard restores tl_dspReplayActive
              }
              if (needsShapeAwareGapExec &&
                  slotOutputBuffersChanged(activeSlot, outputSlots_, totalOutputSlots_,
                                           shapeAwareOutputBufsBefore,
                                           NativeDynamicShapePlan::MAX_OUTPUTS_PER_SLOT)) {
                gapOutputPointersChanged = true;
                DSP_DIAG_SLOT(SHAPE, active.slotIdx,
                    "GAP_DYNAMIC_OUTPUT_REBOUND: slot=%d op=%s executeCount=%d",
                    active.slotIdx, activeSlot.ident.opName.c_str(), executeCount_);
              }
      ...
      for (int s = unit.startSlot; s <= unit.endSlot; s++) {
        ...
        if (!batchedGemmGroups_.empty() && s < (int)slotToBatchedGemmGroup_.size()) {
          ...
          if (bgIdx >= 0 && bgIdx < (int)batchedGemmGroups_.size()) {
            ...
            if (s == bgGroup.triggerSlot) {
              // executeBatchedGemmGroup expects stream VALUE not pointer — pass cudaStr.
              // Matches the fix in the fast-path (ActiveSlotAction::BATCHED_GEMM above)
              // and the comment in _segments.cpp:1180-1182.
              Status batchStatus = executeBatchedGemmGroup(bgIdx, effectiveExternals, numExt, static_cast<void*>(cudaStr));
              if (batchStatus != Status::OK) {
                DSP_DIAG(EXECUTE,
                         "COMPOSITE_REPLAY: batched GEMM group %d FAILED at slot %d status=%s (%d)",
                         bgIdx, s, statusName_gpu(batchStatus),
                         static_cast<int>(batchStatus));
                setGpuBackendFailureDetail(
                    seg, "composite replay batched GEMM group " +
                             std::to_string(bgIdx) + " at slot " +
                             std::to_string(s) + " returned " +
                             statusName_gpu(batchStatus) + " (" +
                             std::to_string(static_cast<int>(batchStatus)) + ")");
                return batchStatus;  // replayGuard restores tl_dspReplayActive
              }
        ...
        if (!viewFastpathDisabled &&
            ...
            !slots_[s].hasValueDependentShape() &&
        ...
        if (slotStatus != Status::OK) {
          DSP_DIAG(EXECUTE,
                   ...
                   s, statusName_gpu(slotStatus), static_cast<int>(slotStatus));
          setGpuBackendFailureDetail(
              seg, "composite replay gap slot " + std::to_string(s) +
                       " (" + slots_[s].ident.opName + ") returned " +
                       statusName_gpu(slotStatus) + " (" +
                       std::to_string(static_cast<int>(slotStatus)) + ")");
          return slotStatus;  // replayGuard restores tl_dspReplayActive
      ...
      if (gapSlotsExecutedSinceArgCopy) {
        ...
        auto* tritonBackend2 =
            dynamic_cast<TritonGraphBackend*>(seg.resolvedGraphBackend);
        if (tritonBackend2 != nullptr) {
          auto refreshStatus = tritonBackend2->refreshArgTablesForReplay(
              seg, effectiveExternals, numExt, outputSlots_, totalOutputSlots_, stream);
          if (refreshStatus != Status::OK) {
            seg.exec.markArgsStale();
            DSP_DIAG(EXECUTE,
                     "COMPOSITE_REPLAY: unmerged post-gap arg-table refresh failed "
                     "status=%s (%d) seg[%d-%d] — aborting before next replay unit",
                     statusName_gpu(refreshStatus), static_cast<int>(refreshStatus),
                     seg.def.startSlot, seg.def.endSlot);
            return refreshStatus;
          }
      ...
      if (idx < 0 || idx >= static_cast<int>(sched.compositeReplayHandles.size()) ||
        ...
        for (int s = unit.startSlot; s <= unit.endSlot; s++) {
          ...
          if (slotStatus != Status::OK) {
            DSP_DIAG(EXECUTE,
                     "COMPOSITE_REPLAY: island fallback slot %d FAILED status=%s (%d)",
                     s, statusName_gpu(slotStatus), static_cast<int>(slotStatus));
            setGpuBackendFailureDetail(
                seg, "composite replay island fallback slot " +
                         std::to_string(s) + " (" + slots_[s].ident.opName +
                         ") returned " + statusName_gpu(slotStatus) + " (" +
                         std::to_string(static_cast<int>(slotStatus)) + ")");
            return slotStatus;
          }
        }
  ...
  if (Environment::getInstance().tritonForceRecapture()) {
    ...
    markBatchD2DInvalidated(seg, "force_recapture_post_composite_replay");
...
void NativeDynamicShapePlan::markReplayInvariantInvalidatedForDispatch(
    GraphSegment& seg, const char* reason,
    bool& extAddrsStable, bool& hasReplayHandle,
    bool& replayHandleNull, bool& hasComposite) {
  DSP_DIAG(EXECUTE,
           "REPLAY_INVARIANT_INVALIDATED: seg[%d-%d] reason=%s "
           "before{batchD2D=%d extAddrsStable=%d hasReplayHandle=%d "
           "replayHandleNull=%d hasComposite=%d} -> "
           "after{batchD2D=0 extAddrsStable=0 hasReplayHandle=0 "
           "replayHandleNull=1 hasComposite=0}",
           seg.def.startSlot, seg.def.endSlot, reason ? reason : "?",
           batchD2DCount_, extAddrsStable ? 1 : 0,
           hasReplayHandle ? 1 : 0, replayHandleNull ? 1 : 0,
           hasComposite ? 1 : 0);
  markBatchD2DInvalidated(seg, reason);
...
void NativeDynamicShapePlan::markBatchD2DInvalidated(GraphSegment& seg, const char* reason) {
  DSP_DIAG(EXECUTE,
           "BATCH_D2D_INVALIDATED: seg[%d-%d] reason=%s batchD2DCount %d -> 0",
           seg.def.startSlot, seg.def.endSlot, reason ? reason : "?",
           batchD2DCount_);
  batchD2DCount_ = 0;
}

void NativeDynamicShapePlan::markGapOpsCapturedInGraph(
    GraphSegment& seg, bool captured, const char* reason) {
  DSP_DIAG(EXECUTE,
           "GAP_OPS_CAPTURED_FLAG: seg[%d-%d] reason=%s gapOpsCapturedInGraph %d -> %d",
           seg.def.startSlot, seg.def.endSlot, reason ? reason : "?",
           seg.exec.gapOpsCapturedInGraph ? 1 : 0, captured ? 1 : 0);
  seg.exec.gapOpsCapturedInGraph = captured;
}

void NativeDynamicShapePlan::markStaleGapGraphInvalidatedForDispatch(
    GraphSegment& seg, const char* reason, int captureMinExec,
    bool& extAddrsStable, bool& hasReplayHandle,
    bool& replayHandleNull, bool& isTritonCompiled) {
  DSP_DIAG(EXECUTE,
           "STALE_GAP_GRAPH_DISPATCH_RESET: seg[%d-%d] reason=%s "
           "before{batchD2D=%d gapOpsCaptured=%d executionCount=%d "
           "extAddrsStable=%d hasReplayHandle=%d replayHandleNull=%d "
           "isTritonCompiled=%d} -> after{batchD2D=0 gapOpsCaptured=0 "
           "executionCount=%d extAddrsStable=0 hasReplayHandle=0 "
           "replayHandleNull=1 isTritonCompiled=0}",
           seg.def.startSlot, seg.def.endSlot, reason ? reason : "?",
           batchD2DCount_, seg.exec.gapOpsCapturedInGraph ? 1 : 0,
           seg.exec.executionCount, extAddrsStable ? 1 : 0,
           hasReplayHandle ? 1 : 0, replayHandleNull ? 1 : 0,
           isTritonCompiled ? 1 : 0, captureMinExec);
  markBatchD2DInvalidated(seg, reason);
  markGapOpsCapturedInGraph(seg, false, reason);
...
Status NativeDynamicShapePlan::segDispatchReplay(
  ...
  // baked into the CUDA graph and must NOT be re-run live via compositeReplay.
  // The capture-path guard (ctx.nativeOnlyGraphCapture at line 3310) prevents composite
  // handles from being created for monolithic segments; this belt-and-suspenders check
  // catches any residual handles that might survive a plan rebuild or lifecycle edge case.
  //
  // RESTORE green (5f3f34b2a4) MONOLITHIC_REPLAY_GAP_DEMOTE invariant: a composite-captured
  // segment with LIVE cuBLAS gap slots (replayStability.gapSlotCount()>0) MUST replay via
  // composite so the gap matmuls re-run live with this step's KV/hidden values. The DSP/Triton
  // dispatch refactor let an erroneously-set hasGapsInGraph() (e.g. via the zero-node
  // native-capture fallback) drop such a segment into monolithic replay of capture-time baked
  // gaps → stale KV/hidden at step 6+ → wrong tokens (regression vs green). gapSlotCount() is
  // the cached structural live-gap count, so this adds no per-step recompute / perf cost.
  const bool segmentHasLiveGaps = seg.exec.replayStability.gapSlotCount() > 0;
  bool compositeReplayReady = hasComposite && (!seg.exec.hasGapsInGraph() || segmentHasLiveGaps);

  DSP_DIAG(EXECUTE, "COMPOSITE_REPLAY_READY_CHECK: seg[%d-%d] compositeReplayReady=%d "
                    "hasCompositeHandles=%d gapsInGraph=%d compiledBy='%s' execCount=%d",
           seg.def.startSlot, seg.def.endSlot, compositeReplayReady ? 1 : 0,
           hasComposite ? 1 : 0, seg.exec.hasGapsInGraph() ? 1 : 0,
  ...
  if (allowTritonCudaGraphReplay &&
    ...
    // Pre-launch merged-graph drift requests a segment-local rebuild. Keep the
    // transition atomic inside this dispatch: invalidate the stale capture and
    // compiled artifact, run one coherent warmup for the current invocation, then
    // immediately rebuild the owner-scoped backend artifact before later segments run.
    if (replayStatus == Status::MAYBE) {
      DSP_DIAG(EXECUTE,
               "COMPOSITE_REPLAY_REBUILD: seg[%d-%d] pre-launch address drift — "
               "running bounded warmup and immediate recompile",
               seg.def.startSlot, seg.def.endSlot);
      handled = true;
      return rebuildSegmentAfterPreLaunchReplayDrift(
          seg, externalArrays, numExt, stream, "merged_graph_addr_drift");
    }

    // A different KERNEL_FAILURE may already have performed full invalidation.
    // Fail closed instead of falling through to capture/direct with an erased
    // owner-scoped backend artifact; the original failure remains observable.
    if (replayStatus == Status::KERNEL_FAILURE && seg.exec.replayHandle == nullptr) {
      DSP_DIAG(EXECUTE,
               "COMPOSITE_REPLAY_INVALIDATED: seg[%d-%d] graph and compiled artifact "
               "were invalidated during replay — failing closed",
               seg.def.startSlot, seg.def.endSlot);
      handled = true;
      return replayStatus;
    }
    // Any remaining KERNEL_FAILURE may follow a partially executed replay schedule.
    // Re-running the segment can duplicate in-place KV/recurrent updates, so fail closed.
    if (replayStatus == Status::KERNEL_FAILURE) {
      DSP_DIAG(EXECUTE,
               "COMPOSITE_REPLAY_KERNEL_FAILURE: seg[%d-%d] replay failed with handles intact — "
               "failing closed because replay progress is not retry-safe",
               seg.def.startSlot, seg.def.endSlot);
      handled = true;
      return replayStatus;
    }
  ...
  // When createOpsExcludedFromGraph=true, create ops run live before cudaGraphLaunch
  // (see replayMonolithicGraph step 0.5), so their input values changing per decode
  // step is safe.  The createValuesStable check is bypassed in that case.
  const bool createStableForReplay = createValuesStable || seg.exec.createOpsExcludedFromGraph;
  if (allowTritonCudaGraphReplay &&
      seg.exec.replayHandle != nullptr &&
      seg.exec.replayHandle->isReady() &&
      seg.exec.cachedShapeKey == segShapeKey &&
      createStableForReplay &&
      extAddrsStable) {

    DSP_DIAG(EXECUTE, "MONOLITHIC_REPLAY_ENTER: seg[%d-%d] replayHandle=%p execCount=%d "
             "(createValuesStable=%d createOpsExcluded=%d)",
             seg.def.startSlot, seg.def.endSlot,
             (void*)seg.exec.replayHandle.get(), seg.exec.executionCount,
             createValuesStable ? 1 : 0, seg.exec.createOpsExcludedFromGraph ? 1 : 0);
    ...
    if (!syncResult.ok() || syncResult.effectiveExternals == nullptr) {
      DSP_DIAG(EXECUTE,
               ...
               static_cast<int>(syncResult.status), syncResult.cudaError);
      return setGpuBackendFailureDetail(
          seg, "monolithic replay input staging failed: syncStatus=" +
                   std::to_string(static_cast<int>(syncResult.status)) +
                   ", cudaError=" + std::to_string(syncResult.cudaError) +
                   " (" + cudaGetErrorString(
                       static_cast<cudaError_t>(syncResult.cudaError)) + ")");
    ...
    if (replayStatus == Status::MAYBE) {
      handled = true;
      return rebuildSegmentAfterPreLaunchReplayDrift(
          seg, externalArrays, numExt, stream, "monolithic_slot_addr_drift");
    }
    if (replayStatus != Status::OK) {
      DSP_DIAG(EXECUTE,
               "MONOLITHIC_REPLAY_FAILED: seg[%d-%d] status=%s (%d) — failing closed",
               seg.def.startSlot, seg.def.endSlot,
 ...
 if (ctx.tritonBackend != nullptr) {
   TritonGraphBackend::setOrderedRangeExecutor(
       [this, &seg, externalArrays, numExt, stream](int startSlot, int endSlot) -> Status {
         ...
         // Ground-truth via the authority (DebugHelper.h), not an inline cudaStreamIsCapturing.
         bool streamIsCapturing = false;
         if (stream != nullptr) {
           cudaStream_t capQueryStream = *static_cast<cudaStream_t*>(stream);
           streamIsCapturing = DebugHelper::streamIsCapturing(&capQueryStream);
         }
  ...
  if (shouldCaptureTritonGraphNow) {
    ...
    // Create the replayHandle BEFORE capture — it must exist to store workspace, host ptrs, etc.
    {
      int deviceId = 0;
      cudaGetDevice(&deviceId);
      seg.exec.replayHandle = GraphReplayFactory::create(ReplayBackend::CUDA, deviceId);
    }
    ...
    if (seg.exec.replayHandle == nullptr) {
      ...
      // line 488-490) which runs executeSegmentSlotBySlot() before capture.
      {
        ...
        // (composite replay writes device, ticks special-actual but not
        ...
        if (warmupStatus != Status::OK) {
          DSP_DIAG(EXECUTE, "FATAL: Triton pre-capture warmup FAILED for seg[%d-%d] status=%s (%d). "
                            "BLOCKING EXECUTION.",
                   seg.def.startSlot, seg.def.endSlot,
                   statusName_gpu(warmupStatus), static_cast<int>(warmupStatus));
      ...
      // Build the composite replay schedule BEFORE the willUseCompositeCapture check and the
      // composite-capture gate (~3296). Without this the schedule is EMPTY on the FIRST
      // capture attempt — it was only built post-capture at ~4871 / in the direct-exec path
      // at ~5162 — so hasIslandUnits=false → composite capture is SKIPPED → the segment falls
      // back to the BANNED monolithic-with-gaps capture, whose baked cuBLAS-gap values churn
      // every step → createValuesStable=false → invalidateForRebuild every step → the plan
      // never reaches REPLAYING (permanent slot-by-slot ~8 tok/s). Valid here: segDispatchCompile
      // and the pre-capture warmup have completed, so getGapSlots() sees the compiled
      // sub-kernels. Regression: df8cee5d5f removed the pre-gate build, leaving only the
      // post-monolithic-capture one.
#if HAVE_TRITON
      // Skip building the composite schedule when monolithic (native-only) capture is
      // used (ctx.nativeOnlyGraphCapture=true). Monolithic capture bakes ALL ops —
      // including gap matmuls — into a single CUDA graph; there are no islands to
      // replay separately. If we pre-build the schedule here, units becomes non-empty,
      // and platformTryFrozenFastPath's `!units.empty()` check incorrectly fires
...
#if HAVE_TRITON
      {
        ...
        if (!stagingResult.ok() || stagingResult.effectiveExternals == nullptr) {
          ...
          SegmentLifecycle::markFailed(
              seg.exec, "composite_capture_staging_failed",
              seg.def.startSlot, seg.def.endSlot);
          return setGpuBackendFailureDetail(
              seg, "pre-composite-capture input staging failed: syncStatus=" +
                       std::to_string(static_cast<int>(stagingResult.status)) +
                       ", cudaError=" + std::to_string(stagingResult.cudaError) +
                       " (" + cudaGetErrorString(
                           static_cast<cudaError_t>(stagingResult.cudaError)) + ")");
        }
...
#if HAVE_TRITON
      {
       ...
       if (!sched.units.empty() && !didCompositeCapture &&
         ...
         // CaptureLifecycleGuard::deactivate() clears tl_graphExecutionActive and
         // tl_graphCaptureStream between merged groups, so captureSafeStreamOrDefault()
         // in ConstantHelper/PointersManager/DataBuffer/NDArray would otherwise fall
         // back to cudaStreamPerThread for slots executing BETWEEN merged groups.
         // tl_compositeCaptureStream = resolved main stream keeps those async ops on the
         // main execution stream throughout the composite capture.
         // cleanupCaptureTlsState() (called from both success and abortCapture paths)
         // sets tl_compositeCaptureStream = nullptr, so no explicit clear is needed here.
         {
           cudaStream_t resolvedCompositeStream = ctx.cudaStr;
           if (resolvedCompositeStream == nullptr) {
             auto* defaultStreamPtr = LaunchContext::defaultContext()->getCudaStream();
             if (defaultStreamPtr != nullptr) resolvedCompositeStream = *defaultStreamPtr;
           }
           tl_compositeCaptureStream = static_cast<void*>(resolvedCompositeStream);
           DSP_DIAG(EXECUTE, "COMPOSITE_CAPTURE: set tl_compositeCaptureStream=%p "
                    "(ctx.cudaStr=%p resolved=%p)",
                    (void*)resolvedCompositeStream, (void*)ctx.cudaStr, (void*)resolvedCompositeStream);
         }
           ...
           auto& unit = sched.units[unitIdx];
            ...
            if (unit.kind == REPLAY_UNIT_GAP) {
               ...
               for (int s = unit.startSlot; s <= unit.endSlot; s++) {
                 ...
                 if (slots_[s].frozenConstantSlot()) {
                  ...
                  if (gapStatus != Status::OK) {
                    DSP_DIAG(EXECUTE,
                             "MERGED_CAPTURE: gap slot %d FAILED status=%s (%d)",
                ...
                if (!gapOk) {
                  ...
                  // Belt-and-braces: the handle may have refused endCapture on a
                  // stale state while the STREAM itself is still capturing —
                  // sweep it so the shared work stream is never left poisoned.
                  terminateActiveStreamCapture(ctx.cudaStr, "gap_abort");
             ...
             if (!captureActive) {
               ...
               auto newHandle = GraphReplayFactory::create(ReplayBackend::CUDA, deviceId);
               if (!newHandle) {
             ...
             if (captureStatus == Status::OK && keepCaptureOpen) {
              ...
              DSP_DIAG(EXECUTE, "MERGED_CAPTURE_END_MIDLOOP: island=%d group=%d [%d-%d] "
                       "captureStatus=%s (%d) keepCaptureOpen=%d — ending merged capture",
                       islandIdx, mergedGroupId, mergedStartSlot, mergedEndSlot,
                       statusName_gpu(captureStatus), static_cast<int>(captureStatus),
                       (int)keepCaptureOpen);
             mergedCapGuard.deactivate();
             ...
             if (captureStatus != Status::OK) {
               DSP_DIAG(COMPILE,
                        "COMPOSITE_CAPTURE_FAIL: island captureStatus=%s (%d) at group=%d [%d-%d]",
                        statusName_gpu(captureStatus), static_cast<int>(captureStatus),
                        mergedGroupId, mergedStartSlot, mergedEndSlot);
               if (mergedNativeHandle->isCapturing()) {
                 mergedNativeHandle->endCapture(ctx.cudaStr);
               }
               // Sweep the stream: handle state may be stale while the CUDA
               // stream is still capturing (OOM paths reset the handle only).
               terminateActiveStreamCapture(ctx.cudaStr, "island_fail");
               allIslandsOk = false;
               const char* backendError =
                   LaunchContext::defaultContext()->errorReference()->errorMessage();
               const std::string islandContext =
                   " [island=" + std::to_string(islandIdx) + " group=" +
                   std::to_string(mergedGroupId) + " range=" +
                   std::to_string(mergedStartSlot) + "-" +
                   std::to_string(mergedEndSlot) + " status=" +
                   statusName_gpu(captureStatus) + " (" +
                   std::to_string(static_cast<int>(captureStatus)) + ")]";
               setCompositeCaptureFailureDetail(
             ...
             // End merged capture, instantiate, and store for scheduled replay
             bool endOk = mergedNativeHandle->endCapture(ctx.cudaStr);
             size_t nodeCount = endOk ? mergedNativeHandle->getNumNodes() : 0;
             DSP_DIAG(EXECUTE, "MERGED_CAPTURE_END: group=%d [%d-%d] endCapture=%d nodes=%zu",
                      mergedGroupId, mergedStartSlot, mergedEndSlot, endOk ? 1 : 0, nodeCount);
         ...
         if (allIslandsOk) {
           ...
           seg.exec.replayUnitCount = static_cast<int>(sched.units.size());
            ...
            //   - isSealed(): plan cache returned a previously captured+sealed segment;
            //     re-capturing updates handles but must NOT re-fire the lifecycle transition.
            //   - else (WARMUP/COMPILING): the segment was invalidated/reset and never
            //     reached CAPTURING → must NOT be sealed (see below).
            ...
            if (seg.exec.segPhase.needsCapture()) {
              ...
              // BUILDING:WARMUP / BUILDING:COMPILING — the segment was reset/invalidated
              // (e.g. markCaptureErrorRetry on a capture failure) and never reached
              // CAPTURING. The previous guard `!isSealed()` is TRUE here, so it called
              // markCaptured → seal(), marking an UNCAPTURED segment SEALED; replay then
              // read stale capture-time data and the decode froze on one token (regression
              // from df8cee5d5f). Skip the seal so the segment re-warms → compiles →
              // captures on the next execution instead of sealing an empty graph.
  ...
  if (!didCompositeCapture) {
    ...
    //   - Composite capture failed (guard deactivated before fall-through)
      ...
      if (captureOk) {
        ...
        // Per-handle capture audit for fused monolithic backend execution. A graph
        // backend executes the entire range as one fused unit, so exact per-slot node
        // attribution is unavailable. Record the fused range's before/after node count
        // for every slot, matching the merged-island contract: nodesContributed>0 means
        // the captured fused range device-wrote that slot's outputs. Replay fixup must
        // never fall back to blanket actuality ticks for an audit-less handle.
        size_t monolithicAuditNodesBefore = 0;
        if (!nativeOnlyCapture) {
          handle->resetCaptureAudit();
          monolithicAuditNodesBefore = handle->getNumNodesDuringCapture(ctx.cudaStr);
          DSP_DIAG(EXECUTE, "Triton capture mid-check: %zu nodes recorded before executeSegment",
                   monolithicAuditNodesBefore);
        }
        ...
        if (!nativeOnlyCapture) {
          DSP_DIAG(EXECUTE,
                   "MONOLITHIC_CAPTURE: executeSegment returned status=%s (%d) for seg[%d-%d]",
                   statusName_gpu(captureStatus), static_cast<int>(captureStatus),
                   seg.def.startSlot, seg.def.endSlot);
          if (captureStatus == Status::OK) {
            size_t monolithicAuditNodesAfter =
                handle->getNumNodesDuringCapture(ctx.cudaStr);
            for (int s = seg.def.startSlot; s <= seg.def.endSlot; s++) {
              if (s < 0 || s >= numSlots_) continue;
              bool backendCapturedSlot = true;
#if HAVE_TRITON
              // Ordered-range monolithic capture skips native gap units. Attribute
              // those slots as zero-node so replay fixup executes them live instead
              // of falsely marking their never-written device buffers actual.
              for (const auto& unit : seg.exec.compositeReplaySchedule.units) {
                if (unit.kind == REPLAY_UNIT_GAP &&
                    s >= unit.startSlot && s <= unit.endSlot) {
                  backendCapturedSlot = false;
                  break;
                }
              }
#endif
        ...
        if (nativeOnlyCapture || captureStatus == Status::KERNEL_FAILURE) {
          ...
          //
          // Also execute value-shape CREATE ops (CONSTANT_GENERATION +
          // VALUE_DEPENDENT_SHAPE, e.g. range, create, lin_space) here.
          // Their inputs change every decode step (position IDs, sequence
          // lengths) so baking them into the CUDA graph causes
          // computeCreateOpValueKey to detect a mismatch → createValuesStable=false
          // → invalidateForRebuild every step → infinite capture-invalidate cycle
          // (replayCount stays at 1, plan never reaches REPLAYING state).
          // Running them live before each cudaGraphLaunch produces correct
          // per-step output values in the slot buffers that the graph reads.
          //
          // Execute both sets here before the fresh capture so downstream
          // compute ops see correct output slot pointers during capture.
          int viewPreExec = 0;
          int createOpPreExec = 0;
          for (int s = seg.def.startSlot; s <= seg.def.endSlot; s++) {
            if (slotHasOnlyTransparentAliasOutputs(
              ...
              if (isValueShapeCreate) {
                auto createStatus = executeSlot(s, effectiveExternalsForCapture, numExt, stream);
                if (createStatus != Status::OK) {
                  DSP_DIAG(EXECUTE,
                           "NATIVE_ONLY_CAPTURE: pre-capture create-op slot %d (%s) FAILED status=%s (%d)",
                           s, slots_[s].ident.opName.c_str(),
                           statusName_gpu(createStatus), static_cast<int>(createStatus));
                }
                createOpPreExec++;
              }
            }
          }
          if (viewPreExec > 0 || createOpPreExec > 0) {
            DSP_DIAG(EXECUTE, "NATIVE_ONLY_CAPTURE: pre-executed %d view/identity + %d value-shape-create slots outside capture scope",
                     viewPreExec, createOpPreExec);
          }
          ...
          captureStatus = Status::OK;
          {
            ...
            for (int s = seg.def.startSlot; s <= seg.def.endSlot; s++) {
              ...
              // These ops (range, create, lin_space) have inputs that change every
              // decode step (position IDs, step count).  Baking them into the CUDA
              // graph freezes their outputs to capture-time values.  Excluding them
              ...
              if (slotStatus != Status::OK) {
                DSP_DIAG(EXECUTE,
                         "NATIVE_ONLY_CAPTURE: slot %d (%s) FAILED status=%s (%d) after %d slots",
                         s, slots_[s].ident.opName.c_str(), statusName_gpu(slotStatus),
                         static_cast<int>(slotStatus), slotsExecuted);
          ...
          if (frozenSkipped > 0 || viewSkipped > 0 || createOpSkipped > 0) {
            DSP_DIAG(EXECUTE, "NATIVE_ONLY_CAPTURE: skipped %d frozen constant + %d view/identity + %d value-shape-create slots (of %d total)",
                     frozenSkipped, viewSkipped, createOpSkipped, seg.def.endSlot - seg.def.startSlot + 1);
          }
          // Record that create ops were excluded from the graph so replay
          // knows to execute them live before cudaGraphLaunch.
          if (createOpSkipped > 0) {
            seg.exec.markCreateOpsExcludedFromGraph(true, "native_only_capture_live_create",
                                                    createOpSkipped);
          ...
          DSP_DIAG(EXECUTE,
                   "NATIVE_ONLY_CAPTURE: seg[%d-%d] done — %d slots executed, %zu graph nodes, status=%s (%d)",
                   seg.def.startSlot, seg.def.endSlot, slotsExecuted, nativeNodes,
                   statusName_gpu(captureStatus), static_cast<int>(captureStatus));
        }
        ...
        DSP_DIAG(EXECUTE,
                 "Triton capture post-exec: %zu nodes, captureStatus=%s (%d)",
                 postExecNodes, statusName_gpu(captureStatus),
                 static_cast<int>(captureStatus));
        ...
        if (captureStatus == Status::OK) {
          ...
          DSP_DIAG(EXECUTE, "FATAL: Triton capture execution FAILED status=%s (%d) for seg[%d-%d]. "
                            "BLOCKING EXECUTION.",
                   statusName_gpu(captureStatus), static_cast<int>(captureStatus),
                   seg.def.startSlot, seg.def.endSlot);
          if (handle->isCapturing()) {
        ...
        if (endOk) {
          ...
          const int captureOutputSlot = dsp::firstSegmentOutputSlot(
              slots_, numSlots_, seg.def.startSlot, seg.def.endSlot,
              totalOutputSlots_);
          if (captureOutputSlot >= 0 && outputSlots_[captureOutputSlot] != nullptr) {
            auto* finalOut = outputSlots_[captureOutputSlot];
            if (finalOut->dataType() == FLOAT32) {
              DSP_DIAG_DUMP_SLOT("capture-post-endCapture", captureOutputSlot,
          ...
          // (matches GRAPH_REPLAY logic for apples-to-apples comparison)
          {
            ...
            if (captureOutputSlot < 0 || captureOutputSlot >= totalOutputSlots_) {
              captureOutputSlot = dsp::firstSegmentOutputSlot(
                  slots_, numSlots_, seg.def.startSlot, seg.def.endSlot,
                  totalOutputSlots_);
            }
            if (captureOutputSlot >= 0 && captureOutputSlot < totalOutputSlots_ &&
        ...
        if (launchOk) {
          const int replayOutputSlot = dsp::firstSegmentOutputSlot(
              slots_, numSlots_, seg.def.startSlot, seg.def.endSlot,
              totalOutputSlots_);
          if (replayOutputSlot >= 0 && outputSlots_[replayOutputSlot] != nullptr) {
            auto* finalOut = outputSlots_[replayOutputSlot];
            if (finalOut->dataType() == FLOAT32) {
              DSP_DIAG_DUMP_SLOT("capture-post-launch", replayOutputSlot,
                                 DSP_BUF(finalOut), finalOut->lengthOf());
            }
          }
          // Dump top logit from first replay (graph launch after capture) via DSP_DIAG
          const int replayArgmaxOutputSlot = dsp::firstSegmentOutputSlot(
              slots_, numSlots_, seg.def.startSlot, seg.def.endSlot,
              totalOutputSlots_);
          if (replayArgmaxOutputSlot >= 0 &&
              outputSlots_[replayArgmaxOutputSlot] != nullptr) {
            auto* out = outputSlots_[replayArgmaxOutputSlot];
            if (out->dataType() == FLOAT32 && out->lengthOf() > 0) {
              DSP_DIAG_DUMP_SEG_OUTPUT("REPLAY_LAUNCH", replayArgmaxOutputSlot,
                                       DSP_BUF(out), out->lengthOf(),
                                       seg.exec.executionCount, stream);
            }
          }
          ...
          //   - isSealed(): plan cache returned a previously captured+sealed segment;
          //     re-capturing updates handles but must NOT re-fire the lifecycle transition.
          //   - else (WARMUP/COMPILING): reset/invalidated, never reached CAPTURING → do
          //     NOT seal (the previous `!isSealed()` guard sealed an uncaptured segment →
          ...
          if (seg.exec.segPhase.needsCapture()) {
            ...
            // BUILDING:WARMUP / BUILDING:COMPILING — never reached CAPTURING. Skip the seal
            // so the segment re-warms → compiles → captures next execution rather than
            // sealing an uncaptured graph and replaying stale data.
          ...
          if (Environment::getInstance().tritonForceRecapture()) {
            ...
            markBatchD2DInvalidated(seg, "force_recapture_post_monolithic_capture");
  ...
  if (!syncResult.ok() || syncResult.effectiveExternals == nullptr) {
      DSP_DIAG(EXECUTE,
               ...
               static_cast<int>(syncResult.status), syncResult.cudaError);
      return setGpuBackendFailureDetail(
          seg, "Triton direct execution input staging failed: syncStatus=" +
                   std::to_string(static_cast<int>(syncResult.status)) +
                   ", cudaError=" + std::to_string(syncResult.cudaError) +
                   " (" + cudaGetErrorString(
                       static_cast<cudaError_t>(syncResult.cudaError)) + ")");
    }
    NDArray** directExternals = syncResult.effectiveExternals;
    ...
    if (status == Status::KERNEL_FAILURE) {
      ...
      status = Status::OK;
      {
        ...
        for (int s = seg.def.startSlot; s <= seg.def.endSlot; s++) {
          ...
          if (slotStatus != Status::OK) {
            DSP_DIAG(EXECUTE,
                     "NATIVE_DIRECT_EXEC: slot %d (%s) FAILED status=%s (%d)",
                     s, slots_[s].ident.opName.c_str(),
                     statusName_gpu(slotStatus), static_cast<int>(slotStatus));
            setGpuBackendFailureDetail(
      ...
      dsp::forEachSegmentOutputSlot(
          ...
          totalOutputSlots_, [&](int, int outputSlot, int) {
            if (outputSlots_[outputSlot] != nullptr &&
                outputSlots_[outputSlot]->dataBuffer() != nullptr) {
    ...
    if (DSP_DIAG_ENABLED(EXECUTE)) {
      ...
      const int directOutputSlot = dsp::firstSegmentOutputSlot(
          slots_, numSlots_, seg.def.startSlot, seg.def.endSlot,
          totalOutputSlots_);
      if (status == Status::OK && directOutputSlot >= 0 &&
          outputSlots_[directOutputSlot] != nullptr) {
        auto* finalOut = outputSlots_[directOutputSlot];
        if (finalOut->dataType() == FLOAT32) {
          DSP_DIAG_DUMP_SLOT("direct", directOutputSlot,
                             DSP_BUF(finalOut), finalOut->lengthOf());
        }
      }
      // Segment exit argmax
    ...
    DSP_DIAG(EXECUTE, "executeSegmentWithGpuGraph: exec%d seg[%d-%d]: backend=%s %s status=%s (%d) "
             ...
             statusName_gpu(status), static_cast<int>(status),
             seg.exec.executionCount,
    ...
    if (status != Status::OK) {
      setGpuBackendFailureDetail(
          seg, "capture/direct dispatch returned " +
                   std::string(statusName_gpu(status)) + " (" +
  ...
  if (status != Status::OK) {
    setGpuBackendFailureDetail(
        seg, "GPU graph capture dispatch returned " +
                 std::string(statusName_gpu(status)) + " (" +
                 std::to_string(static_cast<int>(status)) + ")");
  }
  return status;
...
Status NativeDynamicShapePlan::executeSegmentWithGpuGraph(
  ...
  if (!planLifecycle_.isSlotBySlot()) {
    ...
    // it refreshes/demotes wrappers — fresh wrappers expose new device addresses,
    // forcing the argTable refresh on the next replay (graph stays valid; no
    // recapture). No manual bump+reset needed here.
  ...
  if (runStaleBufferScan) {
    ...
    dsp::forEachSegmentOutputSlot(
        ...
        totalOutputSlots_, [&](int opIndex, int outputSlot, int) {
          // Never invalidate frozen constant outputs — their buffers were
          // validated at classification time and hold frozen refs.
          if (slots_[opIndex].frozenConstantSlot()) return;
          NDArray* cached = outputSlots_[outputSlot];
          if (cached != nullptr && cached->hasValidShapeInfo() && !cached->isEmpty()) {
            auto* db = cached->dataBuffer();
            if (db == nullptr || !db->isValid()) {
              DSP_DIAG_SLOT(MEMORY, outputSlot,
                            "STALE outputSlots_[%d] produced by op[%d] detected "
                            "(arr=%p, db=%p, dbValid=%d, frozenConst=%d). Invalidating.",
                            outputSlot, opIndex, (void*)cached, (void*)db,
                            db ? (db->isValid() ? 1 : 0) : -1,
                            slots_[opIndex].frozenConstantSlot() ? 1 : 0);
              outputSlots_[outputSlot] = nullptr;
              if (slots_[opIndex].slotPhase.isSealed() &&
                  slots_[opIndex].slotPhase.isConstant) {
                slots_[opIndex].slotPhase.isConstant = false;
              }
              invalidCount++;
            }
          }
    ...
    if (invalidCount > 0) {
      ...
      markBatchD2DInvalidated(seg, "stale_view_wrappers");
      DSP_THROW_SEG(COMPILE, seg.def.startSlot,
    ...
    for (int stepIdx = seg.def.startSlot; stepIdx <= seg.def.endSlot; stepIdx++) {
      ...
      for (int i = 0; i < slot.wiring.numInputs; i++) {
        ...
        if (srcIdx >= 0 && srcIdx < totalOutputSlots_ && outputSlots_[srcIdx] != nullptr) {
          // srcIdx is a flat output slot. Resolve its producing operation
          // before consulting operation-slot lifecycle state.
  ...
  if (preExecAllocCount > 0 && hasAnyCapturedGraph) {
    ...
    markBatchD2DInvalidated(seg, "output_addr_change");
  }
  ...
  // Capture gate. ModeContract.isSlotBySlot is the PERMANENT mode guard: it blocks
  // GEM_SLOT_BY_SLOT regardless of phase. Without it, SLOT_BY_SLOT composite-captured
  // the 2419-slot prefill and OOM-crashed once shapes froze and the TRANSIENT
  // planLifecycle_.isSlotBySlot() warmup guard went false. usesGraphCapture lets an
  // explicit graph mode (CUDA_GRAPHS/NVRTC_JIT/PTX_JIT/TRITON/AUTO) capture even when
  // tritonGraphCapture() is false (BenchmarkConfigApplier resets it for non-Triton
  // configs) — else NVRTC_JIT/PTX_JIT hit the MODE VIOLATION throw (NDSP.cpp:3948).
  const ModeContract modeContract = ModeContract::forMode(graphExecutionMode_);
  bool allowTritonCudaGraphReplay =
      (Environment::getInstance().tritonGraphCapture() || modeContract.usesGraphCapture)
  ...
  // Mixed segments (Triton islands + cuBLAS gaps) MUST take the COMPOSITE path:
  // islands captured into sub-graphs, cuBLAS gaps replayed LIVE each step with fresh
  ...
  if (canSkipReplayInvariantRecompute) {
    ...
    // BENIGN: the graph's address dependencies do not move. Comparing the raw input key
    // against the staged captured key could therefore NEVER match → extAddrsStable stuck
    // false → needsArgRefresh stuck true → argStable false → segmentHasStablePointersForPlanPhase
    // false → the plan never promoted SHAPES_FROZEN -> REPLAYING (permanent slot-by-slot,
    // ~8 tok/s) despite a ready graph. A captured Triton graph is address-stable by
    // construction; REAL drift (output-buffer realloc / workspace release) is caught
    // separately by capturedSlotAddrHash. Backends that bake the ACTUAL input device
    ...
    if (seg.resolvedGraphBackend != nullptr &&
      ...
      extAddrsStable = true;
               ...
               !seg.exec.replayHandle->getCapturedExternalAddresses().empty()) {
      extAddrsStable = externalAddrsMatch(seg, externalArrays, numExt);
    } else if (seg.exec.capturedInputAddrKey != 0) {
      extAddrsStable = (seg.exec.capturedInputAddrKey == segInputAddrKey);
    } else {
      extAddrsStable = false;
    }
  ...
  // ── Record the replay-stability decision in ONE place (encapsulated), then drive the
  // arg-generation counter from its verdict. Replaces the scattered extAddrs/createValues/
  // shapeKey checks that were recomputed independently per path (the desync that blocked
  // REPLAYING). Observable post-decode via getPlanSegmentStatisticsJson — DSP_DIAG is
  // silent in the native op. computeCreateOpValueKey() is schedule-aware: it hashes
  // only create ops whose values are actually baked into a graph. Live composite
  // gap creates and monolithic create-excluded slots return key=0 and are stable.
  {
    ...
    if (hasInternalValueShapeInputs || hasDynamicBoundaryInputs) {
      ...
      else if (!wasStable && nowStable) seg.exec.markArgsCurrent();
    }
  }
  ...
  // Composite captures only skip invalidation when computeCreateOpValueKey()
  // proves the create op is live (key=0). If a composite key mismatch remains,
  // a merged/island graph has a baked stale create value and must be rebuilt.
  //
  // ALSO skip invalidation for native-only (CUDA) monolithic captures where
  // value-shape create ops were EXCLUDED from the graph (createOpsExcludedFromGraph=true).
  ...
  if (!createValuesStable && seg.exec.replayHandle && !hasComposite &&
    ...
    DSP_DIAG(EXECUTE, "CREATE_VALUE_KEY mismatch: captured=%lld current=%lld — invalidating "
  ...
  if (allowTritonCudaGraphReplay &&
    ...
    markStaleGapGraphInvalidatedForDispatch(
        seg, "stale_gap_graph", captureMinExec,
        extAddrsStable, hasReplayHandle, replayHandleNull, isTritonCompiled);
  }
