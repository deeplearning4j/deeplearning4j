...
bool GraphUtils::filterOperations(GraphUtils::OpList& ops) {
  ...
  std::sort(filtered.begin(), filtered.end(),
            [](ops::OpDescriptor a, ops::OpDescriptor b) { return a.getOpName()->compare(*(b.getOpName())) < 0; });
  ...
  for (int e = 1; e < filtered.size(); e++) {
    if (0 == filtered[e].getOpName()->compare(name)) {
      ...
      auto fi = std::find_if(ops.begin(), ops.end(),
                             [name](ops::OpDescriptor a) { return a.getOpName()->compare(name) == 0; });
...
#ifdef ONEDNN_PATH
    args.emplace_back(std::string("-I" ONEDNN_PATH "/include"));
#endif
#ifdef OPENBLAS_PATH
