...
Span3::Span3(int64_t startX, int64_t stopX, int64_t incX, int64_t startY, int64_t stopY, int64_t incY, int64_t startZ,
             int64_t stopZ, int64_t incZ) {
  _startX = startX;
  _startY = startY;
  _startZ = startZ;
  _stopX = stopX;
...
Span3 Span3::build(int loop, uint64_t threadID, uint64_t numThreads, int64_t startX, int64_t stopX, int64_t incX,
                   int64_t startY, int64_t stopY, int64_t incY, int64_t startZ, int64_t stopZ, int64_t incZ) {
  switch (loop) {
    case 1: {
...
Span2 Span2::build(int loop, uint64_t threadID, uint64_t numThreads, int64_t startX, int64_t stopX, int64_t incX,
                   int64_t startY, int64_t stopY, int64_t incY) {
  switch (loop) {
    case 1: {
      auto span = (stopX - startX) / numThreads;
      auto s = span * threadID;
...
int ThreadsHelper::pickLoop2d(int numThreads, uint64_t itersX, uint64_t itersY) {
  ...
  // if there's no remainder left in some dimension - we're picking that dimension, because it'll be the most balanced
  // work distribution
  if (remX == 0) return 1;
  if (remY == 0) return 2;
...
int ThreadsHelper::numberOfThreads2d(int maxThreads, uint64_t iters_x, uint64_t iters_y) {
  ...
  if (iters_x < maxThreads && iters_y < maxThreads) return sd::math::sd_max<int>(iters_x, iters_y);

  auto remX = iters_x % maxThreads;
  auto remY = iters_y % maxThreads;

  // in some cases there's nothing to think about, part 2
  if ((iters_x >= maxThreads && remX == 0) || (iters_y >= maxThreads && remY == 0)) return maxThreads;
...
int Threads::parallel_tad(FUNC_1D function, int64_t start, int64_t stop, int64_t increment, uint32_t numThreads) {
  if (start > stop) throw std::runtime_error("Threads::parallel_for got start > stop");
...
int Threads::parallel_for(FUNC_2D function, int64_t startX, int64_t stopX, int64_t incX, int64_t startY, int64_t stopY,
                          int64_t incY, uint64_t numThreads, bool debug) {
  if (startX > stopX) throw std::runtime_error("Threads::parallel_for got startX > stopX");

  if (startY > stopY) throw std::runtime_error("Threads::parallel_for got startY > stopY");
  ...
  if (debug) {
    ...
    if (ticket != nullptr) {
      for (int e = 0; e < numThreads; e++) {
        ...
        ticket->enqueue(e, numThreads, function, span.startX(), span.stopX(), span.incX(), span.startY(), span.stopY(),
                        span.incY());
      }
...
int Threads::parallel_for(FUNC_3D function, int64_t startX, int64_t stopX, int64_t incX, int64_t startY, int64_t stopY,
                          int64_t incY, int64_t startZ, int64_t stopZ, int64_t incZ, uint64_t numThreads) {
  if (startX > stopX) throw std::runtime_error("Threads::parallel_for got startX > stopX");

  if (startY > stopY) throw std::runtime_error("Threads::parallel_for got startY > stopY");

  if (startZ > stopZ) throw std::runtime_error("Threads::parallel_for got startZ > stopZ");
  ...
  if (ticket != nullptr) {
    ...
    for (int e = 0; e < numThreads; e++) {
      ...
      auto span =
          Span3::build(splitLoop, thread_id, numThreads, startX, stopX, incX, startY, stopY, incY, startZ, stopZ, incZ);

      ticket->enqueue(e, numThreads, function, span.startX(), span.stopX(), span.incX(), span.startY(), span.stopY(),
                      span.incY(), span.startZ(), span.stopZ(), span.incZ());
    }
...
int Threads::parallel_do(FUNC_DO function, uint64_t numThreads) {
  ...
  if (ticket != nullptr) {
    ...
    for (uint64_t e = 0; e < numThreads - 1; e++) ticket->enqueue(e, numThreads, function);
...
int64_t Threads::parallel_long(FUNC_RL function, FUNC_AL aggregator, int64_t start, int64_t stop, int64_t increment,
                               uint64_t numThreads) {
  if (start > stop) throw std::runtime_error("Threads::parallel_long got start > stop");
  ...
  for (uint64_t e = 1; e < numThreads; e++) intermediatery[0] = aggregator(intermediatery[0], intermediatery[e]);

  // return accumulated result
  return intermediatery[0];
}

double Threads::parallel_double(FUNC_RD function, FUNC_AD aggregator, int64_t start, int64_t stop, int64_t increment,
                                uint64_t numThreads) {
  if (start > stop) throw std::runtime_error("Threads::parallel_long got start > stop");
...
int Threads::parallel_aligned_increment(FUNC_1D function, int64_t start, int64_t stop, int64_t increment,
                                        size_t type_size, uint32_t req_numThreads) {
  if (start > stop) throw std::runtime_error("Threads::parallel_for got start > stop");
  ...
  int adjusted_numThreads =
      samediff::ThreadsHelper::numberOfThreads(req_numThreads, (num_elements * sizeof(double)) / (200 * type_size));
