...
__global__ void transformStrictSimpleCached(
    const void* x,
    ...
    const sd::LongType* tadOffsets)
{
  ...
  functions::transform::TransformStrict<X, Z>::template transformCuda<OpType>(
...
SD_HOST void TransformStrict<X, Y>::executeTransformShaped(
    dim3 launchDims,
    cudaStream_t* stream,
    const int opNum,
    const void* x,
    ...
    const sd::LongType* tadOffsets)
{
  ...
  sd::DebugHelper::checkErrorCode(stream, "transformStrict executeTransformShaped(...) failed");
}
...
SD_DEVICE void TransformStrict<X, Z>::transformCuda(
    const void* vx,
    const sd::LongType* xShapeInfo,
    void* vparams,
    void* vz,
...
template <typename X, typename Z>
template <typename OpType>
SD_HOST void TransformStrict<X, Z>::intermediateShaped(
    dim3 launchDims,
    cudaStream_t* stream,
    const void* x,
    const sd::LongType* xShape,
    ...
    const sd::LongType* tadOffsets)
{
  ...
  transformStrictSimpleCached<X, Z, OpType>
  ...
  sd::DebugHelper::checkErrorCode(stream, "transformStrict(...) cached kernel failed");
}
