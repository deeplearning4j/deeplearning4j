...
//
// Shared device-side definitions for the CUDA random kernels. Splitting the
// random dispatch TUs keeps each translation unit's template instantiation
// load small enough for nvcc's frontend (cicc), which segfaults when the
// full single+double+triple matrix (4 types x 17 ops x 3 arities) is
// expanded inside one .cu file.
//
#ifndef PROJECT_LOOPS_CUDA_RANDOM_IMPL_H
#define PROJECT_LOOPS_CUDA_RANDOM_IMPL_H
...
#endif  // PROJECT_LOOPS_CUDA_RANDOM_IMPL_H
