...
void BroadcastBool<X, Z>::execInverse(const void *vx, const sd::LongType *xShapeInfo, const void *vy,
 ...
 sd::LongType yRank = shape::rank(yTadShapeShapeInfo);
 sd::LongType zRank = shape::rank(zTadShapeInfo);
 sd::LongType xRank = shape::rank(xShapeInfo);

 const sd::LongType *yShape = shape::shapeOf(yTadShapeShapeInfo);
 const sd::LongType *zShape = shape::shapeOf(zTadShapeInfo);
 const sd::LongType *xShape = shape::shapeOf(xShapeInfo);

 const sd::LongType *yStride = shape::stride(yTadShapeShapeInfo);
 const sd::LongType *zStride = shape::stride(zTadShapeInfo);
 ...
 if (shape::haveSameShapeAndStrides(yTadShapeShapeInfo, xShapeInfo) &&
   ...
   for (auto i = start; i < stop; i++) {
     ...
     for (sd::LongType f = 0; f < tadLength; f++) {
       sd::LongType coords[SD_MAX_RANK];
       INDEX2COORDS(f, zRank, zShape, coords);
       sd::LongType xOffset;
       COORDS2INDEX(xRank, xStride, coords, xOffset);
       sd::LongType yOffset;
       COORDS2INDEX(yRank, yStride, coords, yOffset);
       oZ[f] = OpType::op(x[xOffset], oY[yOffset], extraParams);
     }
   }
 }
}
