...
ConstantShapeBuffer* ConstantShapeHelper::storeAndWrapBuffer(const LongType* shapeInfo) {
 ...
 // Create early copy for validation on host
 ...
 delete[] buffer;  // Clean up host-side temporary buffer

 // Use DirectShapeTrie to handle storage and caching
 // Note: In CUDA version, the trie itself is still on host but the shape buffers it points to can be on device
...
ConstantShapeBuffer* ConstantShapeHelper::createShapeInfoWithUnitiesForBroadcast(const sd::LongType* maxShapeInfo,
 ...
 // Handle dimensions on host since this is shape calculation
 if (!dimensions.empty()) {
