...
int Parameters::getIntParam(std::string string) const {
  if (_intParams.count(string) == 0) throw std::runtime_error("Not available intParameter requested");
...
Parameters* Parameters::addIntParam(std::initializer_list<std::string> strings, std::initializer_list<int> params) {
  ...
  if (s.size() != p.size()) throw std::runtime_error("addIntParam: number of keys and values should match");
...
Parameters* Parameters::addArrayParam(std::initializer_list<std::string> strings,
  ...
  for (int e = 0; e < s.size(); e++) _arrayParams[s[e]] = std::vector<int>(p[e]);

  return this;
}

bool Parameters::getBoolParam(std::string string) const {
  if (_boolParams.count(string) == 0) throw std::runtime_error("Not available boolParameter requested");

  return _boolParams.at(string);
}

std::vector<int> Parameters::getArrayParam(std::string string) const {
  if (_arrayParams.count(string) == 0) throw std::runtime_error("Not available arrayParameter requested");
