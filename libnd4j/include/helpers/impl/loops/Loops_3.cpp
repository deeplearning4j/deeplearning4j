...
class TransformSameWrapper {
    ...
    public:
        ...
        static void wrap(const int opNum, const X *x, const Nd4jLong *xShapeInfo, X *z, const Nd4jLong *zShapeInfo, X *extras) {
            DISPATCH_BY_OPNUM_T(wrapper, PARAMS(x, xShapeInfo, z, zShapeInfo, extras), TRANSFORM_SAME_OPS);
        }
};


BUILD_SINGLE_TEMPLATE(template class ReduceSameWrapper, , LIBND4J_TYPES);
BUILD_SINGLE_TEMPLATE(template class TransformSameWrapper, , LIBND4J_TYPES);
}