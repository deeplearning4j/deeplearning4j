 ...
 public:
  ...
  void executeOnce() override {
    ...
    if (_z->isScalar() || _y == nullptr)
      if (_opType == 0)
        NativeOpExecutioner::execReduceFloatScalar(LaunchContext::defaultContext(), _opNum, _x->buffer(),
                                                   _x->shapeInfo(), _x->specialBuffer(), _x->specialShapeInfo(),
                                                   nullptr, _z->buffer(), _z->shapeInfo(), _z->specialBuffer(),
                                                   _z->specialShapeInfo());
      else
        NativeOpExecutioner::execReduceSameScalar(LaunchContext::defaultContext(), _opNum, _x->buffer(),
      ...
      if (_opType == 0)
        NativeOpExecutioner::execReduceFloat(LaunchContext::defaultContext(), _opNum, _x->buffer(), _x->shapeInfo(),
                                             _x->specialBuffer(), _x->specialShapeInfo(), nullptr, _z->buffer(),
                                             _z->shapeInfo(), _z->specialBuffer(), _z->specialShapeInfo(), nullptr,
                                             _axis.size());
      else
        NativeOpExecutioner::execReduceSame(LaunchContext::defaultContext(), _opNum, _x->buffer(), _x->shapeInfo(),
                                            _x->specialBuffer(), _x->specialShapeInfo(), nullptr, _z->buffer(),
                                            _z->shapeInfo(), _z->specialBuffer(), _z->specialShapeInfo(), nullptr,
                                            _axis.size());
    }
