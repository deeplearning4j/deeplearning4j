...
// /**
// *  this function reduce given matrix to  upper bidiagonal form (in-place operation), matrix must satisfy following
// condition rows >= cols
// *
...
// *  s - diagonal matrix with singular values of input matrix (non-negative) on the diagonal sorted in decreasing
// order,
// *      actually the mathematically correct dimension of s is [m, n], however for memory saving we work with s as
// vector [1, p], where p is smaller among m and n
...
// *  fullUV - if false then only p (p is smaller among m and n) first columns of u and v will be calculated and their
