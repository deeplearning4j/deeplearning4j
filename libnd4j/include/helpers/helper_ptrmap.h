...
class PointersHelper {
 private:
  ...
  // since that's pointers (which is 64-bit on 64bit systems), we limit number of maximum arguments to 1/2 of maxIndex
  // arguments
  int maxArguments;
 ...
 public:
   ...
   */
  SD_HOST_DEVICE
  PointersHelper(void *ptrToParams, int numAggregates, int maxArgs, int maxShapes, int maxIntArrays,
