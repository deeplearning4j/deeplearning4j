...
public class IndexAccumulationAlongDimensionDataBufferTask extends RecursiveTask<INDArray> {
    ...
    protected INDArray compute() {
        ...
        for( int i=0; i<nTensors; i++ ){
            ...
            if(canDoDirectly){
                if(y!=null){
                    task = opOnDimension.getIndexAccumulationOpDataBufferTask(0,opOnDimension.n(),dx,dy,x2.offset(),y2.offset(),
                            x2.elementWiseStride(),y2.elementWiseStride(),0,true);
                } else {
                    task = opOnDimension.getIndexAccumulationOpDataBufferTask(0, opOnDimension.n(), dx, null, x2.offset(), 0,
                            x2.elementWiseStride(), 0, 0, true);
                }
            } else {
