...
public class MeanOpDataBufferTask extends AccumulationDataBufferTask {

    public MeanOpDataBufferTask(Accumulation op, int threshold, int n, DataBuffer x, DataBuffer y,
    ...
    public MeanOpDataBufferTask(Accumulation op, int tadIdx, int tadDim, int threshold, INDArray x, INDArray y, boolean outerTask) {
        super(op, tadIdx, tadDim, threshold, x, y, outerTask);
    }

    @Override
    public double doTask() {
        //Task: accum = 1/N * sum_i x_i
        ...
        return (outerTask ? op.getAndSetFinalResult(sum) : sum);    //Division by N done in op.getAndSetFinalResult()
    }
    ...
    public AccumulationDataBufferTask getSubTask(int threshold, int n, DataBuffer x, DataBuffer y, int offsetX, int offsetY,
        ...
        return new MeanOpDataBufferTask(op, threshold, n, x, y, offsetX, offsetY, incrX, incrY, outerTask);
    }
