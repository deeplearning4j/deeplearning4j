...
public class IntervalNumber implements AbstractNumber {
    ...
    public  AbstractNumber add(AbstractNumber _b) {
        ...
        if (this instanceof IntervalNumber&&_b instanceof IntervalNumber){
            IntervalNumber a = (IntervalNumber)this;
            ...
            switch (a.getTypeOfInterval()){
                case P:
                    switch (b.getTypeOfInterval()){
                        case P:      {return new IntervalNumber(a.low.multiply(b.low, mcLow),a.high.multiply(b.high, mcHigh));}
                        case Z:      {return new IntervalNumber(a.high.multiply(b.low, mcLow),a.high.multiply(b.high, mcHigh));}
                        case negP:   {return new IntervalNumber(a.high.multiply(b.low, mcLow),a.low.multiply(b.high, mcHigh));}
                        case dualZ:  {return new IntervalNumber(a.low.multiply(b.low, mcLow),a.low.multiply(b.high, mcHigh));} // Куда округлять на границах?
                    }
                case Z:
                    switch (b.getTypeOfInterval()){
                        case P:      {return new IntervalNumber(a.low.multiply(b.high, mcLow),a.high.multiply(b.high, mcHigh));}
                        case Z:      {return new IntervalNumber(a.low.multiply(b.high,mcLow).min(a.high.multiply(b.low, mcLow)), a.low.multiply(b.low, mcHigh).max(a.high.multiply(b.high, mcHigh)));}
                        case negP:   {return new IntervalNumber(a.high.multiply(b.low, mcLow),a.low.multiply(b.low, mcHigh));}
                        case dualZ:  {return new IntervalNumber(BigDecimal.ZERO,BigDecimal.ZERO);}
                    }
                case negP:
                    switch (b.getTypeOfInterval()){
                        case P:      {return new IntervalNumber(a.low.multiply(b.high, mcLow), a.high.multiply(b.low, mcHigh));}
                        case Z:      {return new IntervalNumber(a.low.multiply(b.high, mcLow), a.low.multiply(b.low, mcHigh));}
                        case negP:   {return new IntervalNumber(a.high.multiply(b.high, mcLow),a.low.multiply(b.low, mcHigh));}
                        case dualZ:  {return new IntervalNumber(a.high.multiply(b.high, mcLow),a.high.multiply(b.low, mcHigh));}
                    }
                case dualZ:
                    switch (b.getTypeOfInterval()){
                        case P:      {return new IntervalNumber(a.low.multiply(b.low, mcLow),a.high.multiply(b.low, mcHigh));}
                        case Z:      {return new IntervalNumber(BigDecimal.ZERO,BigDecimal.ZERO);}
                        case negP:   {return new IntervalNumber(a.high.multiply(b.high, mcLow),a.low.multiply(b.high, mcHigh));}
                        case dualZ:  {return new IntervalNumber(a.low.multiply(b.low, mcLow).max(a.high.multiply(b.high, mcLow)), a.low.multiply(b.high, mcHigh).min(a.high.multiply(b.low, mcHigh)));}
                    }
            }
        }
        throw new ArithmeticException("Unexpected arithmetical error!!!");
...
*/
    ...
    public  boolean equals(Object obj) {
        ...
        if (obj instanceof IntervalNumber && this instanceof IntervalNumber){
            ...
            IntervalNumber thisInterval=(IntervalNumber)this;
    ...
    public   typeOfInterval getTypeOfInterval(){
        ...
        else if (low.compareTo(BigDecimal.ZERO)<=0&&high.compareTo(BigDecimal.ZERO)<=0) return typeOfInterval.negP;
        else if (high.compareTo(BigDecimal.ZERO)<=0&&0<=low.compareTo(BigDecimal.ZERO)) return typeOfInterval.dualZ;
        return null;
    };
