...
이 경우에 인공 신경망은 [Iterative Reduce (반복적인 리듀스 작업)](iterativereduce)를 통해 병렬로 학습되는데, 이 학습 작업은 자바, [스칼라](http://nd4j.org/scala.html) 및 [Clojure](https://github.com/wildermuthn/d4lj-iris-example-clj/blob/master/src/dl4j_clj_example/core.clj)와 모두 호환됩니다. 이렇게 Deeplearning4j는 오픈 스택에서 편하게 사용할 수 있도록 모듈화되어있습니다. 이 덕분에 우리는 마이크로 서비스 아키텍처에 최초로 딥러닝 기술을 적용하고 있습니다.
...
마지막으로, [ND4J는 저희가 사용하는 자바 기반 연산 엔진](http://nd4j.org/kr-index)입니다. 크기가 큰 행렬을 다루는 경우 벤치마크에서 [ND4J는 Numpy 대비 대략 두 배 가까이 빠른 연산성능](http://nd4j.org/benchmarking)을 보여줍니다.
