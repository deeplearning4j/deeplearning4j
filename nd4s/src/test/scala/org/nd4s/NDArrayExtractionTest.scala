...
trait NDArrayExtractionTestBase extends FlatSpec { self: OrderingForTest =>
  ...
  it should "be able to extract a part of 2d matrix with double data and offset" ignore {
    val ndArray = (1 to 8).map(_.toDouble).mkNDArray(Array(2, 2), NDOrdering.C, offset = 4)
