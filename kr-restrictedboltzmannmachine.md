...
RBM의 구조는 상대적으로 단순한 편입니다. 또 RBM은 심층 신경망의 학습을 돕기 위해 쓰이기도 했습니다. 이제부터 RBM의 작동 원리를 설명드리겠습니다.

RBM은 심층 신경망이 아닙니다. RBM은 두 개의 층으로 구성되어있고 이 RBM은 *심층 신뢰 신경망*(DBN:Deep Belief Network)의 구성 요소이기도 합니다. 아래의 그림을 참고하시기 바랍니다. 첫번째 층은 우리가 볼 수 있는 층인 가시층(visible layer), 혹은 데이터가 입력되는 입력층이고 두번째 층은 특징값이 학습되는 은닉층입니다.
...
즉 층 내부의 연결은 전혀 없으며 이 구조때문에 '제한된' 볼츠만 머신이라는 이름을 붙인 것입니다. 입력층의 노드는 데이터를 입력으며 입력받은 데이터를 은닉층에 얼마나 전달할 것인지를 [확률에 따라 결정(stochastic decision)](../glossary.html#stochasticgradientdescent)합니다. 즉 확률에 따라 입력을 전달할지(1) 혹은 전달하지 않을지(0)를 결정합니다. 
