...
TEST(Shape,ShapeInfoBuffer) {
    ...
    shape::ShapeInformation info(shape, stride, 0, rank);
...
TEST(Shape,Keep) {
    ...
    int keepAssertion2[2];
    keepAssertion2[0] = 2;
    keepAssertion2[1] = 3;

    int keepAssertionIndexes2[2];
    for (int i = 0; i < 2; i++)
...
TEST(Shape,PermuteSwap) {
    ...
    int *permuted = shape::doPermuteSwap(3, range, reverse);
    CHECK(arrsEquals<int>(3, permuted, reverse));
...
TEST(Shape,TadOffset) {
    ...
    int elementWiseStrides[] = {12,6,2,1};
    int tadLengths[] = {12,12,8,12};
